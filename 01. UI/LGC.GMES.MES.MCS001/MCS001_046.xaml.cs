...
--------------------------------------------------------------------------------------
 [Change History]
  2020.09.04  서동현 책임 : Initial Created.    
...
{
    /// <summary>
    /// MCS001_046.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_046 : UserControl, IWorkArea
    {
        ...
        private void btnTransferCancel_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                CMM_MCS_TRANSFER_CANCEL_BY_FP popupTransferCancel = new CMM_MCS_TRANSFER_CANCEL_BY_FP { FrameOperation = FrameOperation };
        ...
        {
            CMM_MCS_TRANSFER_CANCEL_BY_FP popup = sender as CMM_MCS_TRANSFER_CANCEL_BY_FP;
		...
		private void dgStkInfo_LoadedCellPresenter(object sender, DataGridCellEventArgs e)
		{
			...
			dataGrid?.Dispatcher.BeginInvoke(new Action(() =>
			{
				...
				if (e.Cell.Row.Type == DataGridRowType.Item)
				{
					if (Convert.ToString(e.Cell.Column.Name) == "REQ_CNT")
					{
						e.Cell.Presenter.Foreground = new SolidColorBrush(Colors.Blue);
						e.Cell.Presenter.FontWeight = FontWeights.Bold;
					}
		...
		private void dgStkInfo_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
		{
			...
			strZoneId = DataTableConverter.GetValue(drv, "ZONEID").GetString();
		...
		private void SelectOutRackList(bool IsBinding, string cstStat, string cstProd, string prodId)
		{
			...
			_requestOutRackInfoTable = new ClientProxy(_bizRuleIp, _bizRuleProtocol, _bizRulePort, _bizRuleServiceMode, _bizRuleServiceIndex).ExecuteServiceSync(bizRuleName, "RQSTDT", "RSLTDT", inDataTable);
        ...
        {
            const string bizRuleName = "BR_MCS_SEL_STK_FORMATION_REALPALLET_DETAIL2";
            try
            {
					...
					if (CommonVerify.HasTableRow(_requestTransferInfoTable))
                    {
                        foreach (DataRow row in bizResult.Rows)
                        {
                            ...
                            if (query != null)
                            {
								...
								row["ORD_STAT_NAME"] = ObjectDic.Instance.GetObjectName(query.RequestTransferState);
        ...
        private void SelectRequestTransferList(DataTable dt)
        {
            ...
            {
                strCstIdList += "'" + row["CSTID"].ToString() + "',";
            }
            ...
            const string bizRuleName = "DA_SEL_MCS_REQ_TRF_BY_MES_GUI";
            DataTable inDataTable = new DataTable("RQSTDT");
			inDataTable.Columns.Add("LANGID", typeof(string));
            ...
            DataRow dr = inDataTable.NewRow();
			dr["LANGID"] = LoginInfo.LANGID;
			dr["CSTID_LIST"] = strCstIdList == "" ? null : strCstIdList;
            inDataTable.Rows.Add(dr);

            _requestTransferInfoTable = new ClientProxy(_bizRuleIp, _bizRuleProtocol, _bizRulePort, _bizRuleServiceMode, _bizRuleServiceIndex).ExecuteServiceSync(bizRuleName, "RQSTDT", "RSLTDT", inDataTable);
        }

        private void SelectNoPallet()
        {
            const string bizRuleName = "DA_MCS_SEL_FORMATION_STK_EMPTY_PALLET";
            try
            {
                ShowLoadingIndicator();
                ...
                dr["LANGID"] = LoginInfo.LANGID;
                dr["AREAID"] = LoginInfo.CFG_AREA_ID;
                dr["EQGRID"] = "FCW";   //cboStockerType.SelectedValue;
                dr["BLDGCODE"] = cboArea.SelectedValue.ToString() == "" ? null : cboArea.SelectedValue.ToString();
        ...
        private void SelectNoPalletList()
        {
            ...
            const string bizRuleName = "DA_MCS_SEL_FORMATION_STK_EMPTY_PALLET_LIST";
        ...
        private void SelectAbnormalPalletList()
        {
            ...
            //Util.gridClear(dgPortInfo);
            const string bizRuleName = "DA_MCS_SEL_FORMATION_STK_ABNORM_PALLET_LIST";
        ...
        private void SelectPortInfo(string bldgCode, string equipmentCode, string locId, string cstStat)
		{
            try
            {
                ShowLoadingIndicator();
		...
		private void SelectStkInfo(string bldgCode)
		{
			try
			{
				ShowLoadingIndicator();

				const string bizRuleName = "DA_SEL_MCS_EQPT_INFO_EQPT";
		...
		private void SaveManualIssue()
        {
            ...
            {
                ShowLoadingIndicator();
                const string bizRuleName = "BR_GUI_REG_TRF_JOB_BY_MANUAL";

                DateTime dtSystem = GetSystemTime();

                DataTable inTable = new DataTable("IN_DATA");
                inTable.Columns.Add("SRC_EQPTID", typeof(string));      // 추가
                inTable.Columns.Add("SRC_LOCID", typeof(string));
                inTable.Columns.Add("DST_EQPTID", typeof(string));      // 추가
                ...
                inTable.Columns.Add("PRODID", typeof(string));          // 추가
                inTable.Columns.Add("CARRIER_STRUCT", typeof(string));  // 추가
                inTable.Columns.Add("MDL_TP", typeof(string));          // 추가
				inTable.Columns.Add("STK_ISS_TYPE", typeof(string));    // 추가
        ...
        private void InitializeRequestTransferTable()
        {
            ...
            _requestTransferInfoTable.Columns.Add("MACRO_JOBID", typeof(string));
			_requestTransferInfoTable.Columns.Add("STK_ISS_TYPE", typeof(string));
		...
		private void SelectRealPallet(C1DataGrid dg)
		{
			try
			{
				...
				new ClientProxy().ExecuteService("BR_MCS_SEL_STK_FORMATION_REALPALLET2", "INDATA", "OUTDATA", dt, (result, ex) =>
				{
					if (ex != null)
					{
						Util.MessageException(ex);
                    ...
                    */
					...
					{ }).Select(g => new
					{
						...
						ShipCellQty = g.Sum(x => x.Field<decimal>("SHIP_CELL_QTY")),
						HoldCellQty = g.Sum(x => x.Field<decimal>("HOLD_CELL_QTY")),
						InspWaitCellQty = g.Sum(x => x.Field<decimal>("INSP_WAIT_CELL_QTY")),
						NGCellQty = g.Sum(x => x.Field<decimal>("NG_CELL_QTY")),
						VldDateCellQty = g.Sum(x => x.Field<decimal>("VLD_DATE_CELL_QTY")),
						OUT_PLT_CNT_Qty = g.Sum(x => x.Field<Int32>("OUT_PLT_CNT")),
                ...
                if (Math.Abs(rowCount.Value) >= 0)
                {
                    ...
                    if (CommonVerify.HasDataGridRow(dgRealPalletDetail))
                    {
                        ...
                        if (lastIdx >= 0)
                        {
                            ...
                            SetOutPort(cboOutPort, selectedequipmentCode, selectedRackId, selectedCstStat);
        ...
        private void SetOutPort(C1ComboBox cbo, string equipmentCode, string rackId, string cstStat)
        {
            try
            {
                ...
                DataTable dtResult = new ClientProxy(_bizRuleIp, _bizRuleProtocol, _bizRulePort, _bizRuleServiceMode, _bizRuleServiceIndex).ExecuteServiceSync("BR_GUI_GET_GET_TRF_DEST", "IN_EQPT_INFO", "OUT_DEST_INFO", inTable);
                */
				...
				if (cboDstType.SelectedValue.ToString().ToUpper().Equals("PORT"))
				{
					...
					DataTable dtResult = new ClientProxy(_bizRuleIp, _bizRuleProtocol, _bizRulePort, _bizRuleServiceMode, _bizRuleServiceIndex).ExecuteServiceSync("DA_SEL_DST_INFO_BY_MANUAL_UI", "RQSTDT", "RSLTDT", inTable);
					...
					foreach (DataRow row in dtResult.Rows)
					{
						...
						*/
						...
						C1ComboBoxItem comboBoxItem = new C1ComboBoxItem();
						comboBoxItem.Content = row["DST_LOC_GROUP_NAME"].GetString();
						//comboBoxItem.Tag = row["DST_LOC_GROUPID"].GetString();
						comboBoxItem.Tag = row["DST_EQPTID"].GetString() + "|" + row["DST_LOC_GROUPID"].GetString();
