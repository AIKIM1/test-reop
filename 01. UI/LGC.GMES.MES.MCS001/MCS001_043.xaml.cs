/*************************************************************************************
 Created Date : 2020.04.15
      Creator : 이상준
   Decription : CNJ 점보롤 스토커 모니터링
--------------------------------------------------------------------------------------
 [Change History]
    2020.05.04  이상준 차장 : CSR[C20200311-000027] 범례 표시를 Commoncode에서 가져오도록 수정함.
    2024.06.11  park7142    : ESNJ 7동 양극 점보롤 STK 프로젝트 요구사항에 따라 수정.
**************************************************************************************/
using System;
...
{
    /// <summary>
    /// MCS001_043.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_043 : UserControl, IWorkArea
    {
        ...
        {
            List<Button> listAuth = new List<Button> { btnSelect, btnSearch, btnwarehousingHistory };
        ...
        {
            if (!ValidationWareHousingHistory())
                return;
        ...
        private void _monitorTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (CommonVerify.HasDataGridRow(dgRackInfo) || CommonVerify.HasTableRow(_dtRackInfo))
                return;
        ...
        {
            if (!ValidationSelectJumboRollMonitoring())
                return;
        ...
        private void btnFiFoManualIssue_Click(object sender, RoutedEventArgs e)
        {
            ...
            _monitorTimer?.Stop();
            MCS001_043_FIFO_RELEASE popupFiFoManualIssue = new MCS001_043_FIFO_RELEASE { FrameOperation = FrameOperation };
        ...
        {
            MCS001_043_FIFO_RELEASE popup = sender as MCS001_043_FIFO_RELEASE;
        ...
        {
            UcRackStair ucRackStair = sender as UcRackStair;
        ...
        private void SelectJumboRollMonitoring()
        {
            try
            {
                ...
                const string bizRuleName = "DA_MCS_SEL_STK_JUMBOROLL_MONITORING_NJ";
                ...
                UcRackStair ucRackStair = new UcRackStair();

                scrollViewer1.Visibility = Visibility.Hidden;
                scrollViewer2.Visibility = Visibility.Hidden;
                ...
                if (CommonVerify.HasTableRow(bizResult))
                {
                    foreach (DataRow item in bizResult.Rows)
                    {
                        ...
                        {
                            case "1":
                                ucRackStair = _rackStairs1[x][y];
                                scrollViewer1.Visibility = Visibility.Visible;
                                break;
                            case "2":
                                ucRackStair = _rackStairs2[x][y];
                        ...
                        if (string.Equals(item["RACK_STAT_CODE"].GetString(), "UNUSE")) //입고금지
                        {
                            ...
                            ucRackStair.RootLayout.Background = new SolidColorBrush((Color)convertFromString);
                        }
                        ...
                        {
                            if (string.Equals(item["CSTSTAT"].GetString(), "E") && string.Equals(item["ABNORM_TRF_RSN_CODE"].GetString(), "LOT_TERM_BY_LOSS"))  // 공SKID(LOT O)
                            {
                                convertFromString = System.Windows.Media.ColorConverter.ConvertFromString("#85A8FC");
                                ucRackStair.RootLayout.Background = new SolidColorBrush((Color)convertFromString);
                            }
                            else if(string.Equals(item["CSTSTAT"].GetString(), "E"))  // 공SKID(LOT X)
                            {
                                convertFromString = System.Windows.Media.ColorConverter.ConvertFromString("#FF87CEFA");
                                ucRackStair.RootLayout.Background = new SolidColorBrush((Color)convertFromString);
                            }                             
                            else
                            {
        ...
        {
            int xposition = _maxRow == Convert.ToInt16(zPosition) ? 0 : _maxRow - Convert.ToInt16(zPosition);
            return xposition;
        }
        ...
        private void SelectJumboRollInventory()
        {
            try
            {
                ...
                const string bizRuleName = "DA_MCS_SEL_STK_JUMBOROLL_INVENTORY_ESKID_NJ";
                DataTable inDataTable = new DataTable("RQSTDT");
                inDataTable.Columns.Add("LANGID", typeof(string));
        ...
        {
            MCS001_043_MANUAL_ISSUE popupManualIssue = new MCS001_043_MANUAL_ISSUE { FrameOperation = FrameOperation };
        ...
        {
            MCS001_043_MANUAL_ISSUE popup = sender as MCS001_043_MANUAL_ISSUE;
        ...
        {
            MCS001_043_RACK_INFO popupRackInfo = new MCS001_043_RACK_INFO { FrameOperation = FrameOperation };
            object[] parameters = new object[2];
            parameters[0] = cboStocker.SelectedValue;
            parameters[1] = dt;
            C1WindowExtension.SetParameters(popupRackInfo, parameters);
        ...
        {
            MCS001_043_RACK_INFO popup = sender as MCS001_043_RACK_INFO;
        ...
        private void MakeRowDefinition()
        {
            ...
            for (int i = 0; i < _maxRow; i++)
            {
                ...
                textBlock1.Text = _maxRow - i + ObjectDic.Instance.GetObjectName("단");
                ...
                textBlock2.Text = _maxRow - i + ObjectDic.Instance.GetObjectName("단");
        ...
        {
            _rackStairs1 = new UcRackStair[_maxRow][];
            _rackStairs2 = new UcRackStair[_maxRow][];
            //_rackStairs3 = new UcRackStair[_maxRow][];
            //_rackStairs4 = new UcRackStair[_maxRow][];
            //_rackStairs5 = new UcRackStair[_maxRow][];

            for (int r = 0; r < _rackStairs1.Length; r++)
            {
                _rackStairs1[r] = new UcRackStair[_maxCol];
                _rackStairs2[r] = new UcRackStair[_maxCol];
                //_rackStairs3[r] = new UcRackStair[_maxCol];
                //_rackStairs4[r] = new UcRackStair[_maxCol];
                //_rackStairs5[r] = new UcRackStair[_maxCol];
                ...
                //{
                //    UcRackStair ucRackStair3 = new UcRackStair
                //    {
                //        Name = $"r{r:0}c{c:00}",
                //        ProjectName = string.Empty,
                //    };

                //    ucRackStair3.Checked += UcRackStair3_Checked;
                //    ucRackStair3.Click += UcRackStair3_Click;
                //    ucRackStair3.DoubleClick += UcRackStair3_DoubleClick;
                //    _rackStairs3[r][c] = ucRackStair3;
                //}

                //for (int c = 0; c < _rackStairs4[r].Length; c++)
                //{
                //    UcRackStair ucRackStair4 = new UcRackStair
                //    {
                //        Name = $"r{r:0}c{c:00}",
                //        ProjectName = string.Empty,
                //    };

                //    ucRackStair4.Checked += UcRackStair4_Checked;
                //    ucRackStair4.Click += UcRackStair4_Click;
                //    ucRackStair4.DoubleClick += UcRackStair4_DoubleClick;
                //    _rackStairs4[r][c] = ucRackStair4;
                //}

                //for (int c = 0; c < _rackStairs5[r].Length; c++)
                //{
                //    UcRackStair ucRackStair5 = new UcRackStair
                //    {
                //        Name = $"r{r:0}c{c:00}",
                //        ProjectName = string.Empty,
                //    };

                //    ucRackStair5.Checked += UcRackStair5_Checked;
                //    ucRackStair5.Click += UcRackStair5_Click;
                //    ucRackStair5.DoubleClick += UcRackStair5_DoubleClick;
                //    _rackStairs5[r][c] = ucRackStair5;
                //}
            }
        }
        ...
        private bool ValidationManualIssue()
        {
            ...
            if (DataTableConverter.GetValue(dgRackInfo.Rows[0].DataItem, "RACK_STAT_CODE").GetString().Equals("USING") || DataTableConverter.GetValue(dgRackInfo.Rows[0].DataItem, "RACK_STAT_CODE").GetString().Equals("NOREAD"))
            {
                ...
                {
                    if (string.IsNullOrEmpty(DataTableConverter.GetValue(dgRackInfo.Rows[0].DataItem, "MCS_CST_ID").GetString()))
                    {
        ...
        private void SetLegendColorCombo()
        {
            ...
            drColor["CMCDTYPE"] = "JRW_MNT_LOT_LEGEND";
        ...
        private void SendCancelLogis()
        {
            try
            {
                ...
                if (indataSet.Tables["INDATA"].Rows?.Count <= 0)
                {
                    HiddenLoadingIndicator();
        ...
        private void SelectInoutReservationInfo()
        {
            try
            {
                ...
                const string bizRuleName = "DA_MCS_SEL_STK_JUMBOROLL_INOUT_RSV_CMD_NJ";
                DataTable inDataTable = new DataTable("RQSTDT");
                inDataTable.Columns.Add("LANGID", typeof(string));
                inDataTable.Columns.Add("AREAID", typeof(string));
                inDataTable.Columns.Add("EQPTID", typeof(string));
                ...
                new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inDataTable, (bizResult, bizException) =>
                {
                    ...
                    bizResult.Columns.Add("CHK", typeof(string));
