/*************************************************************************************
 Created Date : 2019.01.21
      Creator : 오화백
   Decription : 노칭대기창고 수동출고 예약 팝업 - 호출대상 UI(노칭 대기창고 모니터링)
--------------------------------------------------------------------------------------
 [Change History]
...
namespace LGC.GMES.MES.MCS001
{
    ...
    /// </summary>
    public partial class MCS001_022_MANUAL_ISSUE : C1Window, IWorkArea
    {
        ...
        public MCS001_022_MANUAL_ISSUE()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                if ((TabLotControl.SelectedItem as C1TabItem)?.Name == "TabRackMove")
                {
                    //bizRuleName = "BR_MCS_REG_LOGIS_CMD_PSR_PSR";
                     bizRuleName = "BR_MCS_REG_LOGIS_CMD_NOTCHING_PSR_PSR";
                    ...
                    dr["MCS_CST_ID"] = DataTableConverter.GetValue(dgRackInfo.Rows[0].DataItem, "MCS_CST_ID").GetString();
                    inCst.Rows.Add(dr);
                    ...
                    //    }
                    //});
                    new ClientProxy().ExecuteService_Multi(bizRuleName, "INCST,INDATA", null, (bizResult, bizException) =>
                    {
                        try
                        {
                            HiddenLoadingIndicator();
                ...
                else
                {
                    ...
                    bizRuleName = "BR_MCS_REG_LOGIS_CMD_NOTCHING_PSR_PSP";
