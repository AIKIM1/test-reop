...
{
    /// <summary>
    /// MCS001_018.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_018 : UserControl, IWorkArea
    {
        ...
        {
            List<Button> listAuth = new List<Button> { btnSelect, btnSearch, btnSKIDSearch, btnInputLotSearch, btnManualIssue, btnPancakeWarehousingHistory };
        ...
        private void btnPancakeWarehousingHistory_Click(object sender, RoutedEventArgs e)
        {
            ...
            FrameOperation.OpenMenu("SFU010180130", true, parameters);
        }
        

        private void btnInputLotSearch_Click(object sender, RoutedEventArgs e)
        {
            MCS001_018_INPUT_LOT popupInputLot = new MCS001_018_INPUT_LOT { FrameOperation = FrameOperation };
            object[] parameters = new object[2];
        ...
        {
            MCS001_018_INPUT_LOT popup = sender as MCS001_018_INPUT_LOT;
            if (popup != null)
            {
        ...
        private void popupSkidState_Closed(object sender, EventArgs e)
        {
        ...
        private void cboAutoSearch_SelectedValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            try
            {
                if (_monitorTimer != null)
                {
                    ...
                    {
                        //전극 Pancake 창고 모니터링 자동조회  %1초로 변경 되었습니다.
                        if (cboAutoSearch != null)
                            Util.MessageInfo("SFU6011", cboAutoSearch.SelectedValue.GetString());
                    }
                }
            }
            catch (Exception ex)
        ...
        private void dgInputRank_UnloadedCellPresenter(object sender, DataGridCellEventArgs e)
        {
            if (sender == null)
                return;
        ...
        private void btnWareHouseSetup_Click(object sender , RoutedEventArgs e)
        {
            _monitorTimer?.Stop();
            MCS001_018_WAREHOUSE_SETUP popupSetup = new MCS001_018_WAREHOUSE_SETUP { FrameOperation = FrameOperation };
            object[] parameters = new object[2];
            parameters[0] = cboStocker.SelectedValue;
            parameters[1] = null;

            C1WindowExtension.SetParameters(popupSetup, parameters);

            popupSetup.Closed += popupSetup_Closed;
            Dispatcher.BeginInvoke(new Action(() => popupSetup.ShowModal()));
        }

        private void popupSetup_Closed(object sender, EventArgs e)
        {
            MCS001_018_WAREHOUSE_SETUP popup = sender as MCS001_018_WAREHOUSE_SETUP;
        ...
        private void UcPancakeRackStair1_DoubleClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                DataTable dtRackInfo = _dtRackInfo.Clone();
        ...
        {
            MCS001_018_RACK_INFO popup = sender as MCS001_018_RACK_INFO;
            if (popup != null && popup.IsUpdated)
            {
                _dtRackInfo.Clear();
                SelectPancakeMonitoring();
        ...
        {
            MCS001_018_SKID_STATE popupSkidState = new MCS001_018_SKID_STATE { FrameOperation = FrameOperation };

            popupSkidState.Closed += popupSkidState_Closed;
            Dispatcher.BeginInvoke(new Action(() => popupSkidState.ShowModal()));
        }
        #endregion
        ...
        private void ManualIssue()
        {
            ...
            MCS001_018_MANUAL_ISSUE popupManualIssue = new MCS001_018_MANUAL_ISSUE { FrameOperation = FrameOperation };
            object[] parameters = new object[2];
            parameters[0] = cboStocker.SelectedValue;
        ...
        {
            MCS001_018_MANUAL_ISSUE popup = sender as MCS001_018_MANUAL_ISSUE;
            if (popup != null && popup.IsUpdated)
            {
                _dtRackInfo.Clear();
                SelectPancakeMonitoring();
        ...
        {
            MCS001_018_RACK_INFO popupRackInfo = new MCS001_018_RACK_INFO { FrameOperation = FrameOperation };
            object[] parameters = new object[2];
            parameters[0] = cboStocker.SelectedValue;
            parameters[1] = dt;
