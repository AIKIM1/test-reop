...
{
    public partial class MCS001_083_LOTLIST : C1Window, IWorkArea
    {
        ...
        private string _inputbom;


        public MCS001_083_LOTLIST()
        {
            InitializeComponent();
        }
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (parameters != null)
            {
                ...
                // 원복
                //_woname = Util.NVC(parameters[4]);
                //_wotype = Util.NVC(parameters[5]);
                //_inputbom = Util.NVC(parameters[6]);
        ...
        private void SelectLotListByWorkOrder()
        {
            try
            {
                ...
                const string bizRuleName = "BR_MHS_SEL_WAREHOUSE_SUMMARY_BY_ASSY_WO_LOT_LIST";
                ...
                new ClientProxy().ExecuteService(bizRuleName, "INDATA", "OUTDATA", inTable, (bizResult, bizException) =>
                {
                    ...
                    if (_type == "QTY_AF_NT_SWW")
                    {
                        if (bizResult.Rows.Count > 0)
                        {
                            ...
                            List<string> sIdList = bizResult.AsEnumerable().Select(c => c.Field<string>("GR_CSTID")).Distinct().ToList();
                            ...
                            for (int i = 0; i < GrTray.Rows.Count; i++)
                            {
                                ...
                                {
                                    if (GrTray.Rows[i]["GR_CSTID"].ToString() == bizResult.Rows[j]["GR_CSTID"].ToString())
                                    {
                                        bizResult.Rows[j]["ROWNUM"] = GrTray.Rows[i]["ROWNUM"];
                                    }
                                }
                            }
            ...
            try
            {
                ...
                for (int i = dgList.TopRows.Count; i < dgList.Rows.Count; i++)
                {
                    ...
                    if (dgList.Rows[i].DataItem.GetType() == typeof(System.Data.DataRowView))
                    {
                        ...
                        {
                            bStrt = true;
                            sTmpLvCd = Util.NVC(DataTableConverter.GetValue(dgList.Rows[i].DataItem, "GR_CSTID"));
                            idxS = i;
                        ...
                        {
                            if (Util.NVC(DataTableConverter.GetValue(dgList.Rows[i].DataItem, "GR_CSTID")).Equals(sTmpLvCd))
                            {
                                idxE = i;
                                //마지막 Row 일경우
                                if (i == dgList.Rows.Count - 1)
                                {
                                    ...
                                    e.Merge(new DataGridCellsRange(dgList.GetCell(idxS, dgList.Columns["ROWNUM"].Index), dgList.GetCell(idxE, dgList.Columns["ROWNUM"].Index)));
