...
--------------------------------------------------------------------------------------
 [Change History]
  2022.05.09  최도훈 선임 : Initial Created. (활성화 4동 분리)
...
{
    public partial class MCS001_077 : UserControl, IWorkArea
	{
        ...
        private void SelectEolHistory()
        {
            try
            {
                ...
                const string bizRuleName = "DA_MCS_SEL_FORMATION_EOL_INFO_V2";
                ...
                new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inDataTable, (result, ex) =>
                {
					...
					string[] columnName = new string[] { "AREA", "LINE", "EQPTNAME" };
		...
		private void dgEolHistory_MergingCells(object sender, DataGridMergingCellsEventArgs e)
		{
			try
			{
				...
				string columnNameBase = "EQPTNAME";
                //int columnIdxBase = dg.Columns[columnNameBase].Index;

                string[] columnName = new string[] { "EIOSTAT", "MDLLOT_ID", "PRJT_NAME", "PRODID", "CSTPROD", "QUICKSHIP_YN" };
		...
		private void btnQuickShipSet_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				MCS001_077_QUICK_SHIP_SETTING popup = new MCS001_077_QUICK_SHIP_SETTING { FrameOperation = FrameOperation };
		...
		{
			MCS001_077_QUICK_SHIP_SETTING popup = sender as MCS001_077_QUICK_SHIP_SETTING;
		...
		private void btnHandOver_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				MCS001_077_HAND_OVER popup = new MCS001_077_HAND_OVER { FrameOperation = FrameOperation };
		...
		{
			//MCS001_077_HAND_OVER popup = sender as MCS001_077_HAND_OVER;
			//if (popup != null && popup.DialogResult == MessageBoxResult.OK)
			//{
			//	btnSearch_Click(null, null);
			//}
