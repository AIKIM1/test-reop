/*************************************************************************************
 Created Date : 2019.12.03
      Creator : 신광희
   Decription : Carrier 현황 조회
--------------------------------------------------------------------------------------
 [Change History]
  2019.12.03  신광희 차장 : Initial Created.    
  2020.04.13  정문교 차장 : CSR[C20200410-000002] GMES Carrier 현황 조회 화면 개선
                            1. 전극, 조립 Carrier 중 최종 활동일시 Carrier 조회
                            2. Carrier Type 조회 조건 추가   
  2020.10.01  서동현 책임 : 비정상, 폐기 기능 추가 
  2022.12.28  정문교 차장 : 보빈 상태 변경 팝업 추가
  2023.01.06  오화백 과장 : 공보빈 Hold 기능 관련 문의 관련 수정
  2023.11.27  김태우 책임 : 대표lot 사용할 경우 추가
  2025.03.11  이민형 책임 : 동 AreaCombo 첫 데이터를 초기에 표시하는 것으로 변경
**************************************************************************************/
using System;
...
namespace LGC.GMES.MES.MCS001
{
	...
	/// </summary>
	public partial class MCS001_034 : UserControl, IWorkArea
	{
        ...
        private string _chkDefectBobin = string.Empty;
		...
		private void UserControl_Loaded(object sender, RoutedEventArgs e)
		{
            ...
            if (_util.IsCommonCodeUseAttr("MHS_UNCHECK_SKID_BOBBIN_MAPPING", LoginInfo.CFG_AREA_ID, sAttr2:"N"))
            {
                TabItemSkidHistory.Visibility = Visibility.Collapsed;
            }
            else
            {
                TabItemSkidHistory.Visibility = Visibility.Visible;
            }
		...
		private void cboCarrierType_SelectedItemChanged(object sender, PropertyChangedEventArgs<object> e)
		{
			...
			{
				this.lblLotId.Text = ObjectDic.Instance.GetObjectName("Pallet ID");
			}
			else
			{
				this.lblLotId.Text = ObjectDic.Instance.GetObjectName("Lot ID");
			}
		}
			...
			try
			{
				...
				new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inTable, (bizResult, bizException) =>
				{
					...
					{ }).Select(g => new
					{
                        ...
                        // 2024.10.14. 김영국 - DB상 DATA가 LONG TYPE으로 올라오는 문제가 있어 INT32 -> INT64로 수정.
                        CarrierTotalCount = g.Sum(x => x.Field<Int64>("T_CNT")),
                        CarrierUseCount = g.Sum(x => x.Field<Int64>("U_CNT")),
                        CarrierEmptyCount = g.Sum(x => x.Field<Int64>("E_CNT")),
                        CarrierEmptyTrayCount = g.Sum(x => x.Field<Int64>("ET_CNT")),
                        CarrierDefectCount = g.Sum(x => x.Field<Int64>("D_CNT")),
                        CarrierAbnormalCount = g.Sum(x => x.Field<Int64>("ABNORM_CNT")),
                        CarrierNonUseCount = g.Sum(x => x.Field<Int64>("UN_CNT"))
                    }).FirstOrDefault();
		...
		private void SelectCarrierDetailList(bool isCarrierIdOnly, bool isLotOnly)
		{
			...
			try
			{
				...
				if (isCarrierIdOnly)
				{
                    if(cboCarrierType.SelectedValue.ToString() !="RL")
					    dr["CSTID_LIST"] = !string.IsNullOrEmpty(txtCarrierId.Text.Trim()) ? ConvertData(txtCarrierId.Text.Trim()) : null;
                    else
                    {
                        dr["CSTID_LIST"] = !string.IsNullOrEmpty(txtCarrierId2.Text.Trim()) ? ConvertData(txtCarrierId2.Text.Trim()) : null;
		...
		private  void SetCarrierTypeCombo(C1ComboBox cbo)
		{
            ...
            //2023.01.06  오화백 과장 : 공보빈 HOLD 기능 관련 수정
            try
            {
                ...
                DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMCODE_ALL_CBO", "RQSTDT", "RSLTDT", inTable);
		...
		private void SetGridDetailColumnText()
		{
			...
			dgCarrierDetail.Columns["LOTID"].Header = ObjectDic.Instance.GetObjectName("LOT ID");
            ...
            TabItemCarrierChange.Visibility = Visibility.Visible;
            ...
            switch (cboCarrierType.SelectedValue.ToString())
			{
				...
				case "PT":
					...
					break;
                case "RL":
                    dgCarrierDetail.Columns["BOBBIN_ID"].Visibility = Visibility.Collapsed;
                    dgCarrierDetail.Columns["CST_CLEAN_NAME"].Visibility = Visibility.Collapsed;
                    dgCarrierDetail.Columns["PET_WINDING_CMPL_NAME"].Visibility = Visibility.Collapsed;
                    dgCarrierDetail.Columns["CURR_LOCID"].Visibility = Visibility.Collapsed;
                    dgCarrierDetail.Columns["CURR_LOC_NAME"].Visibility = Visibility.Collapsed;
                    dgCarrierDetail.Columns["CST_DFCT_FLAG_NAME"].Visibility = Visibility.Collapsed;
                    dgCarrierDetail.Columns["CSTSTAT_NAME"].Visibility = Visibility.Collapsed;
                    
                    btnSave.Visibility = Visibility.Collapsed;
                    txtNote.IsReadOnly = true;
                    this.cboDefectReason.IsDropDownOpen = false;
                    this.cboCarrierState.IsDropDownOpen = false;
                    txtCarrier.Visibility = Visibility.Collapsed;
                    txtCarrier2.Visibility = Visibility.Visible;
                    txtCarrierId.Visibility = Visibility.Collapsed;
                    txtCarrierId2.Visibility = Visibility.Visible;

                    dgCarrierDetail.Columns["CARRIERID"].Header = ObjectDic.Instance.GetObjectName("REP_LOTID");
                    TabItemCarrierChange .Visibility = Visibility.Collapsed;
		...
		private void SaveScrap()
		{
			...
			{
				ShowLoadingIndicator();
                //const string bizRuleName = "BR_MCS_REG_CARRIER_SCRAP_LIST"; // 2024.11.19. 김영국 - 호출되는 BR명 변경. (박병성 책임 요청)
                const string bizRuleName = "BR_MHS_REG_CST_SCRAP_LIST"; 
        ...
        private void cboCarrierType_SelectedValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            ...
            btnBobinChange.IsEnabled = false;

            //if (TabItemCarrierInfo.IsSelected)
            //{
                if (cboCarrierType.SelectedValue.ToString().Equals("BB"))
                    btnBobinChange.IsEnabled = true;
            //}

            DataView dv = (DataView)cboCarrierType.ItemsSource;
            DataTable dt = dv.Table;
            DataRow dr = dt.Rows[cboCarrierType.SelectedIndex];
            //Util.MessageInfo("ATTRIBUTE2 :" + dr["ATTR2"].GetString());
            _chkDefectBobin = dr["ATTR2"].GetString();

            SetDefectReasonCombo(cboDefectReason);

        }

        private void btnBobinChange_Click(object sender, RoutedEventArgs e)
        {
            CMM_MCS_CHANGE_BOBIN_STATUS popupBobinStatus = new CMM_MCS_CHANGE_BOBIN_STATUS();
            popupBobinStatus.FrameOperation = this.FrameOperation;

            if (popupBobinStatus != null)
            {
                object[] Parameters = new object[3];
                Parameters[0] = LoginInfo.CFG_AREA_ID;
                Parameters[1] = cboAreaGroup.SelectedValue;
                Parameters[2] = cboCarrierType.SelectedValue;
                ControlsLibrary.C1WindowExtension.SetParameters(popupBobinStatus, Parameters);

                popupBobinStatus.Closed += new EventHandler(PopupBobinStatus_Closed);
                this.Dispatcher.BeginInvoke(new Action(() => popupBobinStatus.ShowModal()));
            }

        }

        private void PopupBobinStatus_Closed(object sender, EventArgs e)
        {
            CMM_MCS_CHANGE_BOBIN_STATUS popup = sender as CMM_MCS_CHANGE_BOBIN_STATUS;
            //if (popup.IsUpdated)
            //{
            //    SelectCarrierDetailList(false, false);
            //}
