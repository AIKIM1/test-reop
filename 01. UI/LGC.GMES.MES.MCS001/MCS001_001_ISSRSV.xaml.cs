...
{
    public partial class MCS001_001_ISSRSV : C1Window, IWorkArea
    {
        ...
        private void SeachData()
        {
			...
			new ClientProxy().ExecuteService("DA_MCS_SEL_ISS_RSV", "RQSTDT", "RSLTDT", RQSTDT, ( result, exception ) => {
				try {
					if( exception != null ) {
        ...
        private void btnManualIssue_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (checkcount < 1)
            {
                LGC.GMES.MES.ControlsLibrary.MessageBox.Show(MessageDic.Instance.GetMessage("SFU4543"), null, "Error", MessageBoxButton.OK, MessageBoxIcon.Warning);   //메세지 작성요
                return;
            }
                ...
                if (result == MessageBoxResult.OK)
                {
                    ...
                    foreach (DataRow row in ((System.Data.DataView)dgList.ItemsSource).Table.Rows)
                    {
                        if (row["CHK"].ToString() == "1")
                        {
                            ...
                            {
                                DataTable OUTDATA = new ClientProxy().ExecuteServiceSync("DA_MCS_UPD_ISS_RSV_CANCEL", "RQSTDT", "OUTDATA", RQSTDT);
                            }
                            else
                            {
                                DataTable OUTDATA = new ClientProxy().ExecuteServiceSync("DA_MCS_UPD_ISS_RSV_EMPTY_CANCEL", "RQSTDT", "OUTDATA", RQSTDT);
                            }
                        }
                    }
