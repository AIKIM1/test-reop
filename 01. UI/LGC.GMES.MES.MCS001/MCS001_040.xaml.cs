...
--------------------------------------------------------------------------------------
 [Change History]
  2020.01.22  신광희 차장 : Initial Created.    
  2021.11.25  조영대      : 불량 Carrier 컬럼 및 탭 추가
**************************************************************************************/
using System;
...
{
    /// <summary>
    /// MCS001_040.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_040 : UserControl, IWorkArea
    {
        ...
        private void dgCapacitySummary_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                else if (cell.Column.Name.Equals("DFCT_QTY"))
                {
                    if (string.Equals(DataTableConverter.GetValue(drv, "EQPTID").GetString(), "XXXXXXXXXX") ||
                        string.Equals(DataTableConverter.GetValue(drv, "EQPTID").GetString(), "ZZZZZZZZZZ"))
                    {
                    ...
                    for (int j = 0; j < dg.Rows.Count; j++)
                    {
                        ...
                        foreach (var item in query1)
                        {
                                ...
                                )
                            {
                                ...
                                e.Merge(new DataGridCellsRange(dg.GetCell(j, dg.Columns["DFCT_QTY"].Index), dg.GetCell(item.Count - 1 + rowIndex, dg.Columns["DFCT_QTY"].Index)));                                
        ...
        {
            const string bizRuleName = "DA_MCS_SEL_WAREHOUSE_SUMMARY";
            try
            {
                ...
                new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inTable, (bizResult, bizException) =>
                {
                    ...
                    // 극성 별 테스트를 위하여 임시로 데이터를 변경 함
                    //for (int x = 0; x < bizResult.Rows.Count; x++)
                    //{
                    //    if (bizResult.Rows[x]["EQPTID"].GetString() == "A7STK101" &&
                    //        bizResult.Rows[x]["PRJT_NAME"].GetString() == "P33" &&
                    //        bizResult.Rows[x]["ELTR_TYPE_CODE"].GetString() == "C" &&
                    //        bizResult.Rows[x]["HOLD_QTY"].GetString() == "1" 
                    //        )
                    //    {
                    //        bizResult.Rows[x]["ELTR_TYPE_CODE"] = "A";
                    //        bizResult.Rows[x]["ELTR_TYPE_NAME"] = "음극";
                    //    }
                    //}
                    ...
                    var queryBase = bizResult.AsEnumerable()
                        .Select(row => new {
                            ...
                            //DfctQty = row.Field<Int32>("DFCT_QTY"),
                            //ErrorQty = row.Field<Int32>("ERROR_QTY"),
                            //RackRate = row.Field<decimal>("RACK_RATE"),
                            //SumCarrierCount = row.Field<Int32>("RACK_QTY"),
                            RackMaxQty = row.Field<long>("RACK_MAX_QTY"),
                            EmptyQty = row.Field<long>("EMPTY_QTY"),
                            DfctQty = row.Field<long>("DFCT_QTY"),
                            ErrorQty = row.Field<long>("ERROR_QTY"),
                            RackRate = row.Field<long>("RACK_RATE"),
                            SumCarrierCount = row.Field<long>("RACK_QTY"),
                        }).Distinct();
                    ...
                    }).Select(g => new
                    {
                        ...
                        DfctCount = queryBase.AsQueryable().Where(x => x.ElectrodeTypeCode == g.Key.electrodeTypeCode).Select(s => s.DfctQty).Sum(),
                        ...
                        LotCount = g.Sum(x => x.Field<long>("LOT_QTY")),
                        HoldCount = g.Sum(x => x.Field<long>("HOLD_QTY")),
                        ProjectName = "",
                    ...
                    {}).Select(g => new
                    {
                        ...
                        DfctCount = queryBase.AsQueryable().Select(s => s.DfctQty).Sum(),
                    ...
                    if (CommonVerify.HasTableRow(bizResult))
                    {
                        for (int i = 0; i < bizResult.Rows.Count; i++)
                        {
                            ...
                            newRow["DFCT_QTY"] = bizResult.Rows[i]["DFCT_QTY"];
                        ...
                        if (query.Any())
                        {
                            foreach (var item in query)
                            {
                                ...
                                newRow["DFCT_QTY"] = item.DfctCount;
                    ...
                    // 2021.11.25 조영대 : 불량 Carrier 컬럼 및 탭 추가
                    //                     조회한 데이터에서 불량 Carrier 컬럼이 있으면 보인다.
                    if (bizResult.Columns.Contains("DFCT_QTY"))
                    {
                        dgCapacitySummary.Columns["DFCT_QTY"].Visibility = Visibility.Visible;
                        tabTrialRunCarrier.Visibility = Visibility.Visible;
                    }
                    else
                    {
                        dgCapacitySummary.Columns["DFCT_QTY"].Visibility = Visibility.Collapsed;
                        tabTrialRunCarrier.Visibility = Visibility.Collapsed;
                    }
        ...
        {
            const string bizRuleName = "DA_MCS_SEL_WAREHOUSE_SUMMARY_LWW";
            try
            {
                ShowLoadingIndicator();
                DataTable inTable = new DataTable("RQSTDT");
                ...
                new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inTable, (bizResult, bizException) =>
                {
                    ...
                    if (bizResult.Columns.Contains("DFCT_QTY"))
                    {
                        dgLamiCapacitySummary.Columns["DFCT_QTY"].Visibility = Visibility.Visible;
                        tabTrialRunCarrier.Visibility = Visibility.Visible;
                    }
                    else
                    {
                        dgLamiCapacitySummary.Columns["DFCT_QTY"].Visibility = Visibility.Collapsed;
                        tabTrialRunCarrier.Visibility = Visibility.Collapsed;
                    }
        ...
        {
            // 2021.11.25 조영대 : 비즈명 변경
            const string bizRuleName = "DA_MCS_SEL_WAREHOUSE_DEFECT_CARRIER_LIST";
        ...
        private void MakeWareHouseCapacityTable()
        {
            ...
            _dtWareHouseCapacity.Columns.Add("DFCT_QTY", typeof(decimal));      // 불량Carrier수
