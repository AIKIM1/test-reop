...
{
    /// <summary>
    /// MCS001_088.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_088 : UserControl, IWorkArea
    {
        ...
        private string _LogicElecArea = string.Empty;
        ...
        private void dgElecWareHouse_LoadedCellPresenter(object sender, DataGridCellEventArgs e)
        {
            ...
            dataGrid?.Dispatcher.BeginInvoke(new Action(() =>
            {
                ...
                if (e.Cell.Row.Type == DataGridRowType.Item)
                {
                    ...
                    {
                        if (DataTableConverter.GetValue(e.Cell.Row.DataItem, e.Cell.Column.Name).GetDecimal() >= (ChkOptmLoadRate(Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, "EQPTID"))) - 10) && (DataTableConverter.GetValue(e.Cell.Row.DataItem, e.Cell.Column.Name).GetDecimal() < (ChkOptmLoadRate(Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, "EQPTID")))-5)))
                        {
                        ...
                        else if (DataTableConverter.GetValue(e.Cell.Row.DataItem, e.Cell.Column.Name).GetDecimal() >= (ChkOptmLoadRate(Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, "EQPTID"))) - 5))
                        {
                    ...
                    if (string.Equals(e.Cell.Column.Name, "U_CNT_ALL_TEXT")) // 실수량 클릭시
                    {
                    ...
                    for (int i = 0; i < dg.Rows.Count; i++)
                    {
                        foreach (var item in query)
                        {
                            ...
                            if (DataTableConverter.GetValue(dg.Rows[i].DataItem, "EQPTID").GetString() == item.EquipmentCode && previewEquipmentCode != DataTableConverter.GetValue(dg.Rows[i].DataItem, "EQPTID").GetString())
                            {
                                ...
                                e.Merge(new DataGridCellsRange(dg.GetCell(i, dg.Columns["PROHIBIT_CNT_TEXT"].Index), dg.GetCell(item.Count - 1 + rowIndex, dg.Columns["PROHIBIT_CNT_TEXT"].Index)));
        ...
        private void dgElecWareHouse_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                if (cell == null || !cell.Column.Name.Equals("U_CNT_ALL_TEXT")) return;
                ...
                // 팝업 호출
                MCS001_088_LOTLIST popupLotlist = new MCS001_088_LOTLIST { FrameOperation = FrameOperation };
                object[] parameters = new object[6];
                parameters[0] = _LogicElecArea;     
                parameters[1] = "PCW";
        ...
        #region STK 완성창고 상세 LOT 팝업 호출 : dgTrauStocker_MouseLeftButtonUp()
        /// <summary>
        /// Tray 창고  팝업
        /// </summary>
        /// <param name="sender"></param>
        ...
        {
            MCS001_088_LOTLIST popup = sender as MCS001_088_LOTLIST;
            if (popup != null)
            {
        ...
        private void SelectCapacityAll()
        {
            ...
            const string bizRuleName = "BR_MHS_SEL_WAREHOUSE_CAPACITY_SUMMARY_ALL";
            try
            {
                ...
                new ClientProxy().ExecuteService_Multi(bizRuleName, "INDATA", "OUTDATA_ELEC,OUTDATA_NNDW,OUTDATA_NNDO,OUTDATA_LAMW,OUTDATA_TRAYW", (bizResult, bizException) =>
                {
                    try
                    {
                        ...
                        //if (bizResult != null && bizResult.Tables.Count > 0 && bizResult.Tables["OUTDATA_NNDW"] != null)
                        //{
                        //    SelecNNDWaitWareHouse_ESWA(bizResult.Tables["OUTDATA_NNDW"]);
                        //}
                        //NND 완성창고
                        ...
                        //if (bizResult != null && bizResult.Tables.Count > 0 && bizResult.Tables["OUTDATA_NNDO"] != null)
                        //{
                        //    SelectNNDWareHouse_ESWA(bizResult.Tables["OUTDATA_NNDO"]);
                        //}
        ...
        private void SelectElecWarehouse(DataTable dt)
        {
            ...
            dt.Columns.Add("U_CNT_ALL_TEXT", typeof(string)); // 금지단 포함 실Tray수
            dt.Columns.Add("E_CNT_ALL_TEXT", typeof(string)); // 금지단 포함 공Tray수
            ...
            if (CommonVerify.HasTableRow(dt))
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    dt.Rows[i]["U_CNT_ALL_TEXT"] = dt.Rows[i]["CARRIER_U_ALL_QTY"].GetString(); // 금지단 포함 실Tray수
                    dt.Rows[i]["E_CNT_ALL_TEXT"] = dt.Rows[i]["CARRIER_E_ALL_QTY"].GetString(); // 금지단 포함 공Tray수
                }
        ...
        #region NND 대기창고 적용(ESWA) : SelecNNDWaitWareHouse_ESWA()
        /// <summary>
        /// NND 대기창고 조회
        /// </summary>
        //private void SelecNNDWaitWareHouse_ESWA(DataTable dt)
        //{
        //    dt.Columns.Add("RACK_CNT_TEXT", typeof(string)); //용량
        //    dt.Columns.Add("U_CNT_TEXT", typeof(string)); //실Tray수
        //    dt.Columns.Add("E_CNT_TEXT", typeof(string)); //공Tray수
        ...
        //        Util.GridSetData(dgNNDWaitWareHouse_ESWA, dtSortTable, null, true);
        //    }
        //}
        ...
        #region NND 완성창고 적용 : SelectNNDWareHouse_ESWA()

        /// <summary>
        /// NND 완성창고
        /// </summary>
        //private void SelectNNDWareHouse_ESWA(DataTable dt)
        //{
        //    dt.Columns.Add("RACK_CNT_TEXT", typeof(string)); //용량
        //    dt.Columns.Add("U_CNT_TEXT", typeof(string)); //실Tray수
        //    dt.Columns.Add("E_CNT_TEXT", typeof(string)); //공Tray수
        ...
        //        Util.GridSetData(dgNNDWareHouse_ESWA, dt, null, true);
        //    }
        ...
        private void SelectLamiWaitWareHouse(DataTable dt)
        {
            ...
            dt.Columns.Add("PROHIBIT_CNT_TEXT", typeof(string)); //금지단수
            dt.Columns.Add("RATE_TEXT", typeof(string)); //적재율
            ...
            if (CommonVerify.HasTableRow(dt))
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    dt.Rows[i]["PROHIBIT_CNT_TEXT"] = dt.Rows[i]["PROHIBIT_QTY"].GetString(); //금지단 수량
        ...
        private void ClearControl()
        {
            ...
            Util.gridClear(dgNNDWaitWareHouse_ESWA);
            Util.gridClear(dgNNDWareHouse_ESWA);
        ...
        #region 해당 동에서 물류적으로 연결된 전극 동  : Chk_Logic_Elec()
        private void Chk_Logic_Elec()
        {
            ...
            DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_AREA_LOGIS_GROUP_ELEC", "RQSTDT", "RSLTDT", dt);

            if (dtResult.Rows.Count != 0)
            {
                _LogicElecArea = dtResult.Rows[0]["AREAID"].ToString();
            }
         
        }
        #endregion

        #region 창고별 적정 적재율   : ChkOptmLoadRate()
        /// <summary>
        /// 적정 적재율 
        /// </summary>
        private int ChkOptmLoadRate(string WH_ID)
        {
            int optRate = 0;
            DataTable dt = new DataTable();
            ...
            DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_TB_MMD_PRDT_WH", "RQSTDT", "RSLTDT", dt);

            if (dtResult.Rows.Count > 0)
            {
                optRate = dtResult.Rows[0]["OPTM_LOAD_RATE"] == DBNull.Value ? 0 : Convert.ToInt16(dtResult.Rows[0]["OPTM_LOAD_RATE"]);
            }
            else
            {
                optRate = 0;
