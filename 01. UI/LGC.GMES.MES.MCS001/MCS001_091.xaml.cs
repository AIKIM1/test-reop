/*************************************************************************************
 Created Date : 2023.12.20
      Creator : 이원열
   Decription : NTC/VD 대기 창고 모니터링
--------------------------------------------------------------------------------------
 [Change History]
    22023.12.20 이원열 : NJ N2 ESS NTC/VD 대기 창고 모니터링  Create
...
{
    /// <summary>
    /// MCS001_091_ESS.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_091 : UserControl, IWorkArea
    {
        ...
        private void btnManualIssue_NTC_Click(object sender, RoutedEventArgs e)
        {
            ManualExec();
        }
        ...
        {
            //ESNA - 전극 > 물류관리 > 창고 수동출고 예약 (Left Grid) - format 적용
            try
            {
                ShowLoadingIndicator();
                ...
                new ClientProxy().ExecuteService("DA_WCS_SEL_MANUAL_OUT_INVENTORY_PANCAKE_NA", "RQSTDT", "RSLTDT", inTable, (bizResult, bizException) =>
                {
                    ...
                    Util.GridSetData(dgProjectStock_ESS, bizResult, null, true);
                });
            }
            catch (Exception ex)
            {
        ...
        {
            // NTC SKID / VD SKID Check
            DataTable dtChk = Skid_Chk();
            DataTable dtRack_SKID = dtChk.DefaultView.ToTable(true, new string[] { "ATTR1" });

            if (dtRack_SKID.Rows.Count > 1)
            {
                Util.MessageValidation("SFU9401");  // NTC 출고 SKID, VD 출고 SIKD 를 같이 선택할 수 없습니다.
                return;
            }

            // 실SKID, 공SKID Check
            DataTable dtStatChk = GetCstStat();
            DataTable dtStat_SKID = dtStatChk.DefaultView.ToTable(true, new string[] { "CSTSTAT" });

            if (dtStat_SKID.Rows.Count > 1)
            {
                Util.MessageValidation("SFU9402");  // 실SKID, 공SKID 를 같이 선택할 수 없습니다.
                return;
            }

            MCS001_091_MANUAL_RESERVATION popupManualReservation = new MCS001_091_MANUAL_RESERVATION { FrameOperation = FrameOperation };

            object[] parameters = new object[3];
            parameters[0] = cboStocker.SelectedValue;
            parameters[1] = ((DataView)dgRackInfo.ItemsSource).Table;
            parameters[2] = dtRack_SKID.Rows[0]["ATTR1"].ToString();
        ...
        private DataTable GetCstStat()
        {
            try
            {
                DataTable dtRack = ((DataView)dgRackInfo.ItemsSource).Table;
                DataTable dtRack_SKID = dtRack.DefaultView.ToTable(true, new string[] { "MCS_CST_ID" });

                DataTable inTable = new DataTable();

                inTable.Columns.Add("CST_ID", typeof(string));

                foreach (DataRow dr in dtRack_SKID.Rows)
                {
                    DataRow row = inTable.NewRow();
                    row["CST_ID"] = dr["MCS_CST_ID"].ToString();

                    inTable.Rows.Add(row);
                }

                return new ClientProxy().ExecuteServiceSync("DA_MCS_SEL_CST_STAT_CHK", "RQSTDT", "RSLTDT", inTable);
        ...
        private DataTable Skid_Chk()
        {
            try
            {
                string sComTypeCode = "SKID_TYPE_CODE";
                ...
                foreach (DataRow dr in dtRack_SKID.Rows)
                {
                    ...
                    row["COM_CODE"] = dr["MCS_CST_ID"].ToString();
                    row["USE_FLAG"] = "Y";

                    inTable.Rows.Add(row);
                }

                return new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_TB_MMD_AREA_COM_CODE_ATTR", "INDATA", "OUTDATA", inTable);
        ...
        {
            MCS001_091_MANUAL_RESERVATION popup = sender as MCS001_091_MANUAL_RESERVATION;
