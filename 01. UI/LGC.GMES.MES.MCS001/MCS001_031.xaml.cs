...
{
    /// <summary>
    /// MCS001_031.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_031 : UserControl, IWorkArea
    {
        ...
        private string _selectedParentsEquipmentCode;
        ...
        private void btnSearch_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ClearControl();
                SelectAutomaticGuidedVehicleTransferSummary();
        ...
        private void dgAgvReturn_LoadedCellPresenter(object sender, DataGridCellEventArgs e)
        {
            if (sender == null)
                return;
            ...
            dataGrid?.Dispatcher.BeginInvoke(new Action(() =>
            {
                ...
                {
                    if (string.Equals(e.Cell.Column.Name, "EQPTNAME") || string.Equals(e.Cell.Column.Name, "AGV_U_QTY"))
                    {
        ...
        private void dgAgvReturn_UnloadedCellPresenter(object sender, DataGridCellEventArgs e)
        {
        ...
        private void dgAgvReturn_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                if (cell.Column.Name.Equals("EQPTNAME") || cell.Column.Name.Equals("AGV_U_QTY"))
                {
                    int rowIdx = cell.Row.Index;
                    DataRowView drv = dg.Rows[rowIdx].DataItem as DataRowView;
                    if (drv == null) return;

                    _selectedParentsEquipmentCode = DataTableConverter.GetValue(drv, "EQPTID").GetString();

                    if (string.Equals(DataTableConverter.GetValue(drv, "EQPTID").GetString(), "Total") || string.IsNullOrEmpty(DataTableConverter.GetValue(drv, "EQPTID").GetString()) )
                    {
                        _selectedParentsEquipmentCode = null;
                    }

                    if (cell.Column.Name.Equals("EQPTNAME"))
                    {
                        txtAGVType.Text = DataTableConverter.GetValue(drv, "EQPTNAME").GetString();
                        txtAGVCount.Text = DataTableConverter.GetValue(drv, "AGV_U_QTY").GetString();
        ...
        private void  SelectAutomaticGuidedVehicleTransferSummary()
        {
            const string bizRuleName = "DA_MCS_SEL_AGV_TRANSFER_SUMMARY";
            try
            {
                ...
                new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inTable, (bizResult, bizException) =>
                {
                    ...
                    var querySum = bizResult.AsEnumerable().GroupBy(x => new{ }).Select(g => new
                    {
                        ...
                        AgvQtyCount = g.Sum(x => x.Field<Int32>("AGV_QTY")),
                        RealCarrierCount = g.Sum(x => x.Field<Int32>("AGV_U_QTY")),
                        EmptyCarrierCount = g.Sum(x => x.Field<Int32>("AGV_E_QTY")),
                        AgvRate = GetAgvRate(g.Sum(x => x.Field<Int32>("AGV_U_QTY")), g.Sum(x => x.Field<Int32>("AGV_QTY"))),
                        Count = g.Count()
                    }).ToList();
                    ...
                    if (CommonVerify.HasTableRow(bizResult))
                    {
                        if (querySum.Any())
                        {
                            foreach (var item in querySum)
                            {
                                ...
                                newRow["AGV_QTY"] = item.AgvQtyCount;
                                newRow["AGV_U_QTY"] = item.RealCarrierCount;
                                newRow["AGV_E_QTY"] = item.EmptyCarrierCount;
                                newRow["TRSF_RATE"] = item.AgvRate;
                                bizResult.Rows.Add(newRow);
                            }
                        }
                    }

                    Util.GridSetData(dgAgvReturn, bizResult, null, true);
                    HiddenLoadingIndicator();
                });
            }
            catch (Exception ex)
        ...
        private void SelectEquipmentProductSummary()
        {
            ...
            try
            {
                ...
                dr["AREAID"] = cboArea.SelectedValue;
                dr["EQGRID"] = "AGV";
                dr["EQPTID"] = _selectedParentsEquipmentCode;
                inTable.Rows.Add(dr);
        ...
        {
            _selectedParentsEquipmentCode = string.Empty;
            _selectedElectrodeTypeCode = string.Empty;
            _selectedProjectName = string.Empty;
            _selectedWipHold = string.Empty;
            txtAGVType.Text = string.Empty;
            txtAGVCount.Text = string.Empty;

            Util.gridClear(dgAgvReturn);
        ...
        private double GetAgvRate(int x, int y)
        {
            double ageyRate = 0;
            if (y.Equals(0)) return ageyRate;
            try
            {
