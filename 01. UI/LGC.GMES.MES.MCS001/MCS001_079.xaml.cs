/*************************************************************************************
 Created Date : 2022.06.10
      Creator : 최도훈
   Decription : 활성화 창고 적재 현황
--------------------------------------------------------------------------------------
 [Change History]
  2022.06.10  최도훈 선임 : Initial Created.    
...
{
    /// <summary>
    /// MCS001_079.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class MCS001_079 : UserControl, IWorkArea
    {
        ...
        private void SelectWareHouseProductList(C1DataGrid dg)
        {
			...
			const string bizRuleName = "BR_MCS_SEL_STO_FORMATION_PRODUCT_LIST";
     ...
     //               dr["QMSHOLD"] = _selectedQmsHold;
        ...
        private void SelectWareHouseEmptyPalletList(C1DataGrid dg)
        {
			...
			const string bizRuleName = "BR_MCS_SEL_STO_FORMATION_EMPTY_CARRIER_LIST";
        ...
        {
            const string bizRuleName = "BR_MCS_SEL_STO_FORMATION_NOREAD_LIST";
            try
            {
                ShowLoadingIndicator();
                DataTable inTable = new DataTable("RQSTDT");
        ...
        private void SelectAbNormalPalletList(C1DataGrid dg)
        {
			...
			const string bizRuleName = "BR_MCS_SEL_STO_FORMATION_ABNORM_LIST";
        ...
        {
            const string bizRuleName = "DA_MCS_SEL_STO_RACK_RATE";
            ...
            try
            {
                ...
                _requestMcsRackRate = new ClientProxy().ExecuteServiceSync(bizRuleName, "RQSTDT", "RSLTDT", inDataTable);
            }
            catch (Exception ex)
            {
                HiddenLoadingIndicator();
        ...
        {
            const string bizRuleName = "DA_MCS_SEL_FORMATION_STO_SUMMARY";
            try
            {
                ShowLoadingIndicator();
                DataTable inTable = new DataTable("RQSTDT");
                ...
                new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inTable, (bizResult, bizException) =>
                {
                    ...
                    var queryBase = bizResult.AsEnumerable()
                        .Select(row => new {
                            ...
                            RackMaxQty = row.Field<double>("RACK_MAX_QTY"),
                            EmptyQty = row.Field<Int64>("EMPTY_QTY"),
                            ErrorQty = row.Field<Int64>("ERROR_QTY"),
                            RackRate = row.Field<Int64>("RACK_RATE"),
                            SumCarrierCount = row.Field<Int64>("RACK_QTY"),
                            AbNormalQty = row.Field<Int64>("ABNORM_QTY")
                        }).Distinct();
                    ...
                    var querySum = bizResult.AsEnumerable().GroupBy(x => new { }).Select(g => new
                    {
                        ...
                        ProjectName = "",
                        ShipPltQty = g.Sum(x => x.Field<Int64>("SHIP_PLT_QTY")),
                        HoldPltQty = g.Sum(x => x.Field<Int64>("HOLD_PLT_QTY")),
                        InspWaitPltQty = g.Sum(x => x.Field<Int64>("INSP_WAIT_PLT_QTY")),
                        NGPltQty = g.Sum(x => x.Field<Int64>("NG_PLT_QTY")),
						VldDatePltQty = g.Sum(x => x.Field<Int64>("VLD_DATE_PLT_QTY")),
        ...
        private void CheckUcRackLayout(UcRackLayout rackLayout)
        {
            ...
            // (2022.06.20) 공Pallet, 공TrayPallet면 전체 검색되도록 되어있는 것 삭제
            //if (rackLayout.LegendColorType == "4")
            //{
            //    _selectedSkIdIdByRackInfo = null;
            //}
