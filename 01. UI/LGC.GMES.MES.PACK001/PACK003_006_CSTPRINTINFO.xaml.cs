/*************************************************************************************
 Created Date : 2021-01-21
      Creator : 
   Decription : Pack 물류 CARRIER 정보 프린트 발행
--------------------------------------------------------------------------------------
 [Change History]
  2021-01-21  김길용 : Initial Created.
  2022-01-28  김길용 : 다국어적용
**************************************************************************************/
...
namespace LGC.GMES.MES.PACK001
{
    ...
    /// </summary>
    public partial class PACK003_006_CSTPRINTINFO : Window, IWorkArea
    {
        ...
        public PACK003_006_CSTPRINTINFO()
        {
            InitializeComponent();
        ...
        {
            reports_name = new string[] { "LOGISCST" };

            tmps = C1WindowExtension.GetParameters(this);
            window_name = tmps[0] as string;
            bCID = tmps[1] as string; //cstID
            bPID = tmps[2] as string; // pltID
        ...
        private void setReport(string gubun = "LOGISCST")
        {
            ...
            {
                case "LOGISCST":
                    param_c1DocumentViewer = c1DocumentViewer_LOGISCST;
                    if (Convert.ToInt32(dtPallet_history.Rows.Count) > 380)
                    {
                            ...
                            switch (gubun)
                            {
                                case "LOGISCST":
                                    if (cr.Fields[fields_iex].Name == "TITLE" || cr.Fields[fields_iex].Name == null || cr.Fields[fields_iex].Name == "21" || cr.Fields[fields_iex].Name == "22" ||
                                        cr.Fields[fields_iex].Name == "23" || cr.Fields[fields_iex].Name == "24" || cr.Fields[fields_iex].Name == "27" || cr.Fields[fields_iex].Name == "28")
                                    {
        ...
        private void setDataTable(string reportname = null)
        {
            try
            {
                ...
                DataTable dtPltData = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_CARRIER_PRINT_INFO", "RQSTDT", "RSLTDT", RQSTDT);
                
                reportname = string.IsNullOrWhiteSpace(reportname) ? "LOGISCST" : reportname;
        ...
        private void SetReportData(string reportname = "LOGISCST")
        {
            #region Reportname 별 Binding Data Setting
            if (dtPallet_history == null || dtPallet_history.Rows.Count == 0)
            {
            ...
            else
            {
                ...
                {
                    case "LOGISCST":
                        dtBindData = setBindData_LOGISCST();
                        break;
        ...
        #region setBinData
        #region setBindData_LOGISCST
        private DataTable setBindData_LOGISCST()
        {
            try
            {
                dtBindData = null;
                ...
                dtBindData.Columns.Add("FROMSLOC", typeof(string));
                dtBindData.Columns.Add("TOSLOC", typeof(string));
                ...
                drBindData["BARCODE"] = dtPallet_history.Rows[0]["PLTID"].ToString();
                drBindData["CSTID"] = dtPallet_history.Rows[0]["CSTID"].ToString();
                drBindData["CSTID1"] = dtPallet_history.Rows[0]["CSTID"].ToString();
                drBindData["BOXID"] = dtPallet_history.Rows[0]["PLTID"].ToString();
                drBindData["BOXID1"] = dtPallet_history.Rows[0]["PLTID"].ToString();
                ...
                drBindData["FROMSLOC"] = dtPallet_history.Rows[0]["FROMSLOC"].ToString();
                drBindData["TOSLOC"] = dtPallet_history.Rows[0]["TOSLOC"].ToString();
        ...
        private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                case "LOGISCST":
                    seleted_c1DocumentViewer = c1DocumentViewer_LOGISCST;
                    break;
        ...
        private void tcMain_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            if (dtPltData != null)
            {
                ...
                reportname = dtPltData.Tables.Contains(reportname.Trim().ToUpper()) ? reportname.Trim().ToUpper() : "LOGISCST";
