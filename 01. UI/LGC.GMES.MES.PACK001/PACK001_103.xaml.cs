...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2025.04.22 유현민 : MI1_OSS_0117 LOT 홀드 탭 선택취소 버튼 오류 수정건
**************************************************************************************/
using C1.WPF;
...
{
    public partial class PACK001_103 : UserControl, IWorkArea
    {
        #region Member Variable Lists...
        private PACK001_103_DataHelper dataHelper = new PACK001_103_DataHelper();
        ...
        private void Initialize()
        {
            ...
            dtpDate.SelectedDateTime = DateTime.Now.AddDays(Double.Parse(TEMP_Expect_date));
        ...
        private void AddHoldLOTGrid(DataTable dt)
        {
            ...
            {
                DataTable dtTo = DataTableConverter.Convert(dgHoldLOT.ItemsSource);

                dtTo.ImportRow(drv);
                Util.gridClear(dgHoldLOT);
                dgHoldLOT.ItemsSource = DataTableConverter.Convert(dtTo);
        ...
        private void HoldProcess()
        {
            try
            {
                ...
                if (dtHoldGroup2.AsEnumerable().Where(x => x.Field<bool>("CHK").Equals(true) || x.Field<bool>("CHK").ToString() == "1").Count() <= 0)
                {
                ...
                DataTable dt = DataTableConverter.Convert(this.dgSelectHold.ItemsSource);
                string holdNote = textRange.Text;
                // MES 2.0 CHK 컬럼 Bool 오류 Patch
                string resnCode = dtHoldGroup2.AsEnumerable().Where(x => x.Field<bool>("CHK").Equals(true) || x.Field<bool>("CHK").ToString() == "1").Select(x => x.Field<string>("RESNCODE")).FirstOrDefault();
                ...
                if (this.dataHelper.HoldTransaction(actionUserID, calDate, dt, holdNote, resnCode, holdCode, dteUnHoldScheduleDate, unusualHoldStockFlag))
                {
                    ...
                    Util.gridClear(this.dgSelectHold);
        ...
        private void btnHoldSelectCancel_Click(object sender, RoutedEventArgs e)
        {
            ...
            // MES 2.0 CHK 컬럼 Bool 오류 Patch
            if (dt.AsEnumerable().Where(x => x.Field<Int64>("CHK").Equals(true) || x.Field<Int64>("CHK").ToString() == "1").Count() <= 0)
            {
            ...
            // MES 2.0 오류 Patch
            int noSelectCount = dt.AsEnumerable().Where(x => !x.Field<Int64>("CHK").Equals(true) && x.Field<Int64>("CHK").ToString() != "1").Count();
            if (noSelectCount > 0)
            {
                DataTable dtSelectCancel = dt.AsEnumerable().Where(x => !x.Field<Int64>("CHK").Equals(true) && x.Field<Int64>("CHK").ToString() != "1").CopyToDataTable();
        ...
        private void RadioButton_Checked(object sender, RoutedEventArgs e)
        {
            ...
            {
                cboHoldType.SelectedValue = Util.NVC(DataTableConverter.GetValue(dataGrid.Rows[selectedIndex].DataItem, "RESNCODE"));
            }
        }
        ...
        #region [Check ALL]
        private void dgHoldLOT_LoadedColumnHeaderPresenter(object sender, C1.WPF.DataGrid.DataGridColumnEventArgs e)
        {
        ...
        {
            if (dgHoldLOT.ItemsSource == null) return;

            DataTable dt = ((DataView)dgHoldLOT.ItemsSource).Table;
        ...
        {
            dgHoldLOT.Selection.Clear();

            CheckBox cb = sender as CheckBox;

            // MES 2.0 오류 Patch
            if (DataTableConverter.GetValue(cb.DataContext, "CHK").Equals("True") ||
                DataTableConverter.GetValue(cb.DataContext, "CHK").Equals(true) ||
                DataTableConverter.GetValue(cb.DataContext, "CHK").ToString().Equals("1")
               )//체크되는 경우
            {
                ...
                DataRow[] drUnchk = DataTableConverter.Convert(dgHoldLOT.ItemsSource).Select("CHK = False");
    ...
    internal class PACK001_103_DataHelper
    {
        #region Member Variable Lists...
        #endregion

        #region Constructor
        internal PACK001_103_DataHelper()
        {
