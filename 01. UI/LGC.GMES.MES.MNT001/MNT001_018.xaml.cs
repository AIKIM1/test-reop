/*************************************************************************************
 Created Date : 2020.12.22
      Creator : 서동현
   Decription : 공Pallet 현황
--------------------------------------------------------------------------------------
 [Change History]
  2020.12.22  DEVELOPER : Initial Created.
**************************************************************************************/
...
{
    public partial class MNT001_018 : Window
    {
		...
		private static string selectedBldg;
		private static string selectedLevel;
		private static string selectedTrayType;
        ...
        private static int selectedViewRowCnt = 7;
		private static int selectedRemarkRowCnt = 20;
		...
		DataTable dtSearchRouteTable;
		DataTable dtSearchLimitCountTable;
		...
		private string[] lstRemark; //로테이션할 Remark 갯수
		...
		private void InitColumnWidth()
        {
			...
			//fullSize.Width = new GridLength(800);
        ...
        private void setTitle()
        {
            this.Dispatcher.BeginInvoke(new Action(() =>
            {
                ...
                {
					//tbTitle.Text = ObjectDic.Instance.GetObjectName("공Pallet 현황") + " ( " + (shop_open_time >= DateTime.Now ? (DateTime.Now.AddDays(-1).Month + "/" + DateTime.Now.AddDays(-1).Day) : (DateTime.Now.Month + "/" + DateTime.Now.Day) )+ " ) " ;
					tbTitle.Text = ObjectDic.Instance.GetObjectName("공Pallet 현황");
				}
                else
                {
        ...
        private void Initialize()
        {
            try
            {
                ...
                DataSet ds = LGC.GMES.MES.MNT001.MNT_Common.GetConfigXML_Formation_Stocker();
				...
				selectedBldg = Convert.ToString(ds.Tables["MNT_CONFIG"].Rows[0]["BLDG_CODE"]);
				selectedLevel = Convert.ToString(ds.Tables["MNT_CONFIG"].Rows[0]["LEVEL"]);
				selectedTrayType = Convert.ToString(ds.Tables["MNT_CONFIG"].Rows[0]["TRAY_TYPE"]);

				selectedViewRowCnt = Convert.ToInt32(ds.Tables["MNT_CONFIG"].Rows[0]["DISPLAYVIEWROWCNT"]);
				selectedRemarkRowCnt = Convert.ToInt32(ds.Tables["MNT_CONFIG"].Rows[0]["DISPLAYREMARKROWCNT"]);
				...
				cboBldg.SelectedValue = selectedBldg;
				cboLevel.SelectedValue = selectedLevel;

				string[] splitTrayType = selectedTrayType.Split(',');
                foreach (string str in splitTrayType)
                {
					cboTrayType.Check(str);
                }

                numViewRowCnt.Value = selectedViewRowCnt;
				numRemarkViewRowCnt.Value = selectedRemarkRowCnt;
        ...
        private void InitCombo()
        {
			...
			//동
			SetBldg(cboBldg, "BLDG_CODE", LoginInfo.CFG_AREA_ID);

			//레벨
			SetLevelCombo(cboLevel);

			//Tray Type
			SetTrayType(cboTrayType, "CSTPROD", "PT");
		...
		private void SetLevelCombo(C1ComboBox cbo)
		{
			try
			{
				...
				DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_FORMATION_LEVEL_CBO", "RQSTDT", "RSLTDT", RQSTDT);
        ...
        private void SetTrayType(MultiSelectionBox cbo, string cmcdType, string attr1)
        {
            try
            {
				...
				drnewrow["CMCDTYPE"] = cmcdType;
				drnewrow["ATTRIBUTE1"] = attr1;
				dtRQSTDT.Rows.Add(drnewrow);

                DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMONCODE_BY_ATTR1", "RQSTDT", "RSLTDT", dtRQSTDT);
		...
		private void SetBldg(C1ComboBox cbo, string cmcdType, string attr2)
		{
			try
			{
				DataTable dtRQSTDT = new DataTable();
				...
				drnewrow["LANGID"] = LoginInfo.LANGID;
				drnewrow["CMCDTYPE"] = cmcdType;
				drnewrow["ATTRIBUTE2"] = attr2;
				dtRQSTDT.Rows.Add(drnewrow);

				DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMONCODE_ATTRIBUTES", "RQSTDT", "RSLTDT", dtRQSTDT);
        ...
        private void searchData()
        {
            try
            {
				// Route데이터부터 먼저조회한다.
				this.GetRouteTable();
				this.GetLimitCountTable();

				string strRouteData = string.Empty;

				foreach (DataRow drR in dtSearchRouteTable.Rows)
				{
					strRouteData += drR["BLDG_CODE"].ToString() + "," + drR["CSTPROD"].ToString() + "," + drR["LEVEL"].ToString() + "|";
				}

				if (strRouteData.Length > 0)
				{
					strRouteData = strRouteData.Substring(0, strRouteData.Length - 1);
				}

				string strLimitCountData = string.Empty;

				if (dtSearchLimitCountTable.Rows.Count > 0)
				{
					strLimitCountData = dtSearchLimitCountTable.Rows[0][0].ToString();
				}
				...
				//indata.Columns.Add("ELEC", typeof(string));
				indata.Columns.Add("BLDG_CODE", typeof(string));
				indata.Columns.Add("TRAY_TYPE_LIST", typeof(string));
				indata.Columns.Add("ROUTE_DATA", typeof(string));
				indata.Columns.Add("LIMIT_DATA", typeof(string));
				...
				//dr["ELEC"] = selectedElec;
				dr["BLDG_CODE"] = selectedBldg;
				dr["TRAY_TYPE_LIST"] = this.ConvertData(selectedTrayType);
				dr["ROUTE_DATA"] = strRouteData;        // 이동중 데이터 포함하기위해
				dr["LIMIT_DATA"] = strLimitCountData;   // 제한수량 데이터 포함하기위해
				indata.Rows.Add(dr);

                ds.Tables.Add(indata);

                //DataSet resultSet = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_PROCESS_MONITORING", "INDATA", "OUTDATA", ds);
				DataSet resultSet = new ClientProxy().ExecuteServiceSync_Multi("DA_PRD_SEL_FORMATION_STK_MNT3", "INDATA", "OUTDATA", ds);

				if (resultSet != null)
                {
                    dtSearchTable = resultSet.Tables["OUTDATA"];
					this.SetCombineData();      // 이동중데이터와 Merge

					//적재율 표시
					DataTable dtRackRate = dtSearchTable.DefaultView.ToTable(true, new string[] { "LV", "RACK_RATE" });

					this.tbLV0.Text = string.Empty;
					this.tbLV1.Text = string.Empty;
					this.tbLV2.Text = string.Empty;

					foreach (DataRow drRate in dtRackRate.Rows)
					{
						if (drRate["LV"].ToString() == "LV 0")
						{
							this.tbLV0.Text = string.Format("{0} : {1:0.##}%", drRate["LV"].ToString(), drRate["RACK_RATE"]);

							if (double.Parse(drRate["RACK_RATE"].ToString()) > 90)
							{
								this.tbLV0.Foreground = new SolidColorBrush(Colors.Red);
							}
							else
							{
								this.tbLV0.Foreground = new SolidColorBrush(Colors.Yellow);
							}
						}
						else if (drRate["LV"].ToString() == "LV 1")
						{
							this.tbLV1.Text = string.Format("{0} : {1:0.##}%", drRate["LV"].ToString(), drRate["RACK_RATE"]);

							if (double.Parse(drRate["RACK_RATE"].ToString()) > 90)
							{
								this.tbLV1.Foreground = new SolidColorBrush(Colors.Red);
							}
							else
							{
								this.tbLV1.Foreground = new SolidColorBrush(Colors.Yellow);
							}
						}
						else if (drRate["LV"].ToString() == "LV 2")
						{
							this.tbLV2.Text = string.Format("{0} : {1:0.##}%", drRate["LV"].ToString(), drRate["RACK_RATE"]);

							if (double.Parse(drRate["RACK_RATE"].ToString()) > 90)
							{
								this.tbLV2.Foreground = new SolidColorBrush(Colors.Red);
							}
							else
							{
								this.tbLV2.Foreground = new SolidColorBrush(Colors.Yellow);
							}
						}
					}

					if (this.tbLV0.Text.Equals(string.Empty))
					{
						cWidth0.Width = new GridLength(0, GridUnitType.Star);
					}
					else
					{
						cWidth0.Width = new GridLength(1, GridUnitType.Star);
					}

					if (this.tbLV1.Text.Equals(string.Empty))
					{
						cWidth1.Width = new GridLength(0, GridUnitType.Star);
					}
					else
					{
						cWidth1.Width = new GridLength(1, GridUnitType.Star);
					}

					if (this.tbLV2.Text.Equals(string.Empty))
					{
						cWidth2.Width = new GridLength(0, GridUnitType.Star);
					}
					else
					{
						cWidth2.Width = new GridLength(1, GridUnitType.Star);
					}
				}
			}
            catch (Exception ex)
		...
		private void SeachRemarkData()
		{
			...
			dr["LANGID"] = LoginInfo.LANGID;
			dr["BLDG_CODE"] = selectedBldg;
			dr["LEVEL"] = selectedLevel;

			RQSTDT.Rows.Add(dr);

			DataTable result = new ClientProxy().ExecuteServiceSync("DA_MCS_SEL_FORMATION_EQP_NOTE", "RQSTDT", "RSLTDT", RQSTDT);

			try
			{
				if (result != null && result.Rows.Count > 0)
				{
					this.m_Remark = result.Rows[0]["EQPT_NOTE"].ToString();
					//this.tbRemark.Text = result.Rows[0]["EQPT_NOTE"].ToString();
				}
				else
				{
					this.m_Remark = string.Empty;
			...
			//new ClientProxy().ExecuteService("DA_MCS_SEL_FORMATION_EQP_NOTE", "RQSTDT", "RSLTDT", RQSTDT, (result, exception) =>
			//{
			//	try
			//	{
			//		if (exception != null)
		...
		{
			foreach(DataRow dr in dtSearchTable.Rows)
			{
				//string strBldgCode = string.Empty;
				//string strCstProd = string.Empty;
				//string strLevel = string.Empty;

				//strBldgCode = dr["BLDG_CODE"].ToString();
				//strCstProd = dr["CSTPROD"].ToString();
				//strLevel = dr["LEVEL"].ToString();

				DataRow[] drs = dtSearchRouteTable.Select("LEVEL = '" + dr["LEVEL"].ToString() + "' AND CSTPROD = '" + dr["CSTPROD"].ToString() + "' AND BLDG_CODE = '" + selectedBldg + "'");

				if (drs.Length > 0)
				{
					dr["R_PLT_IN_BUILDING_CNT"] = Convert.ToInt32(dr["R_PLT_IN_BUILDING_CNT"]) + Convert.ToInt32(drs[0]["R_PLT_IN_BUILDING_CNT"]);
					dr["R_PLT_FROM_PACK2_CNT"] = Convert.ToInt32(dr["R_PLT_FROM_PACK2_CNT"]) + Convert.ToInt32(drs[0]["R_PLT_FROM_PACK2_CNT"]);
				}
			}
		}

		private void GetRouteTable()
		{
			const string bizRuleName = "DA_SEL_MCS_OUTRACK_BY_MES_GUI_MNT";
			...
			dtSearchRouteTable = new ClientProxy(_bizRuleIp, _bizRuleProtocol, _bizRulePort, _bizRuleServiceMode, _bizRuleServiceIndex).ExecuteServiceSync(bizRuleName, "RQSTDT", "RSLTDT", inDataTable);
		}

		private void GetLimitCountTable()
		{
			const string bizRuleName = "DA_SEL_MCS_LIMIT_CNT_BY_MES_GUI_MNT";
			//DataTable inDataTable = new DataTable("RQSTDT");
			...
			//dtSearchLimitCountTable = new ClientProxy(_bizRuleIp, _bizRuleProtocol, _bizRulePort, _bizRuleServiceMode, _bizRuleServiceIndex).ExecuteServiceSync(bizRuleName, "RQSTDT", "RSLTDT", inDataTable);
			dtSearchLimitCountTable = new ClientProxy(_bizRuleIp, _bizRuleProtocol, _bizRulePort, _bizRuleServiceMode, _bizRuleServiceIndex).ExecuteServiceSync(bizRuleName, "RQSTDT", "RSLTDT", null);
		}
        ...
        private void bind()
        {
			...
			if (dgMONITORING.Dispatcher.CheckAccess())
            {
				...
				dgMONITORING.UpdateLayout();
			}
            else
            {
				dgMONITORING.Dispatcher.BeginInvoke(new Action(() =>
				{
					dgMONITORING.ItemsSource = null;
					dgMONITORING.UpdateLayout();
				}));
        ...
        private void SetGridBind(DataTable dtBind)
        {
			...
			if (selectedLevel != "" && !string.IsNullOrEmpty(this.m_Remark))
			{
				string[] strLine = this.m_Remark.Split(new string[] { "\r\n" }, StringSplitOptions.None);

				int iPage = (int)Math.Ceiling(Convert.ToDouble(strLine.Length) / Convert.ToDouble(selectedRemarkRowCnt));
				lstRemark = new string[iPage];

				for(int i=0; i<iPage; i++)
				{
					lstRemark[i] = string.Empty;

					for (int j = 0; j < selectedRemarkRowCnt; j++)
					{
						if (strLine.Length <= i * selectedRemarkRowCnt + j) break;

						lstRemark[i] += strLine[i * selectedRemarkRowCnt + j];

						if (j < selectedRemarkRowCnt - 1)
							lstRemark[i] += "\r\n";
					}
				}
        ...
        private void dgMONITORING_MergingCells(object sender, DataGridMergingCellsEventArgs e)
        {
        ...
        private void SetCellColor(C1DataGrid dataGrid, C1.WPF.DataGrid.DataGridCellEventArgs e)
        {
			...
			*/
            ...
            if (e.Cell.Column.Name.Equals("T_PLT_CLEAN_BEFORE_CNT") || e.Cell.Column.Name.Equals("T_PLT_CLEAN_AFTER_CNT"))
            {
                if (e.Cell.Row.DataItem != null)
                {
                    if (!Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, e.Cell.Column.Name)).Equals(""))
                    {
                        if (!Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, e.Cell.Column.Name)).Equals("-"))
                        {
							//int iRecommendQty = Int32.Parse(Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, "RECOMMEND_CNT")));
							int iProperQty = Int32.Parse(Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, "PROPER_CNT")));
							int iBeforeQty = Int32.Parse(Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, "T_PLT_CLEAN_BEFORE_CNT")));
							int iAferQty = Int32.Parse(Util.NVC(DataTableConverter.GetValue(e.Cell.Row.DataItem, "T_PLT_CLEAN_AFTER_CNT")));

							if(iBeforeQty + iAferQty > iProperQty)
							{
        ...
        private void btnSetSave_Click(object sender, RoutedEventArgs e)
        {
			...
			selectedBldg = Convert.ToString(cboBldg.SelectedValue);
			selectedLevel = Convert.ToString(cboLevel.SelectedValue);
			selectedTrayType = Convert.ToString(cboTrayType.SelectedItemsToString);

			selectedViewRowCnt = Convert.ToInt32(numViewRowCnt.Value);
			selectedRemarkRowCnt = Convert.ToInt32(numRemarkViewRowCnt.Value);
			...
			dr["BLDG_CODE"] = selectedBldg;
			dr["LEVEL"] = selectedLevel;
			dr["TRAY_TYPE"] = selectedTrayType;
            dr["DISPLAYVIEWROWCNT"] = selectedViewRowCnt;
			dr["DISPLAYREMARKROWCNT"] = selectedRemarkRowCnt;
            ...
            LGC.GMES.MES.MNT001.MNT_Common.SetConfigXML_Formation_Stocker(ds);
        ...
        private void btnSetData_Click(object sender, RoutedEventArgs e)
        {
			...
			//selectedProcess = cboProcess.SelectedItemsToString;
			selectedBldg = cboBldg.SelectedValue.ToString();
			selectedLevel = cboLevel.SelectedValue.ToString();
			selectedTrayType = cboTrayType.SelectedItemsToString;

			selectedViewRowCnt = Convert.ToInt32(numViewRowCnt.Value);
			selectedRemarkRowCnt = Convert.ToInt32(numRemarkViewRowCnt.Value);
			selectedDisplayTime = Convert.ToInt32(numRefresh.Value);
			selectedDisplayTimeSub = Convert.ToInt32(numRefreshSub.Value);
			selectedColorDisplayFlag = cboColor.SelectedValue.ToString();
        ...
        private void setChart(int realViewCnt)
        {
            try
            {
				...
				if (grdRemark.Dispatcher.CheckAccess())
				{
					if (dtDiv == null) return;
					...
					if (selectedLevel != "" && !string.IsNullOrEmpty(this.m_Remark) && dtDiv.Length - iPage <= realViewCnt)
					{
						if (this.lstRemark.Length < realViewCnt - (dtDiv.Length - iPage) + 1) return;

						grdRemark.Visibility = Visibility.Visible;
						this.tbRemark.Text = this.lstRemark[realViewCnt - (dtDiv.Length - iPage)];
					}
					else
					{
						grdRemark.Visibility = Visibility.Collapsed;
					}
				}
				else
				{
					grdRemark.Dispatcher.BeginInvoke(new Action(() =>
					{
						if (dtDiv == null) return;
