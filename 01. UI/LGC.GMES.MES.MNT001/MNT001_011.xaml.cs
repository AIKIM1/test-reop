...
{
    public partial class MNT001_011 : Window
    {
        ...
        private static string selectedPlanFlag;
        ...
        private void InitColumnWidth()
        {
            ...
            oneRow_width = Convert.ToInt32(Math.Round(h_temp / 20));
        ...
        private void InitSetting()
        {
            try
            {
                ...
                c1Chart.Width = Screen.PrimaryScreen.Bounds.Width - 20;
        ...
        private void searchData()
        {
            try
            {
                ...
                DataSet resultSet = new ClientProxy().ExecuteServiceSync_Multi("DA_PRD_SEL_STOCK_IN_AREA_TO_MNT_LIST", "INDATA", "OUTDATA", ds);
        ...
        private void bind()
        {
            ...
            SetGridBind(dtSortTable); //화면 VIEW Row수에 설정된 row 갯수를 가지고 rotation 화면수 결정해서 번갈아 가며 띄움.
        ...
        private void SetGridBind(DataTable dtBind)
        {
            ...
            group dr by dr["PRJT_NAME"] into gg
            select new
            {
                PRJT_NAME = gg.Key,
            };
            ...
            //dtBind에서 PJT별로 div[i]에 저장


            dtDiv = null;

            dtDiv = new DataTable[div];
            ...
            foreach (var item in query)
            {
                ...
                if (!String.IsNullOrEmpty(item.PRJT_NAME.ToString()))
                {
                    dtDiv[i] = dtBind.Select("PRJT_NAME ='" + item.PRJT_NAME.ToString() + "'").CopyToDataTable();
            ...
            if (chkLock.Dispatcher.CheckAccess())
            {
                if (chkLock.IsChecked ?? false)
                {
                    if (realViewCnt <= 0)                   
                    {
                        setChartAndGraph(div - 1);
                        realViewCnt = div;
                    }
                    else
                    {                                       // realViewCnt를 화면에 먼저 반영한 뒤 +1함
                        setChartAndGraph(realViewCnt - 1);  // 그래서 realViewCnt-1 이 현재 보여지고 있는 페이지
                    }
                }
                else
                {
                    setChartAndGraph(realViewCnt);          // realViewCnt 는 다음 rotationViewPlay 시 보여질 페이지
                    realViewCnt++;
                }
            }
            else
            {
                chkLock.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(delegate
                {
                    if (chkLock.IsChecked ?? false)
                    {
                        ...
                        {                                  
                            setChartAndGraph(realViewCnt - 1);
                        }
                    }
                    else
                    {
                        setChartAndGraph(realViewCnt);        
                        realViewCnt++;
                    }
                }));
            }
        ...
        private void drawChart(int idx)
        {
            ...
            foreach (var item in dtDiv[idx].Select("SORT = 1").AsQueryable().Select(item => item["GOOD_LOT_QTY"]).ToList())
            {
                if (maxVal < Util.NVC_Int(item.ToString()))
                    maxVal = Util.NVC_Int(item.ToString());
            }
            foreach (var item in dtDiv[idx].Select("SORT = 1").AsQueryable().Select(item => item["PLANQTY"]).ToList())
            {
                if (maxVal < Util.NVC_Int(item.ToString()))
                    maxVal = Util.NVC_Int(item.ToString());
            }

            c1Chart.View.AxisY.Max = maxVal + maxVal / 10;
            c1Chart.View.AxisX.Max = 10;                    //그래프 최대 수
            //c1Chart.View.AxisX.AutoMax = true;            //그래프 수에 따라 라벨 위치 및 그래프 크기 변경 기능
            c1Chart.Data.ItemNames = dtDiv[idx].Select("SORT = 1").AsQueryable().Select(item => item["PRODNAME"]).ToList();

            //양품량
            var ds1 = new C1.WPF.C1Chart.DataSeries();
            ds1.ValuesSource = dtDiv[idx].Select("SORT = 1").AsQueryable().Select(item => item["GOOD_LOT_QTY"]).ToList();
            ds1.Label = "Good QTY";
            ds1.ChartType = C1.WPF.C1Chart.ChartType.Column;
            ds1.SymbolFill = new SolidColorBrush(Colors.Orange);
           ...
           var ds2 = new C1.WPF.C1Chart.DataSeries();
            ds2.ValuesSource = dtDiv[idx].Select().AsQueryable().Select(item => item["GOOD_PCT"]).ToList();
            ds2.Label = "Good/Plan(%)";
            ds2.AxisY = "PCT";
            ds2.ChartType = C1.WPF.C1Chart.ChartType.LineSymbols;
            ds2.ConnectionStrokeThickness = 5;
            ds2.ConnectionFill = new SolidColorBrush(Colors.Aqua);
            ds2.SymbolFill = new SolidColorBrush(Colors.Aqua);
            ...
            */
            ...
            ds3.ValuesSource = dtDiv[idx].Select("SORT = 1").AsQueryable().Select(item => item["PLANQTY"]).ToList();
            ds3.Label = "Plan";
            ds3.ChartType = C1.WPF.C1Chart.ChartType.StepSymbols;
            ds3.ConnectionStrokeThickness = 7;
            ds3.ConnectionStrokeDashes = DoubleCollection.Parse("2.5, 1");
            ds3.ConnectionFill = new SolidColorBrush(Colors.Aqua);
            c1Chart.Data.Children.Add(ds3);
            ds3.SymbolFill = new SolidColorBrush(Colors.Aqua);
        ...
        private void initChart()
        {
            ...
            c1Chart.View.AxisX.MajorUnit = 1;
            c1Chart.View.AxisX.MinorUnit = 1;
            c1Chart.View.AxisX.IsDependent = true;
            c1Chart.View.AxisX.Max = 7;
            c1Chart.View.AxisX.Min = -1;
        ...
        private void dgMONITORING_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            {//수정필요
                    ...
                    && !(Util.NVC(DataTableConverter.GetValue(dgMONITORING.CurrentRow.DataItem, "HOLD_LOT_QTY")) == "-"))
                {
                    ...
                    MNT001_011_HoldList pHoldlist = new MNT001_011_HoldList();


                    object[] Parameters = new object[4];
                    Parameters[0] = selectedArea;
                    Parameters[1] = selectedEquipmentSegment;
                    Parameters[2] = Util.NVC(DataTableConverter.GetValue(dgMONITORING.CurrentRow.DataItem, "PRODID"));
                    Parameters[3] = Util.NVC(DataTableConverter.GetValue(dgMONITORING.CurrentRow.DataItem, "PRJT_NAME"));
        ...
        private void setChartAndGraph(int realViewCnt)
        {
            try
            {
                if (dgMONITORING.Dispatcher.CheckAccess())
