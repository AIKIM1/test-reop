/*************************************************************************************
 Created Date : 2020.
      Creator : 
   Decription : Tray ID 발행
--------------------------------------------------------------------------------------
 [Change History]
  2020.  DEVELOPER : Initial Created.
  2023.12.07   조영대    옵션에 따라 발행시 Carrier 테이블 생성
  2023.12.18   손동혁    Carrier 테이블 생성 시 반장급 이상만 신규 발행 하도록 NA 1동 요청
**************************************************************************************/
...
{
    public partial class FCS001_065 : UserControl, IWorkArea
    {

        #region [Declaration & Constructor]

        private Util Util = new Util();
        private bool isCarrierInsert = false;
        private bool bUseFlag = false; //2023.12.18 NA1동 요청 등록된 아이디 ( 반장급 이상)만 해당 신규 발행 허용
        public FCS001_065()
        {
            InitializeComponent();
        }
        ...
        private void UserControl_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            getLabelPsoi();
            cboDarkNess();
            bUseFlag = Util.IsAreaCommonCodeUse("FORM_TRAY_NEW_REG_PERMIT", LoginInfo.USERID); //동별공통코드에 기준정보 등록되어 있고, Attr1 속성의 값이 "Y"인 경우 기능 사용 가능.
        ...
        private void InitControl()
        {
            ...
            isCarrierInsert = Util.IsAreaCommonCodeUse("PROGRAM_BY_FUNC_USE_FLAG", "FCS001_065_CARRIER_INSERT"); 
        }
        ...
        private void InitCombo()
        {
            ...
            _combo.SetCombo(cboHistTrayType, CommonCombo_Form.ComboStatus.SELECT, sCase: "TRAYTYPE");
        }
        #endregion
        ...
        private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                for (int intLoop = 0; intLoop < int.Parse(txtTrayCnt.Text); intLoop++)
                {
                    ...
                    // Carrier 테이블 생성
                    if (isCarrierInsert && IssueFlag.Equals("I")) InsertCarrier(IssueID);
                ...
                {
                    PrintLabel(new object[] { IssueIDList[i].ToString(), txtPerCnt.Text }, IssueFlag);
                    System.Threading.Thread.Sleep(100);
                }
        ...
        {
            //라벨 포지션 registry 저장
            setLabelPsoi(txtXAXIS.Text, txtYAXIS.Text, txtBarcodeXY.Text, txtBarcodeOption.Text, txtBarTextXY.Text, txtBarTextSize.Text);

            string IssueFlag = "X";

            //PrintLabel(new object[] { "XFTB200216", "1" }, IssueFlag);
            PrintLabel(new object[] { "0000000000", "1" }, IssueFlag);
        }

        private void btnTextPrint_Click(object sender, RoutedEventArgs e)
        {
            PrintLabelText(new object[] { txtTextPrint.Text, txtTextCount.Text }, string.Empty);
        }
        ...
        private void PrintLabelText(object[] PrintData, string IssueFlag)
        {
            try
            {
                string sTrayID = PrintData[0] as string;
                ...
                PrintCode = string.Format(ZplCode, "   " + txtXAXISText.Text, // 0 X축
                                                           txtYAXISText.Text, // 1 Y축
                                                           txtYScale.Text,
                                                           txtXScale.Text,
                                                           sTrayID,       // 2 Tray ID
                                                           sCnt,          // 3  발행 수량
        ...
        private void InserIssueHist(string I_TRAYID, string I_TYPE, string I_POSI, string I_REMARK, string I_USER)
        {
            try
            {
        ...
        private void InsertCarrier(string I_TRAYID)
        {
            try
            {
                ...
                DataTable dtRslt = new ClientProxy().ExecuteServiceSync("BR_FORM_REG_CSTID", "RQSTDT", "RSLTDT", dtRqst);
            }
            catch (Exception ex)
            {
                Util.MessageException(ex);
        ...
        private void cboDarkNess()
        {
            DataTable dt = new DataTable();
            dt.Columns.Add("CBO_CODE", typeof(string));
            dt.Columns.Add("CBO_VALUE", typeof(string));

            for (int i = 0; i < 30; i++)
            {
                DataRow row = dt.NewRow();
                row["CBO_CODE"] = (i + 1).ToString();
                row["CBO_VALUE"] = (i + 1).ToString();
                dt.Rows.Add(row);
            }

            this.cboDarkness.DisplayMemberPath = "CBO_CODE";
            this.cboDarkness.SelectedValuePath = "CBO_VALUE";
            this.cboDarkness.ItemsSource = dt.Copy().AsDataView();

            this.cboDarkness.SelectedValue = "15";
        } 
        #endregion
