...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2023.10.30 이정미 : EMS 설비 관리체계 변경으로 설비 LEVEL 수정
  2023.11.12 조영대 : 적용 Zone(APPLY_ZONE_ID), 적용 구간(APPLY_DVRTR_PORT_ID) 컬럼 추가
**************************************************************************************/
...
{
    public partial class FCS001_127 : UserControl, IWorkArea
    {
        ...
        private void InitCombo()
        {
            ...
            string _sFilter = GetAreaCommoncodeAttr("EQP_LEVEL_TYPE");

            string[] sFilter0 = { _sFilter };
            _combo.SetCombo(cboEqpGrp, CommonCombo_Form.ComboStatus.SELECT, sCase: "CNVR_GRP", sFilter: sFilter0);

            string[] sFilter1 = { "CNVR_LOGIC_TYPE_CODE" };
            _combo.SetCombo(cboStepType, CommonCombo_Form.ComboStatus.ALL, sCase: "CMN", sFilter: sFilter1);

            string[] sFilter2 = { "USE_FLAG" };
            _combo.SetCombo(cboUseFlag, CommonCombo_Form.ComboStatus.NONE, sCase: "CMN", sFilter: sFilter2);
        ...
        private void GetListData(string port_id, string seqno)
        {
            try
            {
               Util.gridClear(dgCMDCOND);
                ...
                DataTable dtRslt = new ClientProxy().ExecuteServiceSync("DA_SEL_CNVR_TRF_CMD_COND_UI", "RQSTDT", "RSLTDT", dtRqst);    
        ...
        private string GetAreaCommoncodeAttr(string sCodeType)
        {
            try
            {
                DataTable RQSTDT = new DataTable();
                ...
                if (dtResult != null)
                {
                    ...
                    {
                        Util.MessageValidation("동별 공통코드 (EQP_LEVEL_TYPE) 데이터 설정을 한개만 해야 합니다.");
                    }
                    else if (dtResult.Rows.Count == 1)
                    {
                        return Util.NVC(dtResult.Rows[0]["ATTR2"]);
                    }
                    else
                    {
                        // 설정이 없을 경우 메인 (M) 리턴
        ...
        private void Cmd_btnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                dtRqst.Columns.Add("APPLY_ZONE_ID", typeof(string));
                dtRqst.Columns.Add("APPLY_DVRTR_PORT_ID", typeof(string));
                ...
                for (int i = 0; i < dgCMD.Rows.Count; i++)
                {
                    if (Util.NVC(DataTableConverter.GetValue(dgCMD.Rows[i].DataItem, "FLAG")).Equals("Y"))
                    {
                        ...
                        dr["APPLY_ZONE_ID"] = Util.NVC(DataTableConverter.GetValue(dgCMD.Rows[i].DataItem, "APPLY_ZONE_ID"));
                        dr["APPLY_DVRTR_PORT_ID"] = Util.NVC(DataTableConverter.GetValue(dgCMD.Rows[i].DataItem, "APPLY_DVRTR_PORT_ID"));
                        dr["UPDUSER"] = LoginInfo.USERID;
        ...
        private void cboEqpGrp_SelectionCommitted(object sender, PropertyChangedEventArgs<object> e)
        {
            ...
            _combo.SetComboObjParent(cboPortId, CommonCombo_Form.ComboStatus.ALL, sCase: "BCR_LOC", objParent: cboCraneParent);
        }
        ...
        private void cboUseFlag_SelectionCommitted(object sender, PropertyChangedEventArgs<object> e)
        {
            btnSearch.PerformClick();
        }
        #endregion
