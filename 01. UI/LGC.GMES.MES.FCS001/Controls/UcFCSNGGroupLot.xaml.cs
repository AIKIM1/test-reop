/*************************************************************************************
 Created Date : 2020.10.12
   ...
   Decription : 활성화 공정진척 - ReCheck / NG Lot 정보
--------------------------------------------------------------------------------------
 [Change History]
  2020.10.12  NAME : Initial Created
  2021.04.18  KDH : 컬럼명 변경 및 데이타 출력값 변경(LOT TYPE)
**************************************************************************************/
using System;
...
{
    /// <summary>
    /// UcFCSNGLot.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class UcFCSNGGroupLot : UserControl
    {
        #region Declaration
        ...
        public C1DataGrid DgNgGroupLot { get; set; }
        ...
        {
            Util.gridClear(dgNgGroupLot);
        }

        private void SetControl()
        {
            DgNgGroupLot = dgNgGroupLot;
        }
        ...
        private void dgNgGroupLot_LoadedCellPresenter(object sender, DataGridCellEventArgs e)
        {
            if (sender == null)
                return;
        ...
        private void dgNgGroupLot_UnloadedCellPresenter(object sender, DataGridCellEventArgs e)
        {
        ...
        private void dgNgGroupLot_MouseRightButtonUp(object sender, MouseButtonEventArgs e)
        {
            Point pnt = e.GetPosition(null);
            C1.WPF.DataGrid.DataGridCell cell = dgNgGroupLot.GetCellFromPoint(pnt);

            if (cell == null) return;

            // 선택한 셀의 Row 위치
            int rowIdx = cell.Row.Index;
            DataRowView dv = dgNgGroupLot.Rows[rowIdx].DataItem as DataRowView;
        ...
        public void SelectNgGroupLotList(string LotID = null)
        {
            GetNgGroupLotList(LotID);
        }
        ...
        private void GetNgGroupLotList(string LotID = null)
        {
            try
            {
                ...
                new ClientProxy().ExecuteService("DA_PRD_SEL_WIP_FCS_RN_DRB", "INDATA", "OUTDATA", inTable, (bizResult, bizException) =>
                {
                    try
                    {
                        ...
                        Util.GridSetData(dgNgGroupLot, bizResult, null, true);
        ...
        private void dgChoiceNg_Checked(object sender, RoutedEventArgs e)
        {
            try
            {
                //if (sender == null)
