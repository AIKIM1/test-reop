...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2024.11.18  이지은 : 2차 저전압 검색시 WRKLOG_TYPE_CODE L로 변경
**************************************************************************************/
...
{
    public partial class FCS001_110_SUMMARY : UserControl, IWorkArea
    {
        ...
        private void GetList()
        {
            try
            {
                ...
                if (bUseFlag)
                {
                    ...
                    {
                        dr["WRKLOG_TYPE_CODE"] = "L"; //2차 저전압 2024.11.18
                    }
                }
                else
                {
        ...
        private void SetGridPerf(C1DataGrid dg, DataTable dt, DataTable dtDetail)
        {
            try
            {
                ...
                foreach (DataRow drDetail in dtDetail.Rows)
                {
                    ...
                    //LOT TYPE 추가 시 수정
                    string sCurrRow = "";
                    sCurrRow = (Util.NVC(drDetail["LOT_COMMENT"]) + "_" + Util.NVC(drDetail["EQSGID"]) + "_" + Util.NVC(drDetail["PROD_LOTID"])
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        //LOT TYPE 추가 시 수정
                        string sRow = "";
                        sRow = (Util.NVC(dt.Rows[i]["LOT_COMMENT"]) + "_" + Util.NVC(dt.Rows[i]["EQSGID"]) + "_" + Util.NVC(dt.Rows[i]["PROD_LOTID"])
                        ...
                        if (sCurrRow.Equals(sRow))
                        {
                            ...
                            if (iRow != -1 && iCol != -1)
                            {
                                ...
                                dt.Rows[iRow]["LOT_COMMENT"] = Util.NVC(drDetail["LOT_COMMENT"]);
                                break;
                            }
                        }
                    }
                ...
                string[] sColumnName = new string[] { "MDLLOT_ID", "EQSGID", "EQSGNAME", "PROD_LOTID" };
        ...
        private void dgDirList_MouseDoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            if (wndPopup != null)
            {
                ...
                if (datagrid.Name.Equals("dgDIR"))
                {
                    ...
                    Parameters[8] = "G";//"AFTER_DEGAS_OCV_SELECTOR_FRST_MOVE";
                    Parameters[9] = "FRST";
                    if (rdo2LowVolt.IsChecked.Equals(true))
                    {
                        Parameters[8] = "L";//"AFTER_DEGAS_OCV2_SELECTOR_FRST_MOVE";
                    }
                    Parameters[11] = "";
                }
                else
                {
                    Parameters[0] = dtpWorkDate.SelectedDateTime.ToString("yyyy-MM-dd"); //WORK_DATE
                    Parameters[1] = Util.GetCondition(cboShift, bAllNull: true); //SHFT_ID
                    Parameters[8] = "G";//"AFTER_DEGAS_OCV_SELECTOR_RWK_MOVE";
                    Parameters[9] = "RWK";
                    if (rdo2LowVolt.IsChecked.Equals(true))
                    {
                        Parameters[8] = "L";//"AFTER_DEGAS_OCV2_SELECTOR_RWK_MOVE";
                    }
