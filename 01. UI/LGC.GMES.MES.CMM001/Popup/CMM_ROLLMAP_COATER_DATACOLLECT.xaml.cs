/*************************************************************************************
 Created Date : 2022.11.22
      Creator : 신광희
   Decription : 코터 롤맵 불량정보 팝업
--------------------------------------------------------------------------------------
 [Change History]
  2022.11.22  신광희 : Initial Created.
  2022.12.08  신광희 : 불량 합계 및 상세 UI 변경 및 변경 BizRule 적용
**************************************************************************************/
...
{
    /// <summary>
    /// CMM_ROLLMAP_COATER_DATACOLLECT.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class CMM_ROLLMAP_COATER_DATACOLLECT : C1Window, IWorkArea
    {
        #region Initialize 
        ...
        public CMM_ROLLMAP_COATER_DATACOLLECT()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private void GetDefectList()
        {
            try
            {
                const string bizRuleName = "BR_PRD_SEL_DEFECT_RM";
                ...
                if(CommonVerify.HasTableInDataSet(dsResult))
                {
                    ...
                    if (CommonVerify.HasTableRow(dsResult.Tables["OUT_BACK"]))
                    {
                        ...
                        }).Select(g => new
                        {
                            ActivityCode = g.Key.activityCode,
                            ActivityName = g.Key.activityName.Split(' ')[0] + " " + ObjectDic.Instance.GetObjectName("소계"),
                            ReasonQty = g.Sum(x => x.Field<decimal>("MES_RESNQTY")),
                            RollMapReasonQty = g.Sum(x => x.Field<decimal>("RM_RESNQTY")),
                            DifferenceYn = "N",
                            ActivityReasonCode = "ZZZZZZZZZZ",
                            Count = g.Count()
                    ...
                    if (CommonVerify.HasTableRow(dsResult.Tables["OUT_PRODUCTION"]))
                    {
                        ...
                        {
                            DataTable dtTop = dsResult.Tables["OUT_TOP"].Copy();
                            topDefectQty = dtTop.AsEnumerable().Where(x => x.Field<string>("ACTID") == "DEFECT_LOT").Sum(s => s.Field<decimal>("RM_RESNQTY"));
                            topDefectQty = topDefectQty * (decimal)0.5;

                            topLossQty = dtTop.AsEnumerable().Where(x => x.Field<string>("ACTID") == "LOSS_LOT").Sum(s => s.Field<decimal>("RM_RESNQTY"));
                            topLossQty = topLossQty * (decimal)0.5;

                            topMesTopLossQty = dtTop.AsEnumerable().Select(s => s.Field<decimal>("MES_TOPLOSS")).FirstOrDefault();
                            topRollMapTopLossQty = dtTop.AsEnumerable().Select(s => s.Field<decimal>("RM_TOPLOSS")).FirstOrDefault();
                        }

                        if (CommonVerify.HasTableRow(dsResult.Tables["OUT_BACK"]))
                        {
                            DataTable dtBack = dsResult.Tables["OUT_BACK"].Copy();
                            backDefectQty = dtBack.AsEnumerable().Where(x => x.Field<string>("ACTID") == "DEFECT_LOT").Sum(s => s.Field<decimal>("RM_RESNQTY"));
                            backLossQty = dtBack.AsEnumerable().Where(x => x.Field<string>("ACTID") == "LOSS_LOT").Sum(s => s.Field<decimal>("RM_RESNQTY"));

                            backMesTopLossQty = dtBack.AsEnumerable().Select(s => s.Field<decimal>("MES_TOPLOSS")).FirstOrDefault();
                            backRollMapTopLossQty = dtBack.AsEnumerable().Select(s => s.Field<decimal>("RM_TOPLOSS")).FirstOrDefault();
                        }
                        ...
                        { }).Select(g => new
                        {
                            ...
                            GoodQty = g.Sum(x => x.Field<decimal>("WIPQTY_ED")), //양품량(MES)
                            InputQty = g.Sum(x => x.Field<decimal>("INPUT_QTY")), //생산량(MES)
                            DefectQty = topDefectQty + backDefectQty,
                            LossQty = topLossQty + backLossQty,
                            ProductRequestQty = g.Sum(x => x.Field<decimal>("CNFM_PRDT_REQ_QTY")),
                            Count = g.Count()
                        }).ToList();
