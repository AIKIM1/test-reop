/*************************************************************************************
 Created Date : 2020.11.09
      Creator : 정문교
   Decription : 작업조, 작업자 조회 팝업
--------------------------------------------------------------------------------------
 [Change History]
  2020.10.12  정문교 : Initial Created.   
                       CMM_SHIFT_USER2 Copy 생성
  ...
  2025.04.07  조범모 : 중복 데이터가 존재 메시지 호출 제거(주석처리)
**************************************************************************************/
...
namespace LGC.GMES.MES.CMM001.Popup
{
    ...
    /// </summary>
    public partial class CMM_SHIFT_USER2_DRB : C1Window, IWorkArea
    {
        ...
        public CMM_SHIFT_USER2_DRB()
        {
            InitializeComponent();
        }

        public CMM_SHIFT_USER2_DRB(TextBox target)
        {
            InitializeComponent();
            _target = target;
            ApplyPermissions();
        ...
        private void btnSelect_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dgWorkUser.Rows.Count > 0)
            {
                ...
                {
                    _RetWrkGrID = Util.NVC(DataTableConverter.GetValue(dgWorkUser.Rows[i].DataItem, "WRK_GR_ID"));
        ...
        private void SetdgShiftUserGrid()
        {
            ...
            if (Util.NVC(_ShiftUserSearchFlag).Equals("Y"))  
            {
                ...
                dgShiftUser.Columns["WRK_GR_ID_3"].Visibility = Visibility.Collapsed;
                ...
                dgShiftUser.Columns["WRK_GR_ID_4"].Visibility = Visibility.Collapsed;
                ...
                dgShiftUser.Columns["WRK_GR_ID_5"].Visibility = Visibility.Collapsed;
        ...
        private void GetShiftUser(string wrkGrID)
        {
            ...
            {
                sBizName = "DA_BAS_SEL_TB_MMD_AREA_PROC_USER5_DRB_V01";
            }
            else
            {
                sBizName = "DA_BAS_SEL_TB_MMD_AREA_PROC_USER5_DRB";
            }
        ...
        private void GetWrkGr()
        {
            ...
            new ClientProxy().ExecuteService("DA_BAS_SEL_TB_MMD_AREA_PROC_GR_LIST_DRB", "RQSTDT", "RSLTDT", IndataTable, (result, searchException) =>
            {
                try
                {
                    if (searchException != null)
                    ...
                    GetShiftUser(result.Rows[0]["WRK_GR_ID_1"].ToString());
                }
                catch (Exception ex)
                {
                    Util.MessageException(ex);
        ...
        private void SaveCalendar()
        {
            ...
            newRow["AREAID"] = _Area;
            newRow["EQSGID"] = _Segment;
            newRow["PROCID"] = _Proc;
            newRow["SHFT_ID"] = _RetShiftCode;
            newRow["WRK_GR_ID"] = _RetWrkGrID;
            newRow["FROMDATE"] = _RetWrkStrtTime.Substring(0, 10);
            newRow["TODATE"] = _RetWrkEndTime.Substring(0,10);
            newRow["USERID"] = LoginInfo.USERID;
            inTable.Rows.Add(newRow);

            string[] UserID = _RetUserID.Split(',');

            for(int row = 0; row < UserID.Length; row++)
            {
                newRow = inWorkUser.NewRow();
                newRow["WRK_USERID"] = UserID[row];
                ...
                if (!bRemove)
                {
                    ...
                    if (!DataTableConverter.GetValue(sUser, "USERID_" + col).ToString().Equals(""))
                    {
                        ...
                        else
                        {
                            ...
                            dt7.Columns.Add("WRK_GR_ID", typeof(String));
                        }
                        ...
                        dr["WRK_GR_ID"] = DataTableConverter.GetValue(sUser, "WRK_GR_ID_" + col).GetString();
