...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2024.01.16  배현우 : W/O 선택시 포트별 자재 설정 화면 사용하는 동에 한하여 Alarm Message 변경
  2024.07.12  이동주 : E20240615-001915  LNS –PKG 고공 물류 반송 삭제
**************************************************************************************/
...
namespace LGC.GMES.MES.CMM001.Popup
{
    ...
    /// </summary>
    public partial class CMM_WORKORDER_LINE_DRB : C1Window, IWorkArea
    {
        ...
        private void InitializeGridColumns()
        {
            ...
            if (_processCode.Equals(Process.TOP_COATING) || _processCode.Equals(Process.INS_COATING) || _processCode.Equals(Process.SRS_COATING) || _processCode.Equals(Process.HALF_SLITTING) ||
                _processCode.Equals(Process.ROLL_PRESSING) || _processCode.Equals(Process.TAPING) || _processCode.Equals(Process.REWINDER) || _processCode.Equals(Process.LASER_ABLATION) ||
            ...
            // 2023.10.31 소형 조립만 HIDDEN 처리함
            // 2023.10.17 조립버전 추가로 주석 처리함 주동석 
        ...
        private void SetWorkOrderSelect(DataRow dr, bool isSelectFlag = true)
        {
            try
            {
                ...
                if (isSelectFlag)
                {
                    if (IsRemoveAreaHardCode("PORT_MTRL_SET_USE"))
                        Util.MessageInfo("SFU3677");
                    else
        ...
        private void WorkOrderChange(DataRow dr, bool isSelectFlag = true)
        {
            ...
            SetCtccEqptByWoChange(); // 2024.07.12 DJLEE W/O 변경 시 기존 명령 취소 요청
        ...
        /// <summary>
        /// W/O 변경 시 기존 명령 취소 요청
        /// </summary>
        private void SetCtccEqptByWoChange()
        {
            try
            {
                DataTable inTable = new DataTable();
                ...
                new ClientProxy().ExecuteServiceSync("BR_PRD_REG_CANCEL_MHS_TRF_CMD", "INDATA", "", inTable);
            }
            catch (Exception ex)
            {
                // Do Nothing
