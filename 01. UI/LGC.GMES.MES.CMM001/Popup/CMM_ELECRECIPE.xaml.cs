...
namespace LGC.GMES.MES.CMM001.Popup
{
    ...
    /// </summary>
    public partial class CMM_ELECRECIPE : C1Window, IWorkArea
    {
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_PROCID.Equals(Process.PRE_MIXING) || _PROCID.Equals(Process.MIXING) || _PROCID.Equals(Process.SRS_MIXING))
            {
                dgRecipeNo.Columns["LANE_QTY"].Visibility = Visibility.Collapsed;
                dgRecipeNo.Columns["LANE_PTN_QTY"].Visibility = Visibility.Collapsed;
                //GetVersion_MX();
                GetRecipeNo(); //믹서도 동일하게 변경
            }
            else
            {
                dgRecipeNo.Columns["LANE_QTY"].Visibility = Visibility.Visible;
                //dgRecipeNo.Columns["LANE_PTN_QTY"].Visibility = Visibility.Visible;
                dgRecipeNo.Columns["LANE_PTN_QTY"].Visibility = Visibility.Collapsed;
                GetRecipeNo();
            }
        }
        ...
        {
            if (LoginInfo.CFG_SHOP_ID.Equals("A041"))
            {
        ...
        private void GetVersion_MX()
        {
            try
            {
                ...
                DataRow Indata = IndataTable.NewRow();
                Indata["CMCDTYPE"] = "PROD_VER_CODE";
                IndataTable.Rows.Add(Indata);

                DataTable dtMain = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMONCODE", "INDATA", "RSLTDT", IndataTable);

                dtMain.Columns.Add("CHK", typeof(int));
                dtMain.Columns["CBO_NAME"].ColumnName = "PROD_VER_CODE";
        ...
        private void dgRecipeNoChoice_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if ((bool)rb.IsChecked)
            {
                ...
                if (string.Equals(_VERSION_CHK, "Y") && !string.Equals(DataTableConverter.GetValue(dg.Rows[idx].DataItem, "LANE_QTY_CNFM_FLAG"), "Y"))
                {
                ...
                {
                    _RecipeNo = DataTableConverter.GetValue(dgRecipeNo.Rows[idx].DataItem, "PROD_VER_CODE").ToString();
                    _Laneqty = DataTableConverter.GetValue(dgRecipeNo.Rows[idx].DataItem, "LANE_QTY").ToString();
                    _PtnQty = DataTableConverter.GetValue(dgRecipeNo.Rows[idx].DataItem, "LANE_PTN_QTY").ToString();
                }
            ...
            if (dataGrid != null && string.Equals(_VERSION_CHK, "Y"))
            {
                dataGrid.Dispatcher.BeginInvoke(new Action(() =>
                {
                    if (e != null && e.Cell != null && e.Cell.Presenter != null)
                    {
                        ...
                        {
                            if ((string.Equals(e.Cell.Column.Name, "CHK") || string.Equals(e.Cell.Column.Name, "PROD_VER_CODE") || string.Equals(e.Cell.Column.Name, "LANE_QTY")) && !string.Equals(DataTableConverter.GetValue(e.Cell.Row.DataItem, "LANE_QTY_CNFM_FLAG"), "Y"))
                            {
