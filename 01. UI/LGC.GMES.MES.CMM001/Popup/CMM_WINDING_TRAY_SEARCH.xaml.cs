/*************************************************************************************
 Created Date : 2019.04.18
      Creator : 강민준C
   Decription : Winding Tray 조회 
--------------------------------------------------------------------------------------
 [Change History]
   2019.04.18   INS 강민준C : Initial Created.
...
{
    public partial class CMM_WINDING_TRAY_SEARCH : C1Window, IWorkArea
    {
        #region Declaration
        ...
        string _sPGM_ID = "CMM_WINDING_TRAY_SEARCH";
        ...
        public CMM_WINDING_TRAY_SEARCH()
        {
            InitializeComponent();
        }
        ...
        #region [Form Load]
        private void CMM_WINDING_TRAY_SEARCH_Loaded(object sender, RoutedEventArgs e)
        {
        ...
        {
            if (string.IsNullOrEmpty(txtLotId.Text.Trim()) || txtLotId.Text.Length != 10)
            {
                Util.MessageValidation("SFU3675");
                return false;
            }

            bool chk = System.Text.RegularExpressions.Regex.IsMatch(txtLotId.Text.ToUpper(), @"[^a-zA-Z0-9가-힣]");

            if (chk)
            {
                //Util.Alert("{0}의 TRAY_ID 특수문자가 있습니다. 생성할 수 없습니다", txtTrayId.Text.ToUpper());
                Util.MessageValidation("SFU1298", txtLotId.Text.ToUpper());
                return false;
            }
        ...
        private void GetSearchGrid()
        {
            ...
            try
            {
                ...
                sBizName = "DA_PRD_SEL_WINDING_TRAY_HISTORY";       // W/O 체크해제
                ...
                inDataRow["CSTID"] = Util.NVC(txtLotId.Text.Trim());
