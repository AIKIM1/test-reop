/*************************************************************************************
 Created Date : 2023.05.13
      Creator : 신광희
   Decription : 코터 롤맵 수정(실적조회) 팝업 - CMM_ROLLMAP_COATER_DEFECT Copy, 변경된 BizRule 적용 
--------------------------------------------------------------------------------------
 [Change History]
  2024.05.13  신광희 : Initial Created.
  2024.07.29  신광희 : Overlay 폭, Sliding, Fat Edge 불량 표현 시 LOTATTR.HALF_SLIT_SIDE 값이 없는 경우 EQUIPMENTATTR.S43 첫번째 LANE 위치[FRST_LANE_PSTN] 가져오도록 수정 함.
  2025.06.09  김현진 : 롤맵 실적 수정 조회 Sample Cut 미표시 및 RW 값 위치 오표시 건 수정
**************************************************************************************/
...
namespace LGC.GMES.MES.CMM001.Popup
{
    ...
    public partial class CMM_RM_CT_RESULT : C1Window, IWorkArea
    {
        ...
        private bool _isEquipmentTotalDry;
                ...
                foreach (C1.WPF.DataGrid.DataGridRow row in dgLotInfo.Rows)
                {
                    if (DataTableConverter.GetValue(row.DataItem, "UPDATE_FLAG").GetString() == "Y")
                    {
                        if (string.IsNullOrEmpty(DataTableConverter.GetValue(row.DataItem, "BACK_RESNCODE").GetString()))
                        {
                            ...
                            ControlsLibrary.MessageBox.Show(MessageDic.Instance.GetMessage("SFU10025"), null, "Modify RollMap", MessageBoxButton.OKCancel, MessageBoxIcon.None, (result) =>
                            {
        ...
        private void tagDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            Util.MessageConfirm("SFU3475", (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    ...
                    {
                        //BizActor 호출 TAG 삭제
                        const string bizRuleName = "DA_PRD_UPD_RM_RPT_TAGSPOT_DEL";
        ...
        private void dgLotInfo_CommittedEdit(object sender, DataGridCellEventArgs e)
        {
            try
            {
                ...
                if (e.Cell.Column.Name == "ADJ_STRT_PSTN" || e.Cell.Column.Name == "ADJ_END_PSTN")
                {
                    ...
                    const string bizRuleName2 = "BR_PRD_CHK_RM_RPT_TAG_SECTION_CHANGE";
        ...
        private void InitializeControl()
        {
            ...
            SetEqptDetailInfoByCommoncode(); //공통코드 ROLLMAP_EQPT_DETAIL_INFO 설정에 따라 웹게이지 MIN/MAX 툴팁반영 여부 Flag 초기화
        ...
        private void SelectRollMap()
        {
            ...
            string pointTop, pointBack, positionTop, positionBack;
            ...
            newRow["SMPL_FLAG"] = "Y";
            newRow["LANE_LIST"] = null;
            newRow["SUM_FLAG"] = "2"; //(gdColorMap.Visibility == Visibility.Visible && (bool)rdoGradeBlock?.IsChecked) ? "1" : "2";
            inTable.Rows.Add(newRow);
            ...
            new ClientProxy().ExecuteService_Multi(bizRuleName, "INDATA", "OUT_LENGTH,OUT_MATERIAL,OUT_HEAD,OUT_GAUGE,OUT_DEFECT,OUT_LANE", (bizResult, bizException) =>
            {
                try
                {
                    ...
                    if (CommonVerify.HasTableInDataSet(bizResult))
                    {
                        ...
                        if (_isSearchMode) SetCoaterChartContextMenu(false); //조성근 2025.02.11 조회팝업에서 조회만 되도록 수정
        ...
        private void SetMeasurementCombo()
        {
            ...
            DataTable dtBinding = dtResult.Select().AsEnumerable().OrderBy(o => o.GetValue("CMCDSEQ").GetDecimal()).CopyToDataTable();
        ...
        private void SelectRollMapTagSection()
        {
            try
            {
                ...
                {
                    DataSeries dataSeries = chartCoater.Data.Children[i];
                    if (dataSeries.Tag.GetString().Equals("TAG_SECTION") || dataSeries.Tag.GetString().Equals("TagSectionStart") || dataSeries.Tag.GetString().Equals("TagSectionEnd") || dataSeries.Tag.GetString().Equals("TAG_SECTION_SINGLE"))
                    {
                        chartCoater.Data.Children.Remove(dataSeries);
                    }
                }
        ...
        private void DrawGauge(DataTable dt)
        {
            try
            {
                ...
                //stopwatch.Start();
                //Debug.WriteLine("DrawGauge Start =======> : ");
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    //{
                    //    stopwatch.Stop();
                    //    Debug.WriteLine("DrawGauge PlotElementLoaded Time =======> : " + stopwatch.Elapsed.ToString());
                    //}
                }
            ...
            if (query.Any())
            {
                ...
                if (queryLength.Any())
                {
                    ...
                    foreach (var item in queryLength)
                    {
                        ...
                        newLength["SOURCE_END_PSTN"] = $"{item.EndPosition:###,###,###,##0.##}"; //(bool)rdoRelativeCoordinates.IsChecked ? $"{item.RawEndPosition:###,###,###,##0.##}" : 
        ...
        private void DrawDefectTagSectionSingle(DataTable dtTagSectionSingle)
        {
            if (CommonVerify.HasTableRow(dtTagSectionSingle))
            {
                dtTagSectionSingle = MakeTableForDisplay(dtTagSectionSingle, ChartDisplayType.TagSectionSingle);

                XYDataSeries ds = new XYDataSeries();
                ds.ItemsSource = DataTableConverter.Convert(dtTagSectionSingle);
                ds.XValueBinding = new Binding("ADJ_STRT_PSTN");
                ds.ValueBinding = new Binding("SOURCE_ADJ_Y_PSTN");
                ds.ChartType = ChartType.XYPlot;
                ds.ConnectionFill = new SolidColorBrush(Colors.Transparent);
                ...
                ds.PointLabelTemplate = grdMain.Resources["chartSectionTag"] as DataTemplate;
                ds.Margin = new Thickness(0);
                ds.Tag = dtTagSectionSingle.TableName;
        ...
        private void BeginUpdateChart()
        {
            foreach (C1Chart c1Chart in Util.FindVisualChildren<C1Chart>(grdMain))
            {
                ...
                if (c1Chart.Name == "chartCoater")
                {
                    ...
                    c1Chart.View.Margin = new Thickness(0, 0, 0, 3);
        ...
        {
            const string bizRuleName = "DA_PRD_SEL_RM_RPT_CT_TAG_SECTION";
        ...
        {
            const string bizRuleName = "DA_PRD_SEL_RM_RPT_OUTSIDE_SCRAP";
        ...
        private DataTable SelectTestCut_NonArea()
        {
            try
            {
                ...
                DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_RM_RPT_ADJ_TEST_CUT", "RQSTDT", "RSLTDT", dt);
        ...
        private void SetRollMapDefaultGauge()
        {
            ...
            try
            {
                ...
                foreach (C1ComboBox comboBox in Util.FindVisualChildren<C1ComboBox>(gbxMeasrPointcombo))
                {
                    if (CommonVerify.HasTableRow(dtResult))
                    {
        ...
        private void SaveTestCut(C1DataGrid dg)
        {
            try
            {
                ...
                ShowLoadingIndicator();
                new ClientProxy().ExecuteService("BR_PRD_REG_RM_ADJ_TEST_CUT", "IN_DATA", null, dtData, (result, bizException) =>
                {
                    try
                    {
                        HiddenLoadingIndicator();
        ...
        private bool IsCommoncodeUse(string codeType, string code)
        {
            try
            {
        ...
        //조성근 2025.02.11 조회팝업에서 조회만 되도록 수정
        private void tbTagSection_ContextMenuOpening(object sender, ContextMenuEventArgs e)
        {
            if (_isSearchMode)
            {
