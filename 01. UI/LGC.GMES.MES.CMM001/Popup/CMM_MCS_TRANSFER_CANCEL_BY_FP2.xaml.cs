...
--------------------------------------------------------------------------------------
 [Change History]
  2022.07.06  최도훈 : Initial Created.
**************************************************************************************/
using System;
...
{
    /// <summary>
    /// CMM_MCS_TRANSFER_CANCEL_BY_FP2.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class CMM_MCS_TRANSFER_CANCEL_BY_FP2 : C1Window, IWorkArea
    {
		...
		public CMM_MCS_TRANSFER_CANCEL_BY_FP2()
        {
            InitializeComponent();
        }
		...
		private void SetPortCombo(string bldgCode)
        {
            try
            {
				...
				else
				{
					...
					DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_MHS_SEL_STO_LIST_FOR_CANCEL", "RQSTDT", "RSLTDT", inTable);

					dtResult.Columns["DST_PORTID"].ColumnName = "PORT_ID";
					dtResult.Columns["DST_PORTNAME"].ColumnName = "PORT_NAME";
        ...
        private void SelectFirstManualTransferCancelList()
        {
            try
            {
                ...
                const string bizRuleName = "DA_MHS_SEL_TRF_CMD_FOR_UI_CANCEL";

                DataTable inDataTable = new DataTable("RQSTDT");
                inDataTable.Columns.Add("LANGID", typeof(string));
                inDataTable.Columns.Add("CMD_SEQNO", typeof(string));
                ...
                inDataTable.Columns.Add("SRC_EQPTID", typeof(string));
                inDataTable.Columns.Add("DST_PORTID", typeof(string));
                ...
                foreach (DataRow row in _dtTransferCancel.Rows)
                {
                    ...
                    newRow["CMD_SEQNO"] = row["CMD_SEQNO"].ToString();
                    newRow["CSTID"] = row["CSTID"].ToString();
                    newRow["FROM_DATE"] = systemDateTime.AddMonths(-1);
                    newRow["TO_DATE"] = systemDateTime;
                    //newRow["BLDG_CODE"] = _BldgCode;
                    newRow["SRC_EQPTID"] = string.IsNullOrEmpty(cboEquipment.SelectedValue.GetString()) ? null : cboEquipment.SelectedValue.GetString();
                    newRow["DST_PORTID"] = string.IsNullOrEmpty(cboPort.SelectedValue.GetString()) ? null : cboPort.SelectedValue.GetString();
                ...
                new ClientProxy().ExecuteService(bizRuleName, "RQSTDT", "RSLTDT", inDataTable, (result, ex) =>
                    {
                        ...
                        foreach (DataRow dr in result.Rows)
                        {
                            ...
                            dr["SEQ"] = iSeq++;
                            dr["REQ_TRF_STAT_NAME"] = ObjectDic.Instance.GetObjectName(dr["REQ_TRF_STAT_NAME"].ToString());
		...
		private void SelectManualTransferCancelList()
        {
            try
            {
                ...
                inData["CMD_SEQNO"] = null;
                inData["CSTID"] = !string.IsNullOrEmpty(txtCarrierId.Text.Trim()) ? txtCarrierId.Text : null;
                inData["FROM_DATE"] = dtpDateFrom.SelectedDateTime.Date;
                inData["TO_DATE"] = dtpDateTo.SelectedDateTime.Date.AddDays(1).AddSeconds(-1);
                ...
                inData["SRC_EQPTID"] = cboEquipment.SelectedValue;
                //inData["EQPT_TP"] = cboEquipmentType.SelectedValue;
                //inData["BLDG_CODE"] = _BldgCode;
                inData["DST_PORTID"] = string.IsNullOrEmpty(cboPort.SelectedValue.GetString()) ? null : cboPort.SelectedValue.GetString();
        ...
        private void SaveManualTransferCancel()
        {
            try
            {
                ...
                //DataTable requestTransferInfoTable = new DataTable("IN_REQ_TRF_INFO");
                ...
                new ClientProxy().ExecuteService_Multi(bizRuleName, "IN_REQ_TRF_INFO", null, (bizResult, bizException) =>
                    {
                        loadingIndicator.Visibility = Visibility.Collapsed;
                        try
                        {
        ...
        private DateTime GetSystemTime()
        {
            ...
            const string bizRuleName = "COR_SEL_GETDATE";
            DataTable inDataTable = new DataTable("INDATA");

            DataTable dtResult = new ClientProxy().ExecuteServiceSync(bizRuleName, null, "RSLTDT", null);
        ...
        private void SetCommonCombo(C1ComboBox cbo, string codeType)
        {
            try
            {
                const string bizRuleName = "COR_SEL_COMMONCODE_CBO";
        ...
        private void SetEquipmentCombo(C1ComboBox cbo, string equipmentType = null)
        {
            try
            {
                const string bizRuleName = "DA_MCS_SEL_FORMATION_STO_CBO";
