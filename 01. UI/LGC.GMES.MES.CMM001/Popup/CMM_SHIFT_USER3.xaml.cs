...
namespace LGC.GMES.MES.CMM001.Popup
{
    ...
    /// </summary>
    public partial class CMM_SHIFT_USER3 : C1Window, IWorkArea
    {
        ...
        public string WORKGROUID
        {
            get { return _RetWrkGrID; }
        }

        public string WORKGROUIDNAME
        {
            get { return _RetWrkGrName; }
        }
        #endregion
        ...
        public CMM_SHIFT_USER3()
        {
            InitializeComponent();
        }

        public CMM_SHIFT_USER3(TextBox target)
        {
            InitializeComponent();
            _target = target;
            ApplyPermissions();
        ...
        private void SetdgShiftUserGrid()
        {
            ...
            if (Util.NVC(_ShiftUserSearchFlag).Equals("Y"))
            {
                ...
                dgShiftUser.Columns["USERNAME_1"].Width = new C1.WPF.DataGrid.DataGridLength(246);
                dgShiftUser.Columns["USERNAME_1"].HorizontalAlignment = HorizontalAlignment.Left;

                dgShiftUser.Columns["USERNAME_2"].Width = new C1.WPF.DataGrid.DataGridLength(246);
        ...
        private void GetShiftUser(string wrkGrID)
        {
            ...
            {
                sBizName = "DA_BAS_SEL_TB_MMD_AREA_PROC_USER4_V01";
            }
            else
            {
                sBizName = "DA_BAS_SEL_TB_MMD_AREA_PROC_USER4";
            }
        ...
        private void GetWrkGr()
        {
            ...
            new ClientProxy().ExecuteService("DA_BAS_SEL_TB_MMD_AREA_PROC_GR_LIST_FM", "RQSTDT", "RSLTDT", IndataTable, (result, searchException) =>
            {
                try
                {
                    if (searchException != null)
                        throw searchException;
                    if (!result.Rows[0].ItemArray[5].Equals(""))
                    {
                ...
                for (int i = 0; i < ((C1.WPF.DataGrid.DataGridCellPresenter)rb.Parent).DataGrid.Rows.Count; i++)
                {
                    if (idx == i)
                    {
                        if (!DataTableConverter.GetValue(((C1.WPF.DataGrid.DataGridCellPresenter)rb.Parent).DataGrid.Rows[i].DataItem, "WRK_GR_NAME_" + idx2).Equals("") || DataTableConverter.GetValue(((C1.WPF.DataGrid.DataGridCellPresenter)rb.Parent).DataGrid.Rows[i].DataItem, "WRK_GR_NAME_" + idx2).Equals(ObjectDic.Instance.GetObjectName("ALL")))
                        {
                            ...
                            _RetWrkGrID = (rb.DataContext as DataRowView).Row["WRK_GR_ID_" + idx2].ToString();
                            _RetWrkGrName = (rb.DataContext as DataRowView).Row["WRK_GR_NAME_" + idx2].ToString();
                        }
                        else
                        {
