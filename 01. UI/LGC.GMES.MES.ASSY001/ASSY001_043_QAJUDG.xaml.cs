...
{
    public partial class ASSY001_043_QAJUDG : C1Window, IWorkArea
    {
        ...
        bool route_change_yn = false;   //2019-05-21 Add By Sejin Yoon : 라우트 변경 여부 Flag // git merge test retry
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (tmps != null && tmps.Length >= 1)
                {
                    ...
                    route_change_yn = (bool)tmps[8]; //2019-05-21 윤세진 추가
        ...
        private void VDReworkLot(string lotid)
        {
            try
            {
                ...
                if (gbLotQAInsp == false)   //2017.08.13 Edited By Kim Joonphil
                {
                    ...
                    result = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_VD_REWORK_LOT", "RQST", "RSLT", dt); //해당LOT과 같은 배치ID를 가진 LOT조회 (VD재작업 대상)
                ...
                //2019-05-21 route_change_yn 에 따라 재작업 호출 비즈 달라짐.
                if(route_change_yn)
                    new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_MOVE_LOT_WAIT_VD_PANCAKE", "INDATA, IN_LOT", null, ds);
                else
        ...
        private void SaveQaJudg()
        {
            ...
            for (int i = 0; i < cboLotID.SelectedItems.Count; i++)
            {
                ...
                row["QA_INSP_COND_LOT_CHECK"] = gbLotQAInsp ? "Y" : "N";    //2017.08.13 Add By Kim Joonphil
            ...
            new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_QAJUDGE_VDQA_VD_PANCAKE", "INDATA,IN_LOT, INRSEN", null, inData);
