...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2023.02.28  윤지해 : C20230103-000554 - GMES V/D QA 대상 LOT 조회 화면에서 자동/수동검사 LOT 색깔구분 기능추가 요청의 건(A1, A2 오창 자동차 조립만 적용)
**************************************************************************************/
...
{
    public partial class ASSY001_024_EQPTWIN : UserControl, IWorkArea
    {
        ...
        private void btnInspectionConfirm_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (wndQA != null)
            {
                ...
                object[] Parameters = new object[8]; //2017.08.13 Edited By Kim Joonphil
        ...
        public static class GridBackColor
        {
            ...
            #region 2023.02.28 C20230103-000554 - GMES V/D QA 대상 LOT 조회 화면에서 자동/수동검사 LOT 색깔구분 기능추가 요청의 건
            public static readonly System.Drawing.Color autoQa = System.Drawing.Color.FromArgb(255, 255, 0);  // 자동검사(24시간 검사)
            public static readonly System.Drawing.Color manQa = System.Drawing.Color.FromArgb(155, 200, 205);    // 수동검사(대LOT검사)
            #endregion
        }
            ...
            dataGrid.Dispatcher.BeginInvoke(new Action(() =>
            {
                try
                {
                    if (e.Cell.Column.Name.Equals("JUDG_NAME"))
                    {
                        ...
                        else if (judgValue.Equals("FAIL") || judgValue.Equals("DF"))
                        {
                    ...
                    #region C20230103-000554 - GMES V/D QA 대상 LOT 조회 화면에서 자동/수동검사 LOT 색깔구분 기능추가 요청의 건
                    if (LoginInfo.CFG_AREA_ID == "A1" || LoginInfo.CFG_AREA_ID == "A2")
                    {
                        if (e.Cell.Column.Name.Equals("PRJT_NAME"))
                        {
                            string qaFlagTypeValue = DataTableConverter.GetValue(dgRunLot.Rows[e.Cell.Row.Index].DataItem, "QA_FLAG_TYPE").GetString();

                            // YELLOW   = 자동검사 (24시간검사)
                            // BLUE     = 수동검사 (대LOT검사)
                            if (qaFlagTypeValue.Equals("YELLOW"))
                            {
                                System.Drawing.Color colorY = GridBackColor.autoQa;
                                e.Cell.Presenter.Background = new SolidColorBrush(System.Windows.Media.Color.FromArgb(colorY.A, colorY.R, colorY.G, colorY.B));
                                e.Cell.Presenter.Foreground = new SolidColorBrush((Color)System.Windows.Media.ColorConverter.ConvertFromString("black"));
                            }
                            else
                            {
                                System.Drawing.Color colorB = GridBackColor.manQa;
                                e.Cell.Presenter.Background = new SolidColorBrush(System.Windows.Media.Color.FromArgb(colorB.A, colorB.R, colorB.G, colorB.B));
                                e.Cell.Presenter.Foreground = new SolidColorBrush((Color)System.Windows.Media.ColorConverter.ConvertFromString("black"));
                            }
                        }
                    }
        ...
        private void btnConfirm_Click(object sender, RoutedEventArgs e)
        {
            ...
            LGC.GMES.MES.ControlsLibrary.MessageBox.Show(MessageDic.Instance.GetMessage("SFU3368"), null, "Information", MessageBoxButton.OKCancel, MessageBoxIcon.None, (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    ...
                    new ClientProxy().ExecuteService_Multi("BR_RPD_REG_QA_CONFIRM_FOR_VD", "INDATA, IN_LOT", null, (bizResult, bizException) =>
                    {
                        try
                        {
