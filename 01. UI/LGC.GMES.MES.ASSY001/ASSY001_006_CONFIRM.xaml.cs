/*************************************************************************************
 Created Date : 2016.08.19
      Creator : INS 김동일K
   Decription : 전지 5MEGA-GMES 구축 - Stacking 공정진척 화면 - 실적 확인 팝업
--------------------------------------------------------------------------------------
 [Change History]
  2016.08.19  INS 김동일K : Initial Created.
...
{
    /// <summary>
    /// ASSY001_006_CONFIRM.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY001_006_CONFIRM : C1Window, IWorkArea
    {
            ...
            try
            {
                ...
                for (int j = 0; j < dgDefect.Rows.Count - dgDefect.BottomRows.Count; j++)
                {
                    ...
                    if (dRate > 0)
                    {
                        ...
                        //dAllowQty = Math.Truncate( Convert.ToDouble(Convert.ToDecimal(goodQty) * dRate / 100)); //버림   
        ...
        private void dgDefect_CommittedEdit(object sender, C1.WPF.DataGrid.DataGridCellEventArgs e)
        {
            ...
            {                
                ...
                else if (e.Cell.Column.Name.Equals("REG_F"))
                {
                    ...
                    DataTableConverter.SetValue(dgDefect.Rows[e.Cell.Row.Index].DataItem, "TEMP_F", sReg_FC);
                }
        ...
        #region [BizCall]
        private void GetStackingLotInfo()
        {
            try
            {
        ...
        private void GetBox()
        {
            try
            {
                ...
                DataTable dtRslt = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_OUT_LOT_LIST_ST", "INDATA", "OUTDATA", inTable);
                    ...
                    try
                    {
                        ...
                        else
                        {
                            for (int i = 0; i < searchResult.Rows.Count; i++)
                            {
                                ...
                                {
                                    txtInHalfType.Text = double.Parse(Util.NVC(searchResult.Rows[i]["PROC_INPUT_CNT"])).ToString();                                    
                                }
                                else if (Util.NVC(searchResult.Rows[i]["PRODUCT_LEVEL2_CODE"]).Equals("MC"))
                                {
                                    txtInMonoType.Text = double.Parse(Util.NVC(searchResult.Rows[i]["PROC_INPUT_CNT"])).ToString();                                    
                                }
                            }

                            if (txtInHalfType.Text.Equals(""))
                            {
                                //Util.Alert("HALF TYPE 불량 기준정보가 존재하지 않습니다.");
                                Util.MessageValidation("SFU1337");
                                return;
                            }

                            if (txtInMonoType.Text.Equals(""))
                            {
                                //Util.Alert("MONO TYPE 불량 기준정보가 존재하지 않습니다.");
                                Util.MessageValidation("SFU1401");
                                return;
                            }

                            gDfctAQty = double.Parse(txtInHalfType.Text);
                            gDfctCQty = double.Parse(txtInMonoType.Text);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void Save()
        {
            try
            {
                ...
                DataSet indataSet = _Biz.GetBR_PRD_REG_CONFIRM_LOT_ST();
        ...
        private void SumDefectQty()
        {
            try
            {
                ...
                //for (int icnt = 0; icnt < dgDefect.GetRowCount(); icnt++)
                //{
                //    double Eqp_qty = double.Parse(Util.NVC(DataTableConverter.GetValue(dgDefect.Rows[icnt].DataItem, "EQP_DFCT_QTY")));
                //    double Calc_F = double.Parse(Util.NVC(DataTableConverter.GetValue(dgDefect.Rows[icnt].DataItem, "CALC_F")));
                //    double Sum = 0;
                ...
                //        DataTableConverter.SetValue(dgDefect.Rows[icnt].DataItem, "CALC_A", Sum * gDfctAQty);
                //        DataTableConverter.SetValue(dgDefect.Rows[icnt].DataItem, "CALC_C", Sum * gDfctCQty);
