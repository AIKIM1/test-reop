...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2023.12.12  안유수 E20231205-000745 작업 시작 시, 선택 LOTID없는 자재 유형 타입 'MTRL', 'PROD_MTRL'의 경우, 투입 LOTID로 대체
**************************************************************************************/
...
{
    /// <summary>
    /// ASSY001_007_RUNSTART.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY001_007_RUNSTART : C1Window, IWorkArea
    {
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (dgWaitList.Columns.Contains("CSTID"))
                    dgWaitList.Columns["CSTID"].Visibility = Visibility.Visible;

                if (dgInList.Columns.Contains("CSTID"))
                    dgInList.Columns["CSTID"].Visibility = Visibility.Visible;
            }
                            ...
                            case "CHK":
                                   ...
                                   !(bool)(dg.GetCell(e.Cell.Row.Index, e.Cell.Column.Index).Presenter.Content as CheckBox).IsChecked)
                                {
                                    ...
                                    SetInput(Util.NVC(DataTableConverter.GetValue(dg.Rows[e.Cell.Row.Index].DataItem, "LOTID")),
                                                ...
                                                Util.NVC(DataTableConverter.GetValue(dg.Rows[e.Cell.Row.Index].DataItem, "CSTID")));
                                         ...
                                         (bool)(dg.GetCell(e.Cell.Row.Index, e.Cell.Column.Index).Presenter.Content as CheckBox).IsChecked)
                                {
                                    ...
                                    for (int i = 0; i < dgInList.Rows.Count - dgInList.BottomRows.Count; i++)
                                    {
                                            ...
                                            Util.NVC(DataTableConverter.GetValue(dg.Rows[e.Cell.Row.Index].DataItem, "LOTID")).Equals(Util.NVC(DataTableConverter.GetValue(dgInList.Rows[i].DataItem, "SEL_LOTID"))))
                                        {
                                            ...
                                            if (dgInList.Columns.Contains("CSTID"))
                                                DataTableConverter.SetValue(dgInList.Rows[i].DataItem, "CSTID", "");
                                            break;
                                        }
                                    }
                                }
        ...
        private string GetNewLotid()
        {
            try
            {
                ...
                DataSet dsRslt = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_NEW_PROD_LOTID_CL", "IN_EQP,IN_INPUT", "OUTDATA", indataSet);
        ...
        private void RunStart(string sNewLot)
        {
            try
            {
                ...
                for (int i = 0; i < dgInList.Rows.Count - dgInList.BottomRows.Count; i++)
                {
                    ...
                    {
                        if (((Util.NVC(DataTableConverter.GetValue(dgInList.Rows[i].DataItem, "MOUNT_MTRL_TYPE_CODE")) == "MTRL") ||
                            (Util.NVC(DataTableConverter.GetValue(dgInList.Rows[i].DataItem, "MOUNT_MTRL_TYPE_CODE")) == "PROD_MTRL")) &&
                            (Util.NVC(DataTableConverter.GetValue(dgInList.Rows[i].DataItem, "INPUT_LOTID")) != ""))
                        {
                            newRow = inMtrlTable.NewRow();
                ...
                DataSet dsRslt = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_START_LOT_CL", "IN_EQP,IN_INPUT", null, indataSet);
            ...
            try
            {
                ...
                new ClientProxy().ExecuteService("DA_PRD_SEL_EQPT_MOUNT_INFO", "INDATA", "OUTDATA", inTable, (searchResult, searchException) =>
                {
                    try
                    {
                        ...
                        Util.GridSetData(dgInList, searchResult, null, false);
                        dgInList.CurrentCellChanged += dgInList_CurrentCellChanged;
        ...
        private void SetInput(string sLot, string sQty, string sProdid, string sCstID)
        {
            ...
            if (Util.NVC(DataTableConverter.GetValue(dgInList.Rows[iRow].DataItem, "MOUNT_MTRL_TYPE_CODE")) != "PROD")
            {
                return;
            }
            ...
            if (!dtTmp.Columns.Contains("CSTID"))
        ...
        private void SetInputKeyIn(string sLot, string sQty, string sProdid, string sCstID)
        {
            if (dgInList.ItemsSource == null || dgInList.Rows.Count <= 0)
                return;
