/*************************************************************************************
 Created Date : 2016.08.18
      Creator : INS 김동일K
   Decription : 전지 5MEGA-GMES 구축 - Stacking 공정진척 화면 - 착공 팝업
--------------------------------------------------------------------------------------
 [Change History]
...
{
    /// <summary>
    /// ASSY001_006_RUNSTART.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY001_006_RUNSTART : C1Window, IWorkArea
    {
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            GetWaitMazList(dgAtype, "HC");
            GetWaitMazList(dgCType, "MC");
        ...
        private void dgAtype_CurrentCellChanged(object sender, C1.WPF.DataGrid.DataGridCellEventArgs e)
        {
            this.Dispatcher.BeginInvoke(new Action(() =>
            {
                C1DataGrid dg = sender as C1DataGrid;
        ...
        private void GetInputMountInfo()
        {
            try
            {
                ...
                new ClientProxy().ExecuteService("DA_PRD_SEL_EQPT_MOUNT_INFO_ST", "INDATA", "OUTDATA", inTable, (searchResult, searchException) =>
                {
                    try
                    {
        ...
        private void GetWaitMazList(C1.WPF.DataGrid.C1DataGrid datagrid, string sType)
        {
            try
            {
                ...
                DataTable inTable = _Biz.GetDA_PRD_SEL_WAIT_LOT_LIST_ST();
                ...
                newRow["PRODUCT_LEVEL2_CODE"] = sType;
                ...
                new ClientProxy().ExecuteService("DA_PRD_SEL_WAIT_MAG_ST", "INDATA", "OUTDATA", inTable, (bizResult, ex) =>
                {
                    try
                    {
                        if (ex != null)
        ...
        private void RunStart(string sNewLot)
        {
            try
            {
                ...
                DataSet indataSet = _Biz.GetBR_PRD_REG_LOTSTART_ST();
        ...
        private bool MagazineValidation()
        {
            bool bRet = false;

            DataTable searchConditionTable = new DataTable();
            searchConditionTable.Columns.Add("LANGID", typeof(string));
            searchConditionTable.Columns.Add("", typeof(string));
            searchConditionTable.Columns.Add("", typeof(string));

            DataRow searchCondition = null;

            for (int i = 0; i < dgInput.Rows.Count; i++)
            {

                if (Util.NVC(DataTableConverter.GetValue(dgInput.Rows[i], "EQPT_MOUNT_PSTN_ID")).Trim().Equals(""))
                {
                    //Util.Alert("위치가 입력되지 않았습니다.");
                    Util.MessageValidation("SFU1234");
                    return bRet;
                }   

                searchCondition = searchConditionTable.NewRow();
                ...
                DataTable dtResult = new ClientProxy().ExecuteServiceSync("", "INDATA", "OUTDATA", searchConditionTable);

                if (dtResult.Rows.Count == 0)
                {
                    //Util.Alert("매거진 확인 중 이상이 발생 하였습니다.");
                    Util.MessageValidation("SFU1224");
                    return bRet;
                }
                else
                {
                    string validRslt = DataTableConverter.GetValue(dtResult, "P_RET").ToString();
                    string validMsg = DataTableConverter.GetValue(dtResult, "P_MSG").ToString();
                    if (validRslt != "OK")
                    {
                        //Util.Alert("매거진 확인 중 이상이 발생 하였습니다.");
                        Util.MessageValidation("SFU1224");
