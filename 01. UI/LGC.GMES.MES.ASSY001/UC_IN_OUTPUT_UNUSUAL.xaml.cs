...
{
    /// <summary>
    /// UC_IN_OUTPUT_UNUSUAL.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class UC_IN_OUTPUT_UNUSUAL : UserControl, IWorkArea
    {
        ...
        public UC_IN_OUTPUT_UNUSUAL(string sProcID, string sEqsgID, string sEqptID, bool bStacking = false)
        {
            PROCID = sProcID;
            EQPTSEGMENT = sEqsgID;
            EQPTID = sEqptID;
        ...
        private void btnCurrInReplace_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                ASSY001_008_PAN_REPLACE wndReplace = new ASSY001_008_PAN_REPLACE();
        ...
        {
            ASSY001_008_PAN_REPLACE window = sender as ASSY001_008_PAN_REPLACE;
        ...
        private void btnWaitMagRework_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                ASSY001_008_MAGAZINE_CREATE wndMAZCreate = new ASSY001_008_MAGAZINE_CREATE();
                ...
                if (wndMAZCreate != null)
                {
                    ...
                    Parameters[2] = string.IsNullOrWhiteSpace(sWodetlid) ? null : sWodetlid;
                    Parameters[3] = PROCID;
        ...
        {
            ASSY001_008_MAGAZINE_CREATE window = sender as ASSY001_008_MAGAZINE_CREATE;
            if (window.DialogResult == MessageBoxResult.OK)
            {
        ...
        public void GetWaitMagazine(string sLot = "")
        {
            try
            {
                ...
                else if (PROCID.Equals(Process.STP))
                {
                    ...
                    if (cboSizeSTP != null && !Util.NVC(cboSizeSTP.SelectedValue).Equals(""))
                        newRow["PRDT_SIZE"] = cboSizeSTP.SelectedValue.ToString();

                    if (cboDirectionSTP != null && !Util.NVC(cboDirectionSTP.SelectedValue).Equals(""))
        ...
        private void BasketInput(bool bAuto, int iRow)
        {
            try
            {
                ...
                new ClientProxy().ExecuteService_Multi("BR_PRD_REG_START_INPUT_LOT_SSC_FD", "IN_EQP,IN_INPUT", null, (searchResult, searchException) =>
                {
                    try
                    {
                        if (searchException != null)
