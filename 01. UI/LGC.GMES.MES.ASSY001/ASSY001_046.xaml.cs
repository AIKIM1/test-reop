...
namespace LGC.GMES.MES.ASSY001
{
    ...
    /// </summary>
    public partial class ASSY001_046 : UserControl, IWorkArea
    {
        ...
        private void ComboInitialize()
        {
            ...
            //공정
            String[] sFilter = { "A7000", Process.LAMINATION, Process.STACKING_FOLDING };
            C1ComboBox[] cboProcessParent = { cboEquipmentSegment };
            _combo.SetCombo(cboProcess, CommonCombo.ComboStatus.NONE, cbParent: cboProcessParent, sFilter: sFilter);
        ...
        private void SearchData()
        {
            try
            {
                ...
                //DataTable result = SummaryData(new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_EQPT_END_QTY_SUM", "RQSTDT", "RSLTDT", dt));
                DataTable result = SummaryData(new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_EQPT_END_QTY_SUM", "RQSTDT", "RSLTDT", dt));
        ...
        private DataTable SummaryData(DataTable srcDt)
        {
            ...
            if (dEltrColInfo.Keys.Count > 1)
            {
                ...
                {
                    newDt.Columns.Add("LEFT_" + dEqptColInfo[dEqpt], typeof(Int32) ); // C20201112-000360 데이터 타입 지정 
                    newDt.Columns.Add("RIGHT_" + dEqptColInfo[dEqpt], typeof(Int32) ); // C20201112-000360 데이터 타입 지정 
                }
            }
            else
            {
            ...
            foreach (string wTime in wrkDttm.Keys)
            {
                ...
                foreach (DataRow dr in tempdt.Rows)
                {
                    ...
                    {
                        newdr["LEFT_" + dr["EQPTNAME"].ToString()] = dr["HOURLY_PLAN_QTY"];
                        newdr["RIGHT_" + dr["EQPTNAME"].ToString()] = dr["SUM_END_QTY"];
                    }
                    else
                    {
                        newdr["LEFT_" + dr["EQPTNAME"].ToString().Substring(0, dr["EQPTNAME"].ToString().Length - 2)] = dr["HOURLY_PLAN_QTY"];
                        newdr["RIGHT_" + dr["EQPTNAME"].ToString().Substring(0, dr["EQPTNAME"].ToString().Length - 2)] = dr["SUM_END_QTY"];
                    }
                }
