...
{
    public partial class ASSY001_024 : UserControl, IWorkArea
    {
        ...
        private void SearchData()
        {
            ...
            #region 2023.02.28  C20230103-000554 - GMES V/D QA 대상 LOT 조회 화면에서 자동/수동검사 LOT 색깔구분 기능추가 요청의 건(A1, A2 오창 자동차 조립만 적용)
            if (LoginInfo.CFG_AREA_ID == "A1" || LoginInfo.CFG_AREA_ID == "A2")
            {
                qaColor.Visibility = Visibility.Visible;
            }
            #endregion

            #region [Lot ID별로 각각 QA검사 선택 하기 위한 CheckBox, 2017.08.13 Add By Kim Joonphil]
            if (rdoFinish.IsChecked == true && LoginInfo.CFG_AREA_ID == "A5")
            {
                ...
                new ClientProxy().ExecuteService("DA_PRD_SEL_EQPT_VD", "RQST", "RSLT", data, (bizResult, bizException) =>
                {
                    ...
                    for (int i = 0; i < bizResult.Rows.Count; i++)
                    {
                        ...
                        if (rdoRun.IsChecked == true)
                        {
                            ...
                            {
                                list[i].dgcCstId_R.Visibility = Visibility.Visible;
                            }
                            else
                            {
                                list[i].dgcCstId_R.Visibility = Visibility.Collapsed;
                            }
                        ...
                        else if (rdoFinish.IsChecked == true)
                        {
                            ...
                            if (_UNLDR_LOT_IDENT_BAS_CODE.Equals("RF_ID"))
                            {
                                list[i].dgcCstId_F.Visibility = Visibility.Visible;
                            }
                            else
                            {
                                list[i].dgcCstId_F.Visibility = Visibility.Collapsed;
                            }
        ...
        private DataTable SetVDFinish()
        {
            try
            {
                ...
                DataSet ds = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_VD_FINISH", "RQSTDT", "OUTDATA", data);
        ...
        private DataTable SetVDoperation()
        {
            try
            {
                ...
                DataSet ds = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_VD_OPERATION", "INDATA", "OUTDATA", data);
                DataTable result = ds.Tables["OUTDATA"];
