...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2017.03.08  신광희 : USB 포트 연결 라벨 프린터 출력 가능하도록 수정 함.
  ...
  2023.12.04  박승렬 : E20231103-001744 ESMI 노칭공정진척 화면에서의 시작취소, 장비완료취소 버튼 권한별 표시 
  2023.12.18  박승렬 : E20231103-001744 ESMI 동별공통코드 설정을 안하면 버튼 Visible 표시 
  2024.01.23  남기운 : 허용비율 초과시 사유 등록 프로세서 추가
  2024.02.14  김용군 : E20240221-000898 ESMI1동(A4) 6Line증설관련 화면별 라인ID 콤보정보에 조회될 Line정보와 제외될 Line정보 처리
  개발 테스트
  2024.03.18  남기운 : E20240318-000164  초과수량 계산식 변경(올림)
**************************************************************************************/
...
namespace LGC.GMES.MES.ASSY001
{
    ...
    /// </summary>
    public partial class ASSY001_001 : UserControl, IWorkArea
    {
        ...
        private bool bWndCnfMsgJobEnd = true;   //로직이 중복으로 타는 현상 발생해 방지하기 위해        
        ...
        bool _bRealWorkerSelFlag = false;
        ...
        private UcAssyToolinfo winInputHistTool = new UcAssyToolinfo();
        ...
        string _sPGM_ID = "ASSY001_001";

        //2019-06-05 최상민 timer 품질 체크
        ...
        private void UserControl_Initialized(object sender, EventArgs e)
        {
            ...
            // 품질 정보 입력 여부 체크 안되어 있을 시 하단 Area Animation 출력
        ...
        private void dispatcherTimer_QC_Tick(object sender, EventArgs e)
        {
            ...
            dpcTmr.Dispatcher.BeginInvoke(new Action(() =>
            {
                ...
                try
                {
                    ...
                    //if (dgProductLot == null)
                    {
                        GetTestMode();
                        return;
                    }

                    string _OutLotID = Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[_Util.GetDataGridCheckFirstRowIndex(dgProductLot, "CHK")].DataItem, "LOTID"));

                    // 2019-06-26 건당 확인시 기능에서 조별 확인 기능으로 변경
                    //if (!CheckInputQCData())                     
                    if (Util.EDCCondition(LoginInfo.CFG_AREA_ID, Process.NOTCHING, Util.NVC(cboEquipmentSegment.SelectedValue), Util.NVC(cboEquipment.SelectedValue), _OutLotID))
                    {
                        // 품질 정보 입력 여부 Notice..
                        //string _LotidList2 = _LotidList;
                        //ShowBottomMsgArea("LotID : "+_LotidList2 + "  품질 정보를 입력하세요.", BOTTOM_MSG_TYPE.QC_NO_INPUT_ALARM);
                        ShowBottomMsgArea("품질 정보 입력이 안되었습니다.", BOTTOM_MSG_TYPE.QC_NO_INPUT_ALARM);
                    }
                    else
                    {                        
                        //if (!bTestMode)
        ...
        private bool CheckInputQCData()
        {
            try
            {
                ...
                //string bEqp_end_chk = string.Empty;
                ...
                for (int i = 0; i < dgProductLot.GetRowCount()+2; i++)
                {
                    //string bEqp_end_chk = Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[i].DataItem, "EQPT_END_CHK"));
                    //string bEqp_end_chk = Util.NVC(dgProductLot.GetCell(i, dgProductLot.Columns["EQPT_END_CHK"].Index).Value);
                    string bEqp_wip_chk = Util.NVC(dgProductLot.GetCell(i, dgProductLot.Columns["WIPSTAT"].Index).Value);

                    if (bEqp_wip_chk.Equals("PROC") || bEqp_wip_chk.Equals("EQPT_END"))
                    {
                        ...
                        newRow["LOTID"] = Util.NVC(dgProductLot.GetCell(i, dgProductLot.Columns["LOTID"].Index).Value); 
                        newRow["WIPSEQ"] = Util.NVC(dgProductLot.GetCell(i, dgProductLot.Columns["WIPSEQ"].Index).Value); 
                        newRow["EQPTID"] = Util.NVC(cboEquipment.SelectedValue);

                        inTable.Rows.Add(newRow);

                        DataTable dtRslt = new ClientProxy().ExecuteServiceSync("DA_QCA_SEL_NCT_INSP_DATA_INPUT_CHK", "INDATA", "OUTDATA", inTable);                        
                        ...
                        }else
                        {
                            ...
                            if (_LotidList == "")
                            {
                                _LotidList = Util.NVC(dgProductLot.GetCell(i, dgProductLot.Columns["LOTID"].Index).Value);
                            }else
                            {
                                _LotidList = _LotidList+", "+ Util.NVC(dgProductLot.GetCell(i, dgProductLot.Columns["LOTID"].Index).Value);
                            }                            
                        }
        ...
        private void recTestMode_MouseWheel(object sender, MouseEventArgs e)
        {
            try
            {
        ...
        private bool CheckUseQCAlarm()
        {
            try
            {
                ...
                DataRow newRow = inTable.NewRow();
                newRow["AREAID"] = LoginInfo.CFG_AREA_ID;
                newRow["COM_TYPE_CODE"] = "INPUT_QUALITY_INFO_INPUT_CK";
                //newRow["COM_CODE"] = "CHK_FLAG";
                newRow["COM_CODE"] = Util.NVC(cboEquipment.SelectedValue);
        ...
        // 2023.12.04 박승렬 E20231103-001744 권한별 버튼표시 적용
        private bool ChkbtnRunCancelPermissionSetYN() // 동별공통코드 확인 (시작취소 버튼)
        {
            try
            {
                DataTable inTable = new DataTable();
        ...
        // 2023.12.04 박승렬 E20231103-001744 권한별 버튼표시 적용
        private bool ChkbtnRunCompleteCancelPermissionSetYN() // 동별공통코드 확인 (장비완료 취소 버튼)
        {
            try
            {
                DataTable inTable = new DataTable();
                ...
                dtRow["AREAID"] = LoginInfo.CFG_AREA_ID;
                dtRow["ATTR2"] = this.GetType().Name;
                dtRow["ATTR3"] = "RunCompleteCancel_W";
                dtRow["USE_FLAG"] = "Y";
        ...
        private void GetButtonPermissionGroup()
        {
            try
            {
                ...
                // Read 권한일 때 btnRunCancel , btnRunCompleteCancel  숨김
                ...
                if (ChkbtnRunCancelPermissionSetYN())  //  시작취소 버튼 권한 설정
                {
                    DataTable inTable = new DataTable();
                ...
                {
                    btnRunCancel.Visibility = Visibility.Visible; // 동별 공통코드에 설정을 안했다면 Visible 처리 
                }


                if (ChkbtnRunCompleteCancelPermissionSetYN())  //  장비완료취소 버튼 권한 설정
                {
                    DataTable inTable = new DataTable();
                    inTable.Columns.Add("USERID", typeof(string));
                    inTable.Columns.Add("AREAID", typeof(string));
                    ...
                    if (dtRslt != null && dtRslt.Rows.Count > 0)
                    {
                        if (dtRslt.Columns.Contains("BTN_PMS_GRP_CODE"))
                        {
                            foreach (DataRow drTmp in dtRslt.Rows)
                            {
                                ...
                                switch (Util.NVC(drTmp["BTN_PMS_GRP_CODE"]))
                                {
                                    case "RunCompleteCancel_W": // 장비완료취소 버튼 권한
                ...
                {
                    btnRunCompleteCancel.Visibility = Visibility.Visible; // 동별 공통코드에 설정을 안했다면 Visible 처리
                }
        ...
        private void btnSearch_Click(object sender, RoutedEventArgs e)
        {
            ...
            dispatcherTimer_QC_Tick(this.dispatcherTimer_QC, null);
        }
        ...
        private void btnRunStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (iRow < 0)
            {
                ASSY001_001_RUNSTART wndRun = new ASSY001_001_RUNSTART();
                    ...
                    Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[iRow].DataItem, "WIP_TYPE_CODE")).Equals("IN")))
                {
                    ...
                    if (_UNLDR_LOT_IDENT_BAS_CODE == "RF_ID")
                    {
                        ...
                        if (RF_ID_Run != null)
                        {
                            ...
                            Parameters[4] = Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[iRow].DataItem, "PR_LOTID"));
        ...
        private void BR_PRD_REG_PERMIT_RATE_OVER_HIST()
        {
            try
            {
                ...
                for (int j = 0; j < lotTab.Rows.Count;j++)
                {
                    ...
                    //indataSet.Tables["INDATA"].Rows.Clear();
                    //indataSet.Tables["IN_RESN"].Rows.Clear();
        ...
        private void popRermitRate_Closed(object sender, EventArgs e)
        {
            ...
            if (popRermitRate != null && popRermitRate.DialogResult == MessageBoxResult.OK)
            {
                ...
                //////////////////////////////
                //확정 처리
                ConfirmProcess_Rate(_bRealWorkerSelFlag);
                /////////////////////////////////////
                //Rate저장 Biz 호출
        ...
        private void btnConfirm_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string _Eqp_End_Check = Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[_Util.GetDataGridCheckFirstRowIndex(dgProductLot, "CHK")].DataItem, "EQPT_END_CHK"));
                int _EQPT_END_NUM = int.Parse(Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[_Util.GetDataGridCheckFirstRowIndex(dgProductLot, "CHK")].DataItem, "EQPT_END_NUM")));

                if (_Eqp_End_Check.Equals("Y") && _EQPT_END_NUM > 1)
                {
                    Util.MessageValidation("SFU7007");
                    return;
                }
        ...
        private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            ASSY001_001_HIST wndPrint = new ASSY001_001_HIST();
        ...
        {
            ASSY001_001_WAITLOT wndWait = new ASSY001_001_WAITLOT();
      ...
      /*  private void dgProductLotChoice_Checked(object sender, RoutedEventArgs e)
        ...
        private void txtInputQty_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.Key == Key.Enter)
                {
                    #region 생산수량 합산 수량 Sum 처리.
                    double increase_prod_qty = 0;

                    double.TryParse(Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[dgDetail.TopRows.Count].DataItem, "PROD_QTY_INCR_QTY")), out increase_prod_qty);
                    
                    txtInputQty.Text = (double.Parse(txtInputQty.Text) + increase_prod_qty).ToString();
                    #endregion
        ...
        private void dgFaulty_CommittedEdit(object sender, C1.WPF.DataGrid.DataGridCellEventArgs e)
        {
            ...
            double dfct, loss, charge_prd, increase_prod_qty;

            double sum = SumDefectQty(dgFaulty, e.Cell.Column.Name, out dfct, out loss, out charge_prd, out increase_prod_qty);
            ...
            if (!_CurrentLotInfo.STATUS.Equals("WAIT"))
            {
                ...
                for (int i = dgDetail.TopRows.Count; i < dgDetail.Rows.Count; i++)
                {
                    ...
                    {
                        inputqty = double.Parse(Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[i].DataItem, "INPUTQTY_ORG")));
                        iRow = i;
                        break;
                    }
                }
                ...
                //if ((inputqty + increase_prod_qty) < totSum)
                int idx = _Util.GetDataGridCheckFirstRowIndex(dgProductLot, "CHK");
                double WipQty = 0;
                if (idx >= 0)
                    WipQty = double.Parse(Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[idx].DataItem, "WIPQTY_EA")));

                if (WipQty < totSum)
                {
                    Util.MessageValidation("SFU1601");
                    //Util.Alert("재공수량보다 불량이 크게 입력 될 수 없습니다.");

                    //Util.Alert("생산량보다 불량이 크게 입력 될 수 없습니다.");

                    DataTableConverter.SetValue(e.Cell.Row.DataItem, e.Cell.Column.Name, 0);

                    sum = SumDefectQty(dgFaulty, e.Cell.Column.Name, out dfct, out loss, out charge_prd, out increase_prod_qty);
                    totSum = sum;
                    ...
                    DataTableConverter.SetValue(dgDetail.Rows[iRow].DataItem, "PROD_QTY_INCR_QTY", increase_prod_qty);
                    //DataTableConverter.SetValue(dgDetail.Rows[iRow].DataItem, "INPUTQTY", inputqty + increase_prod_qty);

                    SetLotInfoCalc();

                    // 생산수량 증가 처리.
                    txtInputQty.Text = (inputqty + increase_prod_qty).ToString();
                    SetInputQty();
                    return;
                }

                // 재공 수량 과 설비생산량 중 큰쪽으로 Validation 처리 요청에 의한 변경(이병렬 책임)
                double iParentWipQty = 0;

                double.TryParse(txtParentQty.Text, out iParentWipQty);

                double value = iParentWipQty > inputqty ? iParentWipQty : inputqty;
                
                double sEqpPQty = inputqty;// double.Parse(Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[sChkIdx].DataItem, "EQPT_INPUT_QTY"))); // 설비생산량
                

                if ((inputqty + increase_prod_qty) > value)
                {
                    DataTableConverter.SetValue(e.Cell.Row.DataItem, e.Cell.Column.Name, 0);
                    ...
                    string sMsgID = "";
                    //길이 초과 등록 조건 카피.. (길이초과는 수량 초과메시지 처리 안하기 위함)
                    if ((inputqty + increase_prod_qty) > iParentWipQty && (inputqty + increase_prod_qty) <= sEqpPQty)
                    {
                    }
                    else
                    {
                        Util.MessageValidation("SFU8110");
                    }
        ...
        public void GetProductLot(bool bSelPrv = true)
        {
            try
            {
                ...
                // 2023.04.27 남재현 RFID 사용 및 투입 CSTID 컬럼 추가로 인해 바뀐 DA 수행을 위한 IF문 작성
                if (_LDR_LOT_IDENT_BAS_CODE.Equals("RF_ID"))
                {

                    new ClientProxy().ExecuteService("DA_PRD_SEL_WIP_NT_RFID", "INDATA", "OUTDATA", inTable, (searchResult, searchException) =>
                ...
                {
                    new ClientProxy().ExecuteService("DA_PRD_SEL_WIP_NT", "INDATA", "OUTDATA", inTable, (searchResult, searchException) =>
        ...
        private void SetMoldInfoData()
        {
            try
            {
                if (tbUBMInfo.Visibility == Visibility.Collapsed)
                    return;
                ...
                DataRow newRow = inTable.NewRow();
                newRow["LANGID"] = LoginInfo.LANGID;
                newRow["LOTID"] = _CurrentLotInfo.PR_LOTID;
                newRow["EQPTID"] = cboEquipment.SelectedValue;

                inTable.Rows.Add(newRow);

                DataTable searchResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_MOLD_INFO_HIST", "INDATA", "OUTDATA", inTable);

                Util.GridSetData(dgMoldInfo, searchResult, FrameOperation, true);
        ...
        private void StartRun(string sNewLot)
        {
            try
            {
                ...
                new ClientProxy().ExecuteService_Multi("BR_PRD_REG_START_LOT_NT_EIF", "IN_EQP,IN_INPUT", "OUT_LOT", (bizResult, bizException) =>
                {
                    try
                    {
                        if (bizException != null)
        ...
        private void RF_ID_CancelRun()
        {
            try
            {
                ...
                inOutLot.Columns.Add("OUTPUT_CSTID", typeof(string));
                inOutLot.Columns.Add("OUTPUT_LOTID", typeof(string));
                ...
                newRow2["INPUT_LOTID"] = Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[iRow].DataItem, "PR_LOTID"));
                inInput.Rows.Add(newRow2);

                DataRow dr = inOutLot.NewRow();
                dr["OUTPUT_CSTID"] = Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[iRow].DataItem, "CSTID"));
                dr["OUTPUT_LOTID"] = Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[iRow].DataItem, "LOTID"));
                inOutLot.Rows.Add(dr);

                new ClientProxy().ExecuteService_Multi("BR_PRD_REG_CANCEL_START_LOT_NT_RFID", "IN_EQP,IN_INPUT,IN_OUTLOT", null, (bizResult, bizException) =>
                {
                    try
                    {
                        if (bizException != null)
        ...
        private void Confirm(int iCopys = -1, string wipNote = "")
        {
            try
            {
        ...
        private void ConfirmLength_Lack(int iCopys = -1, string wipNote = "")
        {
            try
            {
                ...
                // 자동 불량 저장 처리.
                //SaveDefectAllBeforeConfirm(true); //Lack 일 경우 이전에 SaveLoss 에서 저장하고 옴
        ...
        private void AutoPrint(int iCopys = -1)
        {
            try
            {
                ShowLoadingIndicator();
        ...
        private bool CanCancelRun()
        {
            ...
            for (int i = 0; i < dgProductLot.Rows.Count; i++)
            {
                if (Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[i].DataItem, "PR_LOTID")).Equals(sPRLot) && !string.IsNullOrEmpty(Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[i].DataItem, "LOTID"))))
                {
        ...
        private bool CanConfirmCommon()
        {
            ...
            if (dgDetail != null && dgDetail.Rows.Count > 0)
            {
                for (int i = dgDetail.TopRows.Count; i < dgDetail.Rows.Count; i++)
                {
                    ...
                    double goodQty = Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[i].DataItem, "GOODQTY")).Equals("") ? 0 : double.Parse(Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[i].DataItem, "GOODQTY")));
                    if (goodQty < 0)
                    {
        ...
        private bool CanCompleteCancelRun()
        {
            ...
            for (int i = 0; i < dgProductLot.Rows.Count; i++)
            {
                //2020.03.23 완성랏들끼리만 CHILD_GR_SEQNO을 비교하면 되기 때문에 조건 추가
                if (Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[i].DataItem, "PR_LOTID")).Equals(sPRLot) && !string.IsNullOrEmpty(Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[i].DataItem, "LOTID"))))
                {
                    if (!Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[i].DataItem, "CHILD_GR_SEQNO")).Equals(""))
                    {
        ...
        {
            ASSY001_001_HIST window = sender as ASSY001_001_HIST;
            if (window.DialogResult == MessageBoxResult.OK)
            {
        ...
        {
            ASSY001_001_WAITLOT window = sender as ASSY001_001_WAITLOT;
            if (window.DialogResult == MessageBoxResult.OK)
            {
        ...
        private void wndConfirm_Closed(object sender, EventArgs e)
        {
            ...
            try
            {
                if (window.DialogResult == MessageBoxResult.OK && bWndConfirmJobEnd == true)
                {
                    ...
                    {
                        Confirm(window.CHECKED ? 2 : -1, wipNote); // Check 되어 있으면 2매 자동 발행.
                    }
                    else if (window.SELECT_TYPE.Equals("L"))
                    {
                        ...
                        ConfirmLength_Lack(window.CHECKED ? 2 : -1, wipNote);  // Check 되어 있으면 2매 자동 발행.
                    }
                }
            }
            catch (Exception ex)
        ...
        private void wndCnfMsg_Closed(object sender, EventArgs e)
        {
            ASSY001_001_CONFIRM_MSG window = sender as ASSY001_001_CONFIRM_MSG;

            try
            {
                if (window.DialogResult == MessageBoxResult.OK && bWndCnfMsgJobEnd == true)
                {
                    bWndCnfMsgJobEnd = false;

                    SaveLoss(window.REMAINQTY, "MINUS", false, false);

                    ConfirmLength_Lack(window.CHECKED ? 2 : -1);  // Check 되어 있으면 2매 자동 발행.                
                }
            }
            catch (Exception ex)
            {
        ...
        private double SumDefectQty(C1.WPF.DataGrid.C1DataGrid dg, string sColName, out double dfct, out double loss, out double charge_prd, out double increase_prod_qty)
        {
            double sum = 0;
            dfct = 0;
            loss = 0;
            ...
            for (int i = 0; i < dg.Rows.Count - dg.Rows.BottomRows.Count; i++)
            {
                if (Util.NVC(DataTableConverter.GetValue(dg.Rows[i].DataItem, "RSLT_EXCL_FLAG")).Equals("N"))  // 실적 제외 여부 확인.
                {
                    ...
                    // 생산량 증가 수량 sum
                    if (Util.NVC(DataTableConverter.GetValue(dg.Rows[i].DataItem, "PRCS_ITEM_CODE")).Equals("PROD_QTY_INCR"))
                        increase_prod_qty += double.Parse(dg.Columns[sColName].GetCellValue(dg.Rows[i]).ToString());
                }
            }
            return sum;
        }
        ...
        private void SetInputQty(bool bKeyDown = false)
        {
            if (!Util.CheckDecimal(txtInputQty.Text, 0))
            {
                txtInputQty.Text = "";
            ...
            int sChkIdx = _Util.GetDataGridCheckFirstRowIndex(dgProductLot, "CHK");
            double sEqpPQty = double.Parse(Util.NVC(DataTableConverter.GetValue(dgProductLot.Rows[sChkIdx].DataItem, "EQPT_INPUT_QTY"))); // 설비생산량
            ...
            if (!txtParentQty.Text.Equals("") && !txtParent2.Text.Equals(""))
            {
                ...
                if (iInputQty > iParentWipQty && iInputQty <= sEqpPQty)
                {
                    // 생산량 입력 후 엔터 시 처리 방법.
                    ...
                    else // 3. 마지막 완성랏인 경우 : 차이수량 길이초과 자동 등록
                    {
                        Util.MessageConfirm("SFU1921", (result) =>
                        {
                            if (result == MessageBoxResult.OK)
                            {
                                ...
                                double inputQty = iInputQty;// double.Parse(txtInputQty.Text);
                                //double inputQty = 0;
                                double lossQty = 0;
                                ...
                                for (int i = dgDetail.TopRows.Count; i < dgDetail.Rows.Count; i++)
                                {
                                    ...
                                    if (bKeyDown)
                                    {
                                        double dIncreaseProdQty = double.Parse(Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[i].DataItem, "PROD_QTY_INCR_QTY")));
                                        DataTableConverter.SetValue(dgDetail.Rows[i].DataItem, "INPUTQTY_ORG", inputQty - dIncreaseProdQty);
                                    }
                                }
                ...
                else
                {
                    ...
                    // 재공 수량 과 설비생산량 중 큰쪽으로 Validation 처리 요청에 의한 변경(이병렬 책임)
                    double value = iParentWipQty > sEqpPQty ? iParentWipQty : sEqpPQty;
        ...
        private void SetParentRemainQty()
        {
            try
            {
                if (dgDetail.Rows.Count - dgDetail.TopRows.Count > 0)
                {
                    ...
                    if (!(double.TryParse(txtParentQty.Text, out dParentQty) && double.TryParse(Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[dgDetail.TopRows.Count].DataItem, "INPUTQTY")), out dInputQty)))
                        return;

                    if (dParentQty - dInputQty < 0)
                    {
                        txtParent2.Text = "0";
                        txtParent2_M.Text = "0";
                    }
        ...
        {
            DataTable dt = GetChildEqpQty(_CurrentLotInfo.PR_LOTID);
        ...
        private void ConfirmProcess(bool bRealWorkerSelFlag = false)
        {
            try
            {
                int iTimes = 0;
                ...
                {
                    _bRealWorkerSelFlag = bRealWorkerSelFlag;
                    return;
                }
                else
                {
                ...
                if (bFndAnotherChild)
                {
                    ...
                    {
                        Util.MessageConfirm("SFU1963", (result) =>  // "투입LOT 잔량 {0}가 남게 됩니다. 실적확정 하시겠습니까?";
                        {
                            iTimes++;
                ...
                else // 마지막 완성 LOT인 경우
                {
                    if (txtParent2.Text.Trim().Equals("0"))
                    {
                        Util.MessageConfirm("SFU1965", (result) =>  // "투입LOT 잔량이 없습니다. 실적확정 하시겠습니까?";
                        {
                            iTimes++;
                    ...
                    else
                    {
                        ...
                        double dBaseQty = GetRemainBaseQuantity();

                        if (dRemain_M >= dBaseQty)
                        {
                        ...
                        {
                            #region 주석... (메시지 팝업으로 변경 전 로직.)              
                            //Util.MessageConfirm("SFU1853", (result) =>  // 잔량 {0}가 길이부족으로 등록되고, 투입LOT 잔량이 없습니다.\n실적확정 하시겠습니까?
                            //{
                            //    iTimes++;
                            ...
                            //    if (result == MessageBoxResult.OK)
                            //    {
                            //        SaveLoss(dRemain, "MINUS", false, false, "_6");

                            //        ConfirmLength_Lack("_7");
                            //    }
                            //    else
                            //    {
                            //        return;
                            ...
                            ASSY001_001_CONFIRM_MSG wndCnfMsg = new ASSY001_001_CONFIRM_MSG();
                            wndCnfMsg.FrameOperation = FrameOperation;

                            if (wndCnfMsg != null)
                            {
                                object[] Parameters = new object[4];
                                Parameters[0] = MessageDic.Instance.GetMessage("SFU1853", txtParent2.Text.Trim());  // 잔량 {0}가 길이부족으로 등록되고, 투입LOT 잔량이 없습니다.\n실적확정 하시겠습니까?
                                Parameters[1] = true;
                                Parameters[2] = ObjectDic.Instance.GetObjectName("샘플바코드발행");
                                Parameters[3] = dRemain;
                                C1WindowExtension.SetParameters(wndCnfMsg, Parameters);

                                wndCnfMsg.Closed -= new EventHandler(wndCnfMsg_Closed);
                                wndCnfMsg.Closed += new EventHandler(wndCnfMsg_Closed);

                                // 팝업 화면 숨겨지는 문제 수정.
                                this.Dispatcher.BeginInvoke(new Action(() => wndCnfMsg.ShowModal()));
                                //grdMain.Children.Add(wndConfirm);
                                //wndConfirm.BringToFront();
                            }
        ...
        private void ConfirmProcess_Rate(bool bRealWorkerSelFlag = false)
        {
            try
            {
                int iTimes = 0;
            ...
            try
            {
                ...
                for (int i = dgDetail.TopRows.Count; i < dgDetail.Rows.Count; i++)
                {
                    ...
                    if (e.Column.Name.Equals(sColName))
                    {
                        ...
                        #region 진행중인 경우 생산수량 증가 수정 불가.
                        if (_CurrentLotInfo.STATUS.Equals("PROC") && Util.NVC(DataTableConverter.GetValue(e.Row.DataItem, "PRCS_ITEM_CODE")).Equals("PROD_QTY_INCR"))
                        {
                            e.Cancel = true;
                            Util.MessageValidation("SFU3745", Util.NVC(DataTableConverter.GetValue(e.Row.DataItem, "DFCT_CODE_DETL_NAME"))); // 입력오류 : 활동사유[%1]은 생산수량 증가 항목으로 진행중인 Lot에는 입력할 수 없습니다.
                        }
                        #endregion
                    }
                }
        ...
        private double GetRemainBaseQuantity()
        {
            try
            {
                double dRetVal = 50;
                ...
                newRow["CMCDTYPE"] = "NTC_REMAIN_PROC_BASE_QTY";

                inTable.Rows.Add(newRow);

                DataTable searchResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMONCODE_TBL", "INDATA", "OUTDATA", inTable);

                if (searchResult != null && searchResult.Rows.Count > 0 && searchResult.Columns.Contains("ATTRIBUTE1"))
                {
                    double dTmp = 0;
                    if (double.TryParse(Util.NVC(searchResult.Rows[0]["ATTRIBUTE1"]), out dTmp) && dTmp > 0)
                        dRetVal = dTmp;
                }
        ...
        private void SaveRealWorker(string sWrokerName)
        {
            try
            {
                ...
                for (int i = dgDetail.TopRows.Count; i < dgDetail.Rows.Count; i++)
                {
                    DataRow newRow = inTable.NewRow();
                    newRow["LOTID"] = Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[i].DataItem, "LOTID"));
                    newRow["WIPSEQ"] = Util.NVC(DataTableConverter.GetValue(dgDetail.Rows[i].DataItem, "WIPSEQ"));
        ...
        public void CheckHistToolVisibity()
        {
            try
            {
                ...
                dtRow["ATTRIBUTE2"] = Process.NOTCHING;
                ...
                if (dtRslt != null && dtRslt.Rows.Count > 0)
                {
                    tbToolInputHist.Visibility = Visibility.Visible;
                    tbUBMInfo.Visibility = Visibility.Collapsed;
                }
                else
                {
                    tbToolInputHist.Visibility = Visibility.Collapsed;
                    tbUBMInfo.Visibility = Visibility.Visible;
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if (tbToolInputHist.Visibility == Visibility.Collapsed || winInputHistTool == null)
                return;

            winInputHistTool.EQPTID = Util.NVC(cboEquipment.SelectedValue);
            winInputHistTool.PROD_LOTID = _CurrentLotInfo.PR_LOTID;
