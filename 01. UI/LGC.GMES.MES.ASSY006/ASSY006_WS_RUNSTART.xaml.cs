/*************************************************************************************
 Created Date : 2021.12.23
      Creator : 신광희 
   Decription : 소형 조립 공정진척(NFF) Washing 공정 착공 ASSY002_003_RUNSTART 참고
--------------------------------------------------------------------------------------
 [Change History]
...
{
    public partial class ASSY006_WS_RUNSTART : C1Window, IWorkArea
    {
        #region Declaration
        ...
        #region [Form Load]
        private void ASSY006_WS_RUNSTART_Loaded(object sender, RoutedEventArgs e)
        {
        ...
        private void StartRunProcess()
        {
            try
            {
                ...
                string inputLotId = _Util.GetDataGridFirstRowBycheck(dgInputProduct, "CHK", true).Field<string>("LOTID").GetString();
        ...
        private bool ValidateStartRun()
        {
            ...
            int idx = _Util.GetDataGridFirstRowIndexByCheck(dgInputProduct, "CHK", true);
            if (idx < 0)
            {
                //Util.Alert("선택된 투입 반제품이 존재하지 않습니다.");
                Util.MessageValidation("SFU1650");
