/*************************************************************************************
 Created Date : 2021.12.23
      Creator : 신광희 
   Decription : 소형 조립 공정진척(NFF) Assembly 공정 착공 ASSY002_002_RUNSTART 참고
--------------------------------------------------------------------------------------
 [Change History]
 2021.12.23  신광희 : Initial Created.
**************************************************************************************/
...
{
    public partial class ASSY006_AS_RUNSTART : C1Window, IWorkArea
    {
        #region Declaration
        ...
        #region [Form Load]
        private void ASSY006_AS_RUNSTART_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            cathodeBind.Columns.Add("LOTID", typeof(string));                // LOT ID
            //cathodeBind.Columns.Add("WINDING_RUNCARD_ID", typeof(string));   // 이력카드ID
            ...
            if (dtDgInputProduct.Rows.Count > 0)
            {
                for (int i=0; i < dtDgInputProduct.Rows.Count; i++)
                {
                    ...
                    newRow["LOTID"] = dtDgInputProduct.Rows[i]["INPUT_LOTID"];
        ...
        {
            CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH popHalfProduct = new CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH {FrameOperation = FrameOperation};

            // SET PARAMETER
            object[] parameters = new object[5];
            parameters[0] = Util.NVC(_processCode);                                                                       // 공정코드
            parameters[1] = Util.NVC(_equipmentSegmentCode);                                                              // Line
            parameters[2] = Util.NVC(DataTableConverter.GetValue(dgInputProduct.Rows[0].DataItem, "LOTID"));              // LOT ID
            parameters[3] = "N";  //엔터/조회버튼 클릭여부 
            parameters[4] = _equipmentCode;
            C1WindowExtension.SetParameters(popHalfProduct, parameters);

            popHalfProduct.Closed += new EventHandler(popHalfProduct_Closed);
            this.Dispatcher.BeginInvoke(new Action(() => popHalfProduct.ShowModal()));
        }

        private void popHalfProduct_Closed(object sender, EventArgs e)
        {
            CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH popHalfProduct = sender as CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH;

            DataRow selectRow = null;

            if (popHalfProduct.DialogResult == MessageBoxResult.OK)
            {
                selectRow = popHalfProduct.SELECTEDROW;

                for (int i = 0; i < dgInputProduct.Rows.Count; i++)
                {
                    DataTableConverter.SetValue(dgInputProduct.Rows[i].DataItem, "LOTID", Convert.ToString(selectRow["LOTID"]));                           // LOTID
        ...
        {
            if (e.Key == Key.Enter)   // BARCODE 입력 시
            {
                TextBox txtBox = sender as TextBox;

                int rowIndex = ((DataGridCellPresenter)((FrameworkElement)(((FrameworkElement)sender).Parent)).Parent).Row.Index;
                ...
                if (!string.IsNullOrEmpty(txtBox?.Text.Trim()))
                {
                    ...
                    if (txtBox.Text.Trim().Length == 10)
                    {
                        ...
                        {
                            // SET VALUE
                            DataTableConverter.SetValue(dgInputProduct.Rows[rowIndex].DataItem, "LOTID", Convert.ToString(resultDt.Rows[0]["LOTID"]));         // LOTID
                ...
                CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH popHalfProeduct = new CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH();
                popHalfProeduct.FrameOperation = FrameOperation;

                // SET PARAMETER
                object[] parameters = new object[5];
                parameters[0] = _processCode;                // 공정코드
                parameters[1] = _equipmentSegmentCode;       // Line
                parameters[2] = Util.NVC(txtBox.Text);       // LOT ID
                parameters[3] = "Y";  //엔터/조회버튼 클릭여부 
                parameters[4] = _equipmentCode;
                C1WindowExtension.SetParameters(popHalfProeduct, parameters);

                popHalfProeduct.Closed += new EventHandler(SearchEnter_Closed);
                Dispatcher.BeginInvoke(new Action(() => popHalfProeduct.ShowModal()));
                ...
                //    if (tmp.Name == "grdMain")
                //    {
                //        tmp.Children.Add(popHalfProeduct);
                //        popHalfProeduct.BringToFront();
                //        break;
                //    }
                //}
            }
        ...
        private void SearchEnter_Closed(object sender, EventArgs e)
        {
            CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH popup = sender as CMM_ASSY_WG_WAITING_HALF_PRODUCT_SEARCH;

            DataRow selectRow = null;

            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                selectRow = popup.SELECTEDROW;
        ...
        //    try
        //    {
        //        DataTable indataTable = _bizDataSet.GetDA_BAS_SEL_TB_SFC_EQPT_CURR_MOUNT_MTRL_CBO_ASSY();
        ...
        private DataTable GetHalfProductList(string sLotId)
        {
            // SELECT HALF PRODUCT
            try
            {
                ...
                bizRuleName = "DA_PRD_SEL_WINDING_LOT_WAIT_LIST";
                ...
                inDataRow["LOTID"] = Util.NVC(sLotId);
                //inDataRow["WINDING_RUNCARD_ID"] = Util.NVC(_windingRuncardID);
        ...
        {
            // SELECT EQPT SLOT
            txtAssemblyLotId.Text = "";

            try
            {
                string bizRuleName = "BR_PRD_GET_NEW_PROD_LOTID_AS";
                ...
                new ClientProxy().ExecuteService_Multi(bizRuleName, "IN_EQP,IN_INPUT", "OUT_EQP", (bizResult, bizException) =>
                {
                    try
                    {
                        ...
                        txtAssemblyLotId.Text = Util.NVC(bizResult.Tables["OUT_EQP"].Rows[0]["PROD_LOTID"]);
                    }
                    catch (Exception ex)
                    {
                        Util.MessageException(ex);
        ...
        private void StartRunProcess()
        {
            try
            {
                ...
                string bizRuleName = "BR_PRD_REG_START_PROD_LOT_AS";
                ...
                foreach (DataGridRow dRow in dgInputProduct.Rows)
                {
                    if (Util.NVC(DataTableConverter.GetValue(dRow.DataItem, "LOTID")).ToString() != string.Empty)
                    {
                            ...
                            newRow["WINDING_RUNCARD_ID"] = Util.NVC(DataTableConverter.GetValue(dRow.DataItem, "LOTID"));
