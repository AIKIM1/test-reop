/*************************************************************************************
 Created Date : 2022.05.23
      Creator : 배현우
   Decription : ASSY003_MAGAZINE_CREATE를 카피하여 트레이 재구성 화면구현
--------------------------------------------------------------------------------------
 [Change History]
  2022.05.23  배현우 : Initial Created.
  2022.06.22  배현우 : 구분 유형 변경.  잔량, 재생 -> FTF, CCW 
**************************************************************************************/
...
{
    /// <summary>
    /// ASSY006_TRAY_RECONF.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY006_TRAY_RECONF : C1Window, IWorkArea
    {
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (tmps != null && tmps.Length >= 5)
                {
                    ...
                    Console.WriteLine(_LineID + " " + _EqptID + " " + _WoID + " " + _ProcID + " " + _WoDetlID);
                }
                else
                {
                    _LineID = "";
        ...
        private void CreateMagazine()
        {
            try
            {
                ...
                newRow["OUTPUT_QTY"] = txtQty.Text.Equals("") ? 0 : int.Parse(txtQty.Text);
                //newRow["MKT_TYPE_CODE"] = Util.NVC(cboMkTypeCode.SelectedValue);

                // 구분 FTF, CCW  
                if (rdoFtf.IsChecked != null && (bool)rdoFtf.IsChecked)
                {
                    //FTF
                    newRow["MGZN_RECONF_TYPE_CODE"] = "FTF";
                }
                else
                {
                    //CCW
                    newRow["MGZN_RECONF_TYPE_CODE"] = "CCW";
                }
