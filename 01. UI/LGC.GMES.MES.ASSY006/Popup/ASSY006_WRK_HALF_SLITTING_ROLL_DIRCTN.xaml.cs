/*************************************************************************************
 Created Date : 2023.11.29
      Creator : 배현우 사원
   Decription : 오창 2산단 NFF MP GMES 구축 proj. - 무지부/권취 방향설정
--------------------------------------------------------------------------------------
 [Change History]
  2023.11.29  배현우 사원 : Initial Created. (CMM_ELEC_WORK_HALF_SLITTING_ROLL_DIRCTN Copy)
  2024.04.23  배현우 사원 : 기존설정된 무지부/권취 방향이 Display 되도록 수정
**************************************************************************************/
...
namespace LGC.GMES.MES.ASSY006.Popup
{
    ...
    /// </summary>
    public partial class ASSY006_WRK_HALF_SLITTING_ROLL_DIRCTN : C1Window, IWorkArea
    {
        ...
        public ASSY006_WRK_HALF_SLITTING_ROLL_DIRCTN()
        {
            InitializeComponent();
                        ...
                        case 0:
                            ...
                            foreach (Label lbl in wpMain.Children.OfType<Label>())
                            {
                                if (!((TextBlock)lbl.Content).Text.Equals(""))
                                    sPstnID = drCurr["EQPT_MOUNT_PSTN_ID"].ToString();
                            }
                            break;
                        case 1:
                            foreach (RadioButton rdo in wpMain1.Children.OfType<RadioButton>())
        ...
        private void SetComponentEXP(DataTable dtRdo, DataTable dtUWND)
        {
            ...
            {
                _currNumber = i;
                temp = dtCurr.Rows[i]["WRK_HALF_SLIT_SIDE"].ToString() + dtCurr.Rows[i]["EM_SECTION_ROLL_DIRCTN"].ToString();
                switch (i)
                {
                    case 0:
                        drCurr = dtCurr.Rows[i];
                        ...
                        foreach (DataRow dr in dtRdo.Rows)
                        {
                            ...
                            rb.Content = dr["CBO_NAME"];
                            rb.Tag = dr["CBO_CODE"];
                            if (temp.Equals(dr["CBO_CODE"].ToString())) rb.IsChecked = true;
                            wpMain.Children.Add(rb);
                        }
                        break;
                    case 1:
                        ...
                        foreach (DataRow dr in dtRdo.Rows)
                        {
                            ...
                            rb1.Tag = dr["CBO_CODE"];
                            if (temp.Equals(dr["CBO_CODE"].ToString())) rb1.IsChecked = true;
                    ...
                    case 2:
                        ...
                        foreach (DataRow dr in dtRdo.Rows)
                        {
                            ...
                            rb2.Tag = dr["CBO_CODE"];
                            if (temp.Equals(dr["CBO_CODE"].ToString())) rb2.IsChecked = true;
                    ...
                    case 3:
                        ...
                        foreach (DataRow dr in dtRdo.Rows)
                        {
                            ...
                            rb3.Tag = dr["CBO_CODE"];
                            if (temp.Equals(dr["CBO_CODE"].ToString())) rb3.IsChecked = true;
                    ...
                    case 4:
                        ...
                        foreach (DataRow dr in dtRdo.Rows)
                        {
                            ...
                            rb4.Tag = dr["CBO_CODE"];
                            if (temp.Equals(dr["CBO_CODE"].ToString())) rb4.IsChecked = true;
