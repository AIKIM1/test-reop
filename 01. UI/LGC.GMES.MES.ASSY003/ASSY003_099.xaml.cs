...
{
    /// <summary>
    /// ASSY003_099.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY003_099 : UserControl, IWorkArea
    {
        Util _Util = new Util();
        ...
        private void InitCombo()
        {
            try
            {
                ...
                newRow = dtTemp.NewRow();
                newRow.ItemArray = new object[] { "阳极(C)", "C" };
                dtTemp.Rows.Add(newRow);

                newRow = dtTemp.NewRow();
                newRow.ItemArray = new object[] { "单面(S)", "S" };
                dtTemp.Rows.Add(newRow);

                newRow = dtTemp.NewRow();
                newRow.ItemArray = new object[] { "阴极(A)", "A" };
                dtTemp.Rows.Add(newRow);

                cboElectType.ItemsSource = dtTemp.Copy().AsDataView();
                cboElectType.DisplayMemberPath = "NAME";
                cboElectType.SelectedValuePath = "CODE";
                cboElectType.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
        ...
        {            
            try
            {
                ...
                dt.Columns.Add("ELECTYPE", typeof(string));
        ...
        private void txtQty_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.Key == Key.Enter)
                {
                    ...
                    DataRow newRow = dtTemp.NewRow();
                    newRow.ItemArray = new object[] { cboEquipmentSegment.SelectedValue, cboModel.SelectedValue, txtLotID.Text.Trim(), cboElectType.SelectedValue, txtQty.Text };
                    dtTemp.Rows.Add(newRow);
        ...
        private bool CanAdd()
        {
            ...
            if (cboElectType == null || cboElectType.SelectedValue == null || Util.NVC(cboElectType.SelectedValue).Equals(""))
            {
        ...
        private void SaveProcess()
        {
            try
            {
                ...
                inTable.Columns.Add("ACS", typeof(string));
                ...
                for (int i = 0; i < dgList.Rows.Count; i++)
                {
                    ...
                    newRow["ACS"] = Util.NVC(DataTableConverter.GetValue(dgList.Rows[i].DataItem, "ELECTYPE"));
                ...
                new ClientProxy().ExecuteService("BR_PRD_CREATE_LAMI_WAIT_LOT_TEMP", "INDATA", null, inTable, (searchResult, searchException) =>
                {
                    try
                    {
