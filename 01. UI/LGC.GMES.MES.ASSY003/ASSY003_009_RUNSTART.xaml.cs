...
{
    /// <summary>
    /// ASSY003_009_RUNSTART.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY003_009_RUNSTART : C1Window, IWorkArea
    {   
        ...
        private void dgMGZ_CommittingEdit(object sender, DataGridEndingEditEventArgs e)
        {
            try
            {
                ...
                if (DataTableConverter.GetValue(e.Row.DataItem, "CHK").Equals(1))
                {
                    ...
                    if (!Util.NVC(DataTableConverter.GetValue(dgInput.Rows[idx].DataItem, "INPUT_LOTID")).Trim().Equals(""))
                    {
                        //Util.Alert("해당 위치는 이미 투입 정보가 존재하여 투입할 수 없습니다.");
                        Util.MessageValidation("SFU2021");
                        return;
        ...
        private void RunStart(string sNewLot)
        {
            try
            {
                ...
                new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_START_PROD_LOT_STP", "IN_EQP,IN_INPUT", "OUT_INPUT", indataSet);
