/*************************************************************************************
 Created Date : 2019.11.06
      Creator : 이상준C
   Decription : 전지 5MEGA-GMES 구축 - ZZS 실적 확정 화면 - 팝업
--------------------------------------------------------------------------------------
 [Change History]
...
{
    /// <summary>
    /// ASSY003_023_CONFIRM.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY003_023_CONFIRM : C1Window, IWorkArea
    {
        ...
        private void GetProductionResult()
        {
            try
            {
                ...
                DataTable SearchResult = Util.Get_ResultQty_Chk(_LotID, Process.ZZS, _EqptID, _LineID, _WipSeq);
        ...
        private void Save()
        {
            try
            {
                ...
                new ClientProxy().ExecuteService_Multi("BR_PRD_REG_END_LOT_ZZS", "INDATA,IN_INPUT", null, (bizResult, bizException) =>
                {
                    try
                    {
                        if (bizException != null)
