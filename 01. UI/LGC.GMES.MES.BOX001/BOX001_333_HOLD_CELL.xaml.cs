...
--------------------------------------------------------------------------------------
 [Change History]
 2020.06.03 BOX001_213_HOLD Copy 후 작성
 2023.03.28 김대현 ESWA 1동 조립 GMES의 포장홀드 시 활성화 전환 동의 Hold Validation
  2024.07.22 유재홍                        BOX001_333로 복사 후 HOLDCODE추가
**************************************************************************************/
...
namespace LGC.GMES.MES.BOX001
{
    ...
    /// </summary>
    public partial class BOX001_333_HOLD_CELL : C1Window, IWorkArea
    {
        ...
        private DataTable dtPkgLotRuleArea = new DataTable();   //2023.03.28 김대현
        private string AreaName = string.Empty; //2023.03.28 김대현
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            dtPkgLotRuleArea = GetPkgOutLotIDRuleArea();    //2023.03.28 김대현
        }
        ...
        private void dgHold_CommittedEdit(object sender, DataGridCellEventArgs e)
        {
            ...
            //2023.03.28 김대현
            if(e.Cell.Column.Name == "STRT_SUBLOTID")
            {
                string strt_sublotid = Util.NVC(dataGrid.GetCell(e.Cell.Row.Index, dataGrid.Columns["STRT_SUBLOTID"].Index).Value);

                if (!IsPkgOutLotIDRuleAreaCheck(strt_sublotid))
                {
                    Util.MessageInfo("SFU9017", new object[] { AreaName, strt_sublotid });
                }
            }
        ...
        public bool IsPkgOutLotIDRuleAreaCheck(string sublot)
        {
            bool result = true;
            ...
            newRow["STRT_SUBLOTID"] = sublot;
            newRow["LANG_ID"] = LoginInfo.LANGID;
            inDataTable.Rows.Add(newRow);

            if (dtPkgLotRuleArea != null && dtPkgLotRuleArea.Rows.Count > 0)
            {
                try
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        {
                            DataRow[] dr = dtPkgLotRuleArea.Select("CMCODE='" + AreaID + "'");
            ...
            try
            {
                ...
                if (fd.ShowDialog() == true)
                {
                    using (Stream stream = fd.OpenFile())
                    {
                        ...
                        {
                            string sublot = Util.NVC(dtInfo.Rows[row]["STRT_SUBLOTID"]);

                            if (!IsPkgOutLotIDRuleAreaCheck(sublot))
                            {
                                Util.MessageValidation("SFU9017", new object[] { AreaName, sublot });
                                return;
                            }
                        }
                    }
        ...
        private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            //2023.03.28 김대현
            DataTable dtData = DataTableConverter.Convert(dgHold.ItemsSource);
            for(int row = 0; row < dtData.Rows.Count; row++)
            {
                string sublot = Util.NVC(dtData.Rows[row]["STRT_SUBLOTID"]);
            ...
            try
            {
                ...
                CMM_BOX_HOLD_CELL_PROGRESSBAR_HOLDRESN popupProgressing = new CMM_BOX_HOLD_CELL_PROGRESSBAR_HOLDRESN();
                ...
                if (popupProgressing != null)
                {
                    ...
                    parameters[10] = Util.GetCondition(cboHoldType, "SFU1593");//사유는 필수입니다.
        ...
        {
            CMM_BOX_HOLD_CELL_PROGRESSBAR_HOLDRESN window = sender as CMM_BOX_HOLD_CELL_PROGRESSBAR_HOLDRESN;
