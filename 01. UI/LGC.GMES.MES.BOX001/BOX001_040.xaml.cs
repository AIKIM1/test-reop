...
{
    public partial class BOX001_040 : UserControl, IWorkArea
    {
        Util _Util = new Util();
        ...
        private void btnProcess_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string sProdid2 = drChk[0]["PRODID"].ToString();
                ...
                for (int i = 0; i < dgPackList.GetRowCount(); i++)
                {
                    if (Util.NVC(DataTableConverter.GetValue(dgPackList.Rows[i].DataItem, "CHK")) == "1")
                    {
                        ...
                        if (sProdid[0] != Util.NVC(DataTableConverter.GetValue(dgPackList.Rows[i].DataItem, "PRODID")))
                        {
                ...
                for (int i = 0; i < dgPackList.GetRowCount(); i++)
                {
                    if (Util.NVC(DataTableConverter.GetValue(dgPackList.Rows[i].DataItem, "CHK")) == "1")
                    {
                        ...
                        row["INOUT_FLAG"] = "O"; // O:출하, I:입고, X:취소
                        ...
                        row["WIPQTY2"] = DataTableConverter.GetValue(dgPackList.Rows[i].DataItem, "WIPQTY").ToString();
        ...
        private void btnSearch_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                DataTable SearchResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_WO_FOR_ROLL", "RQSTDT", "RSLTDT", RQSTDT);
        ...
        private void btnSearch2_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                DataTable SearchResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_ROLL_WIPACT_HIST", "RQSTDT", "RSLTDT", RQSTDT);
            ...
            try
            {
                ...
                if (dgPlanList.GetRowCount() > 0)
                {
                    ...
                    else
                    {
                        ...
                        sProdid = drChk[0]["PRODID"].ToString().Split('_');
                ...
                else
                {
                    ...
                    DataTableConverter.SetValue(dgPackList.CurrentRow.DataItem, "LANE_QTY", SearchResult.Rows[0]["LANE_QTY"].ToString());
                    ...
                    DataTableConverter.SetValue(dgPackList.CurrentRow.DataItem, "WIPQTY2", SearchResult.Rows[0]["WIPQTY2"].ToString());
            ...
            try
            {
                ...
                for (int i = 0; i < dgOutList.GetRowCount(); i++)
                {
                    if (Util.NVC(DataTableConverter.GetValue(dgOutList.Rows[i].DataItem, "CHK")) == "1")
                    {
                        ...
                        string laneQty = "1";
                        if (sProdid.Length > 1)
                            laneQty = System.Text.RegularExpressions.Regex.Replace(sProdid[1], @"\D", "");
                        ...
                        row["PRODID"] = sProdid[0];
                        row["WIPQTY"] = DataTableConverter.GetValue(dgOutList.Rows[i].DataItem, "WIPQTY").ToString();
                        row["WIPQTY2"] = decimal.Parse(DataTableConverter.GetValue(dgOutList.Rows[i].DataItem, "WIPQTY").ToString()) * decimal.Parse(laneQty);
                        row["USERID"] = LoginInfo.USERID;
                        row["LANE_QTY"] = laneQty;
