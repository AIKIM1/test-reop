...
{
    public partial class BOX001_230 : UserControl, IWorkArea
    {
       ...
       // private string _rcvStat = string.Empty;
        ...
        private void PrintOutBox()
        {
            ...
            string sBizRule = "BR_PRD_REG_IN_OUT_BOX_NJ";
            ...
            inBoxTable.Columns.Add("FORM_INBOXID");
            inBoxTable.Columns.Add("WEIGHT");
            inBoxTable.Columns.Add("MODE_FLAG");
            //inBoxTable.Columns.Add("DESAY_FLAG");

            newRow = inBoxTable.NewRow();
            newRow["BOXID"] = txtBoxId.Text.Trim();
            newRow["FORM_INBOXID"] = (bool)chkMode.IsChecked ? txtInboxId.Text : "";
            ...
            //new ClientProxy().ExecuteService_Multi("BR_PRD_REG_IN_OUT_BOX_NJ", "INPALLET,INBOX,INPRINT", "OUTDATA", (bizResult, bizException) =>
...
+ "^PQ ");
        ...
        private void btnRunStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            BOX001_230_RUNSTART popup = new BOX001_230_RUNSTART();
        ...
        {
            BOX001_230_RUNSTART popup = sender as BOX001_230_RUNSTART;
            ...
            Util.MessageConfirm("SFU1168", (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    try
                    {
                        ...
                        dt.Rows.Add(dr);
                        new ClientProxy().ExecuteService("BR_PRD_DEL_12ND_PLT_NJ", "INDATA", null, dt, (bizResult, bizException) =>
                        {
                            if (bizException != null)
                            {
                                Util.MessageException(bizException);
            ...
            try
            {
                ...
                Util.MessageConfirm("SFU4007", (result) =>
                {
                    if (result == MessageBoxResult.OK)
                    {
                        ...
                        loadingIndicator.Visibility = Visibility.Visible;
                        new ClientProxy().ExecuteService_Multi("BR_PRD_UPD_12ND_DETAIL_NJ", "INDATA,INBOX", "OUTDATA,OUTINBOX,OUTCTNR", (bizResult, bizException) =>
                        {
                            try
                            {
                                if (bizException != null)
        ...
        private void txtStartPalletID_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.Key == Key.Enter)
                {
                    ...
                    if((bool)chkLotSuffix.IsChecked && string.IsNullOrEmpty(txtLotSuffix.Text)){
                        //조립Lot Suffix를 입력해주세요.
                        Util.MessageValidation("SFU8297"); 
                        return;
                    }
                    ...
                    Util.MessageConfirm("SFU1987", (result) =>
                    {
                        if (result == MessageBoxResult.OK)
                        {
                            ...
                            inDataTable.Columns.Add("USERID");
                            inDataTable.Columns.Add("SUFFIX");
                            ...
                            newRow["BOXID"] = sPalletId;
                            newRow["USERID"] = txtWorker_Main.Tag;
                            newRow["SUFFIX"] = (bool)chkLotSuffix.IsChecked ? txtLotSuffix.Text : "";
        ...
        private void dgInTray_CommittedEdit(object sender, C1.WPF.DataGrid.DataGridCellEventArgs e)
        {
            ...
            if (e.Cell.Column.Name == "RESTQTY")
            {
                ...
                // Pallet의잔량과 비교
                int idxPallet = _util.GetDataGridCheckFirstRowIndex(dgPalletList, "CHK");
                int RemainQty = Util.NVC_Int(Util.NVC(dgPalletList.GetCell(idxPallet, dgPalletList.Columns["RESTQTY"].Index).Value));
            ...
            try
            {
                ...
                Util.MessageConfirm("SFU1230", (result) =>
                {
                    if (result == MessageBoxResult.OK)
                    {
                        ...
                        new ClientProxy().ExecuteService_Multi("BR_PRD_DEL_OUTBOX_FOR_12ND_PLT_NJ", "INDATA,INBOX", null, (bizResult, bizException) =>
                        {
                            try
                            {
                                if (bizException != null)
        ...
        private void GetPalletList()
        {
            try
            {
                ...
                DataTable RSLTDT = new ClientProxy().ExecuteServiceSync("BR_PRD_GET_12ND_PALLET_LIST_NJ", "INDATA", "OUTDATA", RQSTDT);
        ...
        private void GetDetailInfo()
        {
            try
            {
                ...
                new ClientProxy().ExecuteService_Multi("BR_PRD_GET_12ND_DETAIL_NJ", "INDATA", "OUTDATA,OUTCTNR,OUTINBOX", (bizResult, bizException) =>
                {
                    try
                    {
                        if (bizException != null)
                        ...
                        if (bizResult.Tables.Contains("OUTDATA"))
                        {
                            ...
                            txtPRODID.Text = sProdId;
                            txtInputQty.Value = iWipQty;
                            ////txtRestCellQty.Value = iRestQty;  체크
                            //   txtRemainQty.Value = iRestQty;
                            ////if (!string.IsNullOrWhiteSpace(sInboxLoadQty)) txtBoxCellQty.Value = Util.NVC_Int(sInboxLoadQty); 체크
                            //txtSoc.Text = sSOC;
                            new TextRange(txtNote.Document.ContentStart, txtNote.Document.ContentEnd).Text = sNote;

                            if (sLabelType == "DESAY BOX") // 팔레트의 라벨타입이 DESAY인 경우에만 Checkbox 선택 가능함.
                            {
                        ...
                        if (bizResult.Tables.Contains("OUTINBOX"))
                        {
                            ...
                            txtTotalCellQty.Text = totalCellQty.ToString();
        ...
        private void chkLotSuffix_Click(object sender, RoutedEventArgs e)
        {
            if ((bool)chkLotSuffix.IsChecked)
            {
                txtLotSuffix.IsEnabled = true;
            }
            else
            {
                txtLotSuffix.IsEnabled = false;
            }
        }
    }
}
