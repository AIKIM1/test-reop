/*************************************************************************************
 Created Date : 2016.06.16
      Creator : 
   Decription : 포장 Pallet 구성 (개별 Cell) - Pallet 실적확정 PopUp
--------------------------------------------------------------------------------------
 [Change History]
  2016.06.16  DEVELOPER : Initial Created.
  2020.07.18 이제섭 UNCODE (수출인증코드) 추가 (공통코드 등록된 Plant는 필수입력)
  2021.02.01 김동일 C20201104-000144   QA 승인 관리동인 경우 검사 sKIP 처리 시 특이사항 필수 입력 체크 및 Mail 전송 추가.
  2021.11.12 안인효 C20210924-000280   수동포장 실적확정 시 자동 HOLD 처리
...
{
    public partial class BOX001_007_CONFIRM : C1Window, IWorkArea
    {
        ...
        public BOX001_007_CONFIRM()
        {
            InitializeComponent();
            Loaded += BOX001_007_CONFIRM_Loaded;

        }

        private void BOX001_007_CONFIRM_Loaded(object sender, RoutedEventArgs e)
        {
            Loaded -= BOX001_007_CONFIRM_Loaded;
            InitSet();
        ...
        private void InitSet()
        {
            ...
            // 자동HOLD 사용 동 체크 후 HOLD해제예정담당자 처리 
            if (!UseUnHoldChangeUser())
            {
        ...
        private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (ChkUseInspSkipQAApprUseArea() && String.IsNullOrWhiteSpace(txtNote.Text))
                {
                    Util.MessageValidation("SFU3773"); // 검사조건 Skip 한 Cell 이 존재 합니다. 특이사항을 입력해 주세요.
                    return;
                }
            }

            if (txtUser.IsEnabled == true && string.IsNullOrEmpty(txtUser.Text))
            {
        ...
        private string InsertCELLTOPALLETData()
        {
            ...
            try
            {
                ...
                inData["UNHOLD_CHARGE_USERID"] = txtUser.Tag;
                ...
                loadingIndicator.Visibility = Visibility.Visible;
                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_PACKING_BY_USER_CP", "INDATA,INDATA_SUBLOT", "OUTDATA", indataSet);
                ...
                else
                {
                    ...
                    try
                    {
                        if (Util.NVC(txtSkip.Text) == "SKIP" && ChkUseInspSkipQAApprUseArea())
                        {
                            MailSend mail = new CMM001.Class.MailSend();
                            string sMsg = ObjectDic.Instance.GetObjectName("INSP_SKIP_INFORM_MAIL_TITLE");      // 제목
                            string sTitle = ObjectDic.Instance.GetObjectName("INSP_SKIP_INFORM_MAIL_CONTENT");  // 메일 내용 항목
                            string sMailNote = ObjectDic.Instance.GetObjectName("INSP_SKIP_INFORM_MAIL_REASON");// 메일 사유 항목
                            ...
                            dr["AREAID"] = Util.NVC(sAREAID);
                            dr["COM_TYPE_CODE"] = "BOX_INSP_SKIP_QA_MAIL_LIST";
                            ...
                            if (searchResult != null && searchResult.Rows.Count > 0)
                            {
                                ...
                                {
                                    sTo += Util.NVC(dRow["ATTR1"]) + ";";
                                }

                                mail.SendMail(LoginInfo.USERID, LoginInfo.USERNAME, sTo, "", sMsg, this.makeBodyApp(sTitle, sMailNote, Util.NVC(dsResult.Tables["OUTDATA"].Rows[0]["BOXID"])));
                            }
                        }
                    }
                    catch (Exception ex)
        ...
        /// </summary>
        /// <returns></returns>
        private bool UseUnHoldChangeUser()
        {
            DataTable inTable = new DataTable();
            inTable.Columns.Add("LANGID", typeof(string));
            inTable.Columns.Add("CMCDTYPE", typeof(string));
            ...
            DataRow dr = inTable.NewRow();
            dr["LANGID"] = LoginInfo.LANGID;
            dr["CMCDTYPE"] = "MANUAL_PALLET_AUTO_HOLD_AREA";
            dr["CMCODE"] = sAREAID;
        ...
        private bool ChkUseInspSkipQAApprUseArea()
        {
            DataTable inTable = new DataTable();
            inTable.Columns.Add("LANGID", typeof(string));
            inTable.Columns.Add("CMCDTYPE", typeof(string));
            ...
            DataRow dr = inTable.NewRow();
            dr["LANGID"] = LoginInfo.LANGID;
            dr["CMCDTYPE"] = "BOX_INSP_SKIP_QA_APPR_USE_AREA";
            dr["CMCODE"] = Util.NVC(sAREAID);
        ...
        private string makeBodyApp(string sTitle, string sContent, string sPalletID)
        {
            ...
            try
            {
                ...
                //{
                    ...
                    sb.Append("                        <th>" + ObjectDic.Instance.GetObjectName("PALLETID") + "</th>");
                    sb.Append("                        <th>" + ObjectDic.Instance.GetObjectName("수량") + "</th>");
                    sb.Append("                        <th>" + ObjectDic.Instance.GetObjectName("모델") + "</th>");
                    sb.Append("                        <th>" + ObjectDic.Instance.GetObjectName("제품ID") + "</th>");
                    sb.Append("                        <th>" + ObjectDic.Instance.GetObjectName("라인") + "</th>");
                    sb.Append("                        <th>" + ObjectDic.Instance.GetObjectName("출하처") + "</th>");
                    sb.Append("                        <th>" + ObjectDic.Instance.GetObjectName("특이사항") + "</th>");
                    sb.Append("                    </tr>");
                    sb.Append("                </thead>");
                    sb.Append("                <tbody>");
                    ...
                    //{
                        sb.Append("                	<tr>");
                        sb.Append("                     <td>" + Util.NVC(sPalletID) + "&nbsp;</td>");
                        sb.Append("                     <td>" + Util.NVC_NUMBER(txtOutputqty.Text) + "&nbsp;</td>");
                        sb.Append("                     <td>" + Util.NVC(txtModel.Text) + "&nbsp;</td>");
                        sb.Append("                     <td>" + Util.NVC(txtProdID.Text) + "&nbsp;</td>");
                        sb.Append("                     <td>" + Util.NVC(txtLine.Text) + "&nbsp;</td>");
                        sb.Append("                     <td>" + Util.NVC(cboComp.Text) + "&nbsp;</td>");
                        sb.Append("                     <td>" + Util.NVC(txtNote.Text) + "&nbsp;</td>");
                        sb.Append("                 </tr>");
                    //}
                    sb.Append("                </tbody>");
                    sb.Append("            </table>");
