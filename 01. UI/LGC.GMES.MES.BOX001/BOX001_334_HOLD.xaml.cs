...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2024.07.30  유재홍    : Hold_code,Unhold_Code 추가 및 선택/조회기능 추가
**************************************************************************************/
...
namespace LGC.GMES.MES.BOX001
{
    ...
    /// </summary>
    public partial class BOX001_334_HOLD : C1Window, IWorkArea
    {
        ...
        private void InitControl()
        {
            ...
            //E20240219 - 000132
            //포장시 NA, HM만 해당 칼럼 보도록 되어있어서 특화 로직 제거
        ...
        private void Save()
        {
            try
            {
                ...
                //newRow["PACK_HOLD_FLAG"] = chkInputHold.IsChecked == true ? "Y" : "N"; //포장시 NA, HM만 해당 칼럼 보도록 되어있어서 특화 로직 제거
        ...
        private void notHoldList(int holdcount)
        {
            try
            {
                BOX001_334_NOTHOLD popupProgress = new BOX001_334_NOTHOLD();
        ...
        {
            BOX001_334_NOTHOLD popup = sender as BOX001_334_NOTHOLD;
                ...
                try
                {
                    ...
                    for (int idx = 0; idx < iQuotient + 1; idx++)
                    {
                        try
                        {
                            ...
                            if (idx == iQuotient)
                                idxEnd = iRowCnt - 1; //2024.07.30 x <= idxEnd 조건을 만족할려면 -1 해야 함, 김수용

                            if (iRowCnt <= iDiv)
                                idxEnd = iRowCnt - 1; //2024.07.30 x <= idxEnd 조건을 만족할려면 -1 해야 함, 김수용

                            //2024.07.30 HOLD Cell Excel import 시 500*idx번째 Cell 누락 오류 조치, 김수용
