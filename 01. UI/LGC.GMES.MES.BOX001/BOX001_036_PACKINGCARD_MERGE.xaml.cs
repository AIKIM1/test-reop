...
{
    public partial class BOX001_036_PACKINGCARD_MERGE : UserControl, IWorkArea
    {


        #region Declaration & Constructor 

        public BOX001_036 BOX001_036;
        ...
        private string m_OwmsCode = string.Empty;
        private string m_PkgName = string.Empty;
        private string m_EltrValueDecimalApplyFlag = string.Empty;  //[E20230306-000128]전극수 소수점 개선

        bool bFormLoad = false;


        public BOX001_036_PACKINGCARD_MERGE()
        {
            InitializeComponent();
        }
        ...
        private void init_Form()
        {
            try
            {
                Util.gridClear(dgLotIDListRemain01);
                Util.gridClear(dgLotIDListRemain02);
                Util.gridClear(dgPancakeListSelected01);
                Util.gridClear(dgPancakeListSelected02);

                m_LotID1 = BOX001_036.sTempLot_1.ToString();
                m_LotID2 = BOX001_036.sTempLot_2.ToString();

                Search_Lot_List(m_LotID1, dgLotIDListRemain01);

                txtLotID1.Text = m_LotID1;
                txtLotID2.Text = m_LotID2;

                txtLaneQty1.Text = Util.NVC(DataTableConverter.GetValue(BOX001_036.dgOut.Rows[0].DataItem, "LANE_QTY"));
                txtCutM1.Text = Util.NVC(DataTableConverter.GetValue(BOX001_036.dgOut.Rows[0].DataItem, "M_WIPQTY"));
                txtCellM1.Text = Util.NVC(DataTableConverter.GetValue(BOX001_036.dgOut.Rows[0].DataItem, "CELL_WIPQTY"));
                m_PROCID = Util.NVC(DataTableConverter.GetValue(BOX001_036.dgOut.Rows[0].DataItem, "PROCID"));

                if (!m_LotID2.Equals(""))
                {
                    Search_Lot_List(m_LotID2, dgLotIDListRemain02);
                    txtLaneQty2.Text = Util.NVC(DataTableConverter.GetValue(BOX001_036.dgOut.Rows[1].DataItem, "LANE_QTY"));
                    txtCutM2.Text = Util.NVC(DataTableConverter.GetValue(BOX001_036.dgOut.Rows[1].DataItem, "M_WIPQTY"));
                    txtCellM2.Text = Util.NVC(DataTableConverter.GetValue(BOX001_036.dgOut.Rows[1].DataItem, "CELL_WIPQTY"));

                }
                else
                {
                    txtLaneQty2.Text = string.Empty;
                    txtCutM2.Text = string.Empty;
                    txtCellM2.Text = string.Empty;
                }

                numLane_Qty1.Value = dgLotIDListRemain01.GetRowCount();
                numLane_Qty2.Value = dgLotIDListRemain02.GetRowCount();

                txtLocation.Text = BOX001_036.cboTransLoc2.Text;
        ...
        private void Search_Lot_List(string sLotid, C1.WPF.DataGrid.C1DataGrid DataGrid)
        {
            try
            {
                ...
                DataTable SearchResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_ELEC_PACKING_ROLL", "RQSTDT", "RSLTDT", RQSTDT);
        ...
        private void cboPackWay_SelectedValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            ...
            WhTable.TableName = "RQSTDT";
            WhTable.Columns.Add("LANGID", typeof(string));
            WhTable.Columns.Add("CMCDTYPE", typeof(string));
            WhTable.Columns.Add("CMCODE", typeof(string));

            DataRow dr = WhTable.NewRow();
            dr["LANGID"] = LoginInfo.LANGID;
            dr["CMCDTYPE"] = "WH_TYPE";
            dr["CMCODE"] = cboPackWay.SelectedValue.ToString();
            WhTable.Rows.Add(dr);

            DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMONCODE_TBL", "RQSTDT", "RSLTDT", WhTable);

            m_PkgWay = dtResult.Rows[0]["CMCDNAME"].ToString();
            m_OwmsCode = dtResult.Rows[0]["ATTRIBUTE1"].ToString();
            m_PkgName = dtResult.Rows[0]["CMCDNAME"].ToString();
            ...
            for (int i = 0; i < dgLotIDListRemain01.GetRowCount(); i++)
            {
                DataTableConverter.SetValue(dgLotIDListRemain01.Rows[i].DataItem, "POSITION", 0);
                DataTableConverter.SetValue(dgLotIDListRemain01.Rows[i].DataItem, "CHK", true);
            }

            for (int i = 0; i < dgLotIDListRemain02.GetRowCount(); i++)
            {
                DataTableConverter.SetValue(dgLotIDListRemain02.Rows[i].DataItem, "POSITION", 0);
                DataTableConverter.SetValue(dgLotIDListRemain02.Rows[i].DataItem, "CHK", true);
            }
        }

        private void InitLaneList()
        {
            DelFromPosition(0, dgPancakeListSelected01, dgLotIDListRemain01);
            DelFromPosition(0, dgPancakeListSelected02, dgLotIDListRemain02);
        ...
        {
            AddToPosition(1, dgPancakeListSelected01, dgLotIDListRemain01, 1);
        ...
        {
            AddToPosition(2, dgPancakeListSelected02, dgLotIDListRemain02, 2);
        ...
        private void InitPackingNo()
        {
            ...
            // ROLL 전극 출하 BOXID 중복되는 현상으로 신규 OUTER_BOXID 발행 로직 추가 [2017-08-28]
            string new_PackingNo = GetOuterBoxNewLotID(m_PackingNo1);
            if (!string.IsNullOrEmpty(new_PackingNo))
                m_PackingNo1 = new_PackingNo;
            ...
            if (!m_LotID2.Equals(""))
            {
                m_PackingNo2 = txtLotID2.Text.ToString().Substring(0, 9) + "0";

                new_PackingNo = string.Empty;
                new_PackingNo = GetOuterBoxNewLotID(m_PackingNo2);
                if (!string.IsNullOrEmpty(new_PackingNo))
                    m_PackingNo2 = new_PackingNo;
        ...
        private void PackingCardDataBinding_Config()
        {
            try
            {
                ...
                sPackageWay = sPackageWay + " " + m_PkgName;
                sOWMS_Code = m_OwmsCode;                
                ...
                if (Util.NVC(m_EltrValueDecimalApplyFlag).Equals("Y"))  //[E20230306-000128]전극수 소수점 개선
                {
                    ...
                    {
                        m_CutStr1 = String.Format("{0:#,##0.#}", (decimal)(Math.Truncate( (m_dCutM1 + m_dCutM2) * 10) / 10) + "/" + (decimal)(Math.Truncate( (m_dConvCutM1 + m_dConvCutM2) * 10) / 10) ) + "M";
                        s_CutStr2 = String.Format("{0:#,##0.#}", (decimal)(Math.Truncate( (m_dCell1 + m_dCell2) * 10) / 10) + "/" + (decimal)(Math.Truncate( (m_dConvCutC1 + m_dConvCutC2) * 10) / 10) ) + "M";
                    }
                    else
                    {
                ...
                else
                {
                    if (m_dConvCutM1 > 0 || m_dConvCutM2 > 0)
                    {
                        m_CutStr1 = String.Format("{0:#,##0}", (m_dCutM1 + m_dCutM2) + "/" + (m_dConvCutM1 + m_dConvCutM2)) + "M";
                        s_CutStr2 = String.Format("{0:#,##0}", (m_dCell1 + m_dCell2) + "/" + (m_dConvCutC1 + m_dConvCutC2)) + "M";
                    }
                    else
                    {
                ...
                if (!m_LotID2.Equals(""))
                {
                    ...
                    dtPackingCard_Merge.Columns.Add("Text5", typeof(string));
                    dtPackingCard_Merge.Columns.Add("Text13", typeof(string));
                    dtPackingCard_Merge.Columns.Add("Text7", typeof(string)); // LOT ID
                ...
                string sSHIPTO_ID = BOX001_036.cboTransLoc2.SelectedValue.ToString();
                ...
                else
                {
                    ...
                    drInfo["QTY2"] = Convert.ToDecimal("0");
                    drInfo["PRODID"] = DataTableConverter.GetValue(dgPancakeListSelected01.Rows[0].DataItem, "PRODID");
                    drInfo["SHIPTO_ID"] = sSHIPTO_ID;
                    drInfo["FROM_AREAID"] = DataTableConverter.GetValue(dgPancakeListSelected01.Rows[0].DataItem, "FROM_AREAID");
                    drInfo["FROM_SLOC_ID"] = DataTableConverter.GetValue(dgPancakeListSelected01.Rows[0].DataItem, "FROM_SLOC_ID");
                    ...
                    drInfo["TOTAL_QTY3"] = Convert.ToDecimal("0");
                    drInfo["TOTAL_QTY4"] = Convert.ToDecimal("0");
                    drInfo["OWMS_BOX_TYPE_CODE"] = sOWMS_Code;
                    drInfo["TO_SHOPID"] = sShopID;
                    drInfo["TYPE"] = "SHIP";
                    drInfo["PROCID"] = m_PROCID;
        ...
        private decimal GetPatternLength(string prodID, string lotID)
        {
            try
            {
        ...
        private void Print_Result(object sender, EventArgs e)
        {
            try
            {
                ...
                if (wndPopup.DialogResult == MessageBoxResult.OK)
                {
                    ...
                    Util.gridClear(BOX001_036.dgOut);
                    BOX001_036.dgSub.Children.Clear();

                    BOX001_036.dgOut.IsEnabled = true;
                    BOX001_036.txtLotID.IsReadOnly = false;
                    BOX001_036.btnPackOut.IsEnabled = true;
                    BOX001_036.txtLotID.Text = "";
                    BOX001_036.txtLotID.Focus();
                }
            }
            catch (Exception ex)
            {
        ...
        private string GetOuterBoxNewLotID(string sBoxID)
        {
            try
            {
                ...
                dataRow["BOXID"] = sBoxID;

                dt.Rows.Add(dataRow);

                DataTable result = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_BOX_OUTER_NEW_SEQ", "INDATA", "RSLTDT", dt);

                if (result != null && result.Rows.Count > 0 && !string.IsNullOrEmpty(Util.NVC(result.Rows[0]["BOX_SEQ"])))
                {
                    int seq = (int)Convert.ToChar(result.Rows[0]["BOX_SEQ"]) + 1;

                    if (seq == 58)  // 1 ~9 까이후는 대문자 A부터 변환
                        seq = 65;
                    else if (seq == 91) // Z이후는 사용안함
                    {
                        Util.MessageValidation("SFU4054");  //ROLL구성용 신규BOXID를 발행할 수 없습니다.
                        return "";
                    }

                    seqNo = ((char)seq).ToString();

                    if (!string.IsNullOrEmpty(seqNo))
                        return sBoxID.Substring(0, 9) + seqNo;
                    else
                        Util.MessageValidation("SFU4054");  //ROLL구성용 신규BOXID를 발행할 수 없습니다.
                }
                else
