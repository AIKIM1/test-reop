/*************************************************************************************
 Created Date : 2024.04.03
      Creator : 이제섭
   Decription : 포장 Pallet 구성 (Tray) - Pallet 실적확정 PopUp
--------------------------------------------------------------------------------------
 [Change History]
...
{
    public partial class BOX001_328_CONFIRM : C1Window, IWorkArea
    {
        #region Declaration & Constructor 
        //private string _SavePalletBizName = "BR_SET_END_PALLET_BY_TRAY_BX";

        string _sRoute = string.Empty;
        string _sProcID = string.Empty;
        ...
        private void InitControl()
        {
            ...
            _sRoute = Util.NVC(tmps[0]);
            _sProcID = Util.NVC(tmps[1]);
            txtBoxQty.Text = "1";
            ...
            _dt = tmps[13] as DataTable;
            _sCstID = Util.NVC(tmps[14]);
        ...
        private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                inData["ROUTID"] = _sRoute;
                inData["CSTID"] = _sCstID;

                inDataTable.Rows.Add(inData);

                if (_dt != null)
                {
                    for (int lsCount = 0; lsCount < _dt.Rows.Count; lsCount++)
                    {
                        DataRow inDataCell = inDataCellTable.NewRow();
                        inDataCell["PACKCELLSEQ"] = Util.NVC_Decimal(_dt.Rows[lsCount]["SEQ"]);
                        inDataCell["SUBLOTID"] = Util.NVC(_dt.Rows[lsCount]["SUBLOTID"]);
                        inDataCell["LOTID"] = Util.NVC(_dt.Rows[lsCount]["LOTID"]);
                ...
                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_SET_END_PALLET_BY_TRAY_BX", "INDATA,INDATA_SUBLOT", "OUTDATA", indataSet);
