...
{
    public partial class Report_Packing_Assy : C1Window, IWorkArea
    {
            ...
            try
            {
                if (tmmp03.Rows.Count == 2)
                {
                    ...
                    else if (tmmp03.Rows[0]["TYPE"].ToString() == "SHIP")
                    {
                        ...
                        row["TO_AREAID"] = "";
                        row["TO_SLOC_ID"] = tmmp03.Rows[0]["TO_SLOC_ID"].ToString();
                        row["OUTBOXID"] = tmmp03.Rows[0]["OUTBOXID"].ToString();
                        ...
                        decimal total_qty = 0;
                        decimal total_qty1 = 0;
                        string iss_note = string.Empty;
                        string note = string.Empty;

                        for (int i = 0; i < tmmp03.Rows.Count; i++)
                        {
                            total_qty += Convert.ToDecimal(tmmp03.Rows[i]["TOTAL_QTY"].ToString());
                            total_qty1 += Convert.ToDecimal(tmmp03.Rows[i]["TOTAL_QTY2"].ToString());
                        }

                        //NOTE
                        string PackingRemark = string.Empty;                       
                        string m_gubun = string.Empty;
                        string inBoxTemp = string.Empty;
                        string lotTemp = string.Empty;

                        m_gubun = "-------------  -----------  ";
                        PackingRemark = m_gubun + PackingRemark;
                        PackingRemark = "   Box ID                 Lot ID\n" + PackingRemark + "\n";

                        for (int i = 0; i < tmmp04.Rows.Count; i++)
                        {                            
                            lotTemp = tmmp04.Rows[i]["LOTID"].ToString();
                            inBoxTemp = tmmp04.Rows[i]["INBOXID"].ToString();
                            PackingRemark = PackingRemark + inBoxTemp + "  " + lotTemp + "\n";
                        }

                        if(tmmp05.Rows.Count > 0 )
                        {
                            for (int i = 0; i < tmmp05.Rows.Count; i++)
                            {
                                lotTemp = tmmp05.Rows[i]["LOTID"].ToString();
                                inBoxTemp = tmmp05.Rows[i]["INBOXID"].ToString();
                                PackingRemark = PackingRemark + inBoxTemp + "  " + lotTemp + "\n";
                            }
                        }

                        row["ISS_NOTE"] = PackingRemark;
                        row["NOTE"] = PackingRemark;
                        row["TOTAL_QTY"] = total_qty;
                        row["TOTAL_QTY2"] = total_qty1;
                ...
                else
                {
                    ...
                    else if (tmmp03.Rows[0]["TYPE"].ToString() == "SHIP")
                    {
                        ...
                        {
                            DataRow row2 = inBox.NewRow();
                            row2["OUTBOXID"] = tmmp03.Rows[0]["PACKING_NO"].ToString();
