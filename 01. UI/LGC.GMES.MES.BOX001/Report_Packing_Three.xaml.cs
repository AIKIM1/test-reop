...
{
    public partial class Report_Packing_Three : C1Window, IWorkArea
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            tmmp06 = tmps[5] as DataTable; //dtSel03
            
            if (tmps.Length == 7)
            {
                tmmp07 = tmps[6] as DataTable;
            }
            ...
            using (Stream stream = a.GetManifestResourceStream("LGC.GMES.MES.BOX001.Report." + filename))
            {
                ...
                /// LotList 출력 추가 ///
                if (tmmp07 != null && tmmp07.Rows.Count > 0)
                {
                    ...
                    int AddRowCount = 10 - tmmp07.Rows.Count;

                    for (int i = 0; i < AddRowCount; i++)
                    {
                        DataRow NewRow = tmmp07.NewRow();
                        tmmp07.Rows.Add(NewRow);
                    }                    

                    for (int j = 0; j < tmmp07.Columns.Count; j++)
                    {
                        for (int i = 0; i < tmmp07.Rows.Count; i++)
                        {
                            string Col = tmmp07.Columns[j].ColumnName;
                            string ColumnName = Col + (i + 1);

                            if (cr.Fields.Contains(ColumnName))
                            {
                                cr.Fields[ColumnName].Text = Util.NVC(tmmp07.Rows[i][Col]);
                            }
                        }                        
                    }                    
                }                
        ...
        private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (tmmp03.Rows.Count == 2)
                {
                    ...
                    //    if (tmmp06 != null)
                    //        for (int i = 0; i < tmmp06.Rows.Count; i++)
                    //        {
                    //            DataRow row4 = inLot.NewRow();

                    //            row4["INBOXID"] = tmmp06.Rows[i]["INBOXID"].ToString();
                    //            row4["LOTID"] = tmmp06.Rows[i]["LOTID"].ToString();
                    //            row4["LOTQTY"] = tmmp06.Rows[i]["M_WIPQTY"].ToString();
                    //            row4["LOTQTY2"] = tmmp06.Rows[i]["CELL_WIPQTY"].ToString();

                    //            indataSet.Tables["INLOT"].Rows.Add(row4);
                    //        }
                ...
                else
                {
                    ...
                    else if (tmmp03.Rows[0]["TYPE"].ToString() == "SHIP")
                    {
                        ...
                        if (tmmp03 != null && !tmmp03.Rows[0]["PACK_NO1"].ToString().Equals("") && !tmmp03.Rows[0]["TOTAL_QTY2"].ToString().Equals("0"))
                        {
                            DataRow row2 = inBox.NewRow();
                        ...
                        if (tmmp03 != null && !tmmp03.Rows[0]["PACK_NO3"].ToString().Equals("") && !tmmp03.Rows[0]["TOTAL_QTY6"].ToString().Equals("0"))
                        {
                            DataRow row2 = inBox.NewRow();

                            row2["INBOXID"] = tmmp03.Rows[0]["PACK_NO3"].ToString();
                            row2["PRODID"] = tmmp03.Rows[0]["PRODID"].ToString();
                            row2["BOXTYPE"] = tmmp03.Rows[0]["PKGWAY"].ToString();
                            row2["BOXLAYER"] = "0";
                            row2["PACK_LOT_TYPE_CODE"] = "LOT";
                            row2["TOTAL_QTY"] = tmmp03.Rows[0]["TOTAL_QTY5"].ToString();
                            row2["TOTAL_QTY2"] = tmmp03.Rows[0]["TOTAL_QTY6"].ToString();
                            row2["EXP_DOM_TYPE_CODE"] = "E";
                            row2["OWMS_BOX_TYPE_CODE"] = tmmp03.Rows[0]["OWMS_BOX_TYPE_CODE"].ToString();
                        ...
                        if (tmmp06 != null)
                        {
                            for (int i = 0; i < tmmp06.Rows.Count; i++)
                            {
                                ...
                                row4["INBOXID"] = tmmp03.Rows[0]["PACK_NO3"].ToString();
                                row4["LOTID"] = tmmp06.Rows[i]["LOTID"].ToString();
                                row4["LOTQTY"] = tmmp06.Rows[i]["M_WIPQTY"].ToString();
                                row4["LOTQTY2"] = tmmp06.Rows[i]["CELL_WIPQTY"].ToString();
        ...
        private void SetDemandTypeForSkidID_Four()
        {
            try
            {
                tmmp07.Columns.Add("DEMAND_TYPE", typeof(string));

                for (int i = 0; i < tmmp07.Rows.Count; i++)
                {
                    if (!tmmp07.Rows[i]["SKID_ID"].ToString().Equals(""))
                    {
                        tmmp07.Rows[i]["DEMAND_TYPE"] = GetDemandTypeForSkidID(tmmp07.Rows[i]["SKID_ID"].ToString());
                    }
                }
            }
            catch (Exception ex)
