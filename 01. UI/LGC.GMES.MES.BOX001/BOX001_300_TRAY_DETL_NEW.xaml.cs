...
{
    public partial class BOX001_300_TRAY_DETL_NEW : C1Window, IWorkArea
    {
        ...
        public BOX001_300_TRAY_DETL_NEW()
        {
            InitializeComponent();
            Loaded += BOX001_300_TRAY_DETL_NEW_Loaded;
        }

        private void BOX001_300_TRAY_DETL_NEW_Loaded(object sender, RoutedEventArgs e)
        {
            Loaded -= BOX001_300_TRAY_DETL_NEW_Loaded;
        ...
        {
            int eRow = dgCELLInfo.SelectedIndex;
            if (dgCELLInfo.CurrentCell != null && dgCELLInfo.SelectedIndex > -1 && e.ChangedButton.ToString().Equals("Left"))
            {
                ...
                else if (dgCELLInfo.CurrentCell.Column.Name == "CELLID3")
                {
                    ...
                    // 활성화 BizActor에 연결해서 로직 수행한 후 다시 전극/조립 BizActor로 연결하는 함수 호출
                    FormationDBLink(sCellid, eRow, "CELLID3");
                }
            }
        }
        ...
        private void FormationDBLink(string sCellid, int eRow, string coNm)
        {
            ...
            for (int iRow = 0; iRow < dtOrigin.Rows.Count; iRow++)
            {
                ...
                if (coNm == "CELLID3")
                {
                    if (!string.IsNullOrEmpty(Util.NVC(dtOrigin.Rows[eRow]["CELLID3"])) && Util.NVC(dtOrigin.Rows[iRow]["CELLID3"]) == sCellid)
                    {
                        intoCell += 1;
                    }
                }
            ...
            for (int row = 0; row < dt.Rows.Count; row++)
            {
                cellList += "," + dt.Rows[row]["CELLID3"].ToString();
            }
            ...
            if (dnResult.Rows.Count > 0 && dsResult.Rows.Count > 0) // 시생산 cell과 시생산 아닌 cell 모두 존재하는 경우
            {
                ...
                {
                    DataTableConverter.SetValue(dgCELLInfo.Rows[eRow].DataItem, "CELLID3", "");
                }
        ...
        private void UpdateCellInformation()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dgCELLInfo.GetRowCount(); i++)
                {
                    ...
                    {
                        DataRow inDataSub = inDataSubTable.NewRow();
                        inDataSub["PSTN_NO"] = Util.NVC(dgCELLInfo.GetCell(i, dgCELLInfo.Columns["SEQ_NO3"].Index).Value);
                        inDataSub["SUBLOTID"] = Util.NVC(dgCELLInfo.GetCell(i, dgCELLInfo.Columns["CELLID3"].Index).Value);
                        inDataSubTable.Rows.Add(inDataSub);

                        FormationDBLink(Util.NVC(dgCELLInfo.GetCell(i, dgCELLInfo.Columns["CELLID3"].Index).Value), i, "SAVE");
                    }
                }
