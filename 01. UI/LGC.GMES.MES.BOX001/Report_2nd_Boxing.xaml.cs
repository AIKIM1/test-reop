...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2023.05.24  성민식 : E20230424-000075 ESNJ PACKINGLIST 출력 시 실제 OUTBOX 수량 REMARK에 표기
  2024.04.03  오수현 : E20240122-000144 [ESNJ] 고객에 따라 포장 목록의 기본 인쇄 매수를 MMD의 '포장/출하 조건(소형)'에 컬럼 추가하여 관리
...
{
    public partial class Report_2nd_Boxing : C1Window, IWorkArea
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                txtPrintQty.Value = Convert.ToInt32(tmps[2]); // E20240122-000144 - 인쇄 매수 MMD로 관리
            }
            else
            {
                txtPrintQty.Value = 1;// Defalut
            }
        ...
        private void txtPage_ValueChanged(int iPage)
        {
            try
            {
                ...
                using (Stream stream = a.GetManifestResourceStream("LGC.GMES.MES.BOX001.Report.Pallet_2nd_Boxing.xml"))
                {
                    ...
                    if (dtOutData != null && dtOutData.Rows.Count > 0)
                    {
                        for (int col = 0; col < dtOutData.Columns.Count; col++)
                        {
                            ...
                            else if (strColName.Equals("REAL_QTY"))
                            {
                                if(!String.IsNullOrEmpty(dtOutData.Rows[0]["REAL_QTY"].ToString()))
                                {
                                    cr.Fields["PACK_NOTE"].Text = "Actual Box Qty : " + dtOutData.Rows[0]["REAL_QTY"].ToString();
                                }
                            }
