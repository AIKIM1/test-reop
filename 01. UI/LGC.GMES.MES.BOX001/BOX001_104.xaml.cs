...
{
    public partial class BOX001_104 : UserControl, IWorkArea
    {
        ...
        private void btnShip_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                int row = _util.GetDataGridCheckFirstRowIndex(dgSearhResult, "CHK");
                ...
                string sBoxID = Util.NVC(dgSearhResult.GetCell(row, dgSearhResult.Columns["BOXID"].Index).Value);
                ...
                new ClientProxy().ExecuteService_Multi("BR_PRD_REG_SHIP_INPALLET_FM", "INBOX", "OUTDATA", (bizResult, bizException) =>
                {
                    try
                    {
                        ...
                        int iRow = _util.GetDataGridRowIndex(dgSearhResult, "RCV_ISS_ID", Util.NVC(bizResult.Tables["OUTDATA"].Rows[0]?["RCV_ISS_ID"]));
                        DataTableConverter.SetValue(dgSearhResult.Rows[iRow].DataItem, "CHK", true);
                    }
                    catch (Exception ex)
                    {
                        Util.MessageException(ex);
