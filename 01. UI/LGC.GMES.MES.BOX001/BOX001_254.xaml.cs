/*************************************************************************************
 Created Date : 2022.02.15
      Creator : 이제섭
   Decription : 오창 2산단 NFF Pallet 생성 화면
--------------------------------------------------------------------------------------
 [Change History]
  2022.02.15  이제섭 : Initial Created.
  2022.06.09  장희만 : ESNJ 요구사항 반영. Pallet 추가 구성 시 Mixline 확인 항목 추가
...
{
    public partial class BOX001_254 : UserControl, IWorkArea
    {
        ...
        private void btnOutboxAdd_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                dtInbox.Rows.Add(dr);
                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_INPUT_OUTBOX_NEW_NFF", "INDATA,INOUTBOX", "OUTDATA", ds);
                if (dsResult != null && dsResult.Tables.Count > 0 && dsResult.Tables["OUTDATA"].Rows.Count > 0)
                {
                    if(GetCompleteOutbox(txtOutBox.Text))
        ...
        private void btnRun_Click(object sender, RoutedEventArgs e)
        {
            ...
            BOX001_254_CREATEPALLET popup = new BOX001_254_CREATEPALLET();
        ...
        {
            BOX001_254_CREATEPALLET popup = sender as BOX001_254_CREATEPALLET;
            ...
            try
            {
                    ...
                    Util.MessageConfirm("SFU2802", (result) =>
                {
                    if (result == MessageBoxResult.OK)
                    {
                        ...
                        new ClientProxy().ExecuteService("BR_PRD_REG_SHIPMENT_NFF_FM", "INDATA", null, RQSTDT, (RSLTDT, ex) =>
                        {
                            try
                            {
                                loadingIndicator.Visibility = Visibility.Collapsed;
            ...
            try
            {
                ...
                Util.MessageConfirm("SFU2805", (result) =>
                {
                    if (result == MessageBoxResult.OK)
                    {
                        ...
                        new ClientProxy().ExecuteService("BR_PRD_REG_CANCEL_SHIPMENT_NFF_FM", "INDATA", null, RQSTDT, (RSLTDT, ex) =>
                        {
                            try
                            {
                                if (ex != null)
            ...
            Util.MessageConfirm("SFU1230", (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    try
                    {
                        ...
                        new ClientProxy().ExecuteService_Multi("BR_PRD_UNPACK_OUTBOX_NEW_NJ", "INDATA,INOUTBOX", null, (bizResult, bizException) =>
                        {
                            try
                            {
        ...
        private void btnOutboxMulti_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (puOutbox != null)
            {
                ...
                */
                ...
                Parameters[8] = Util.NVC(dgPalletList.GetCell(idxPallet, dgPalletList.Columns["PALLET_TYPE"].Index).Value);//2022.06.09
                Parameters[9] = Util.NVC(dgOutbox.GetCell(idxPallet, dgOutbox.Columns["PKG_LOTID"].Index).Value);          //2022.06.09
        ...
        private void AddOutBox(string boxID)
        {
            try
            {
                int idxPallet = _util.GetDataGridCheckFirstRowIndex(dgPalletList, "CHK");
                ...
                dr["BOXID"] = boxID;
                dtInbox.Rows.Add(dr);
                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_PACK_OUTBOX_NEW_NFF", "INPALLET,INOUTBOX", null, ds);
        ...
        private Boolean GetCompleteOutbox(string BoxID)
        {
            try
            {
                DataTable inTable = new DataTable();
