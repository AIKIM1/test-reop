...
--------------------------------------------------------------------------------------
 ...
 2023.10.11  E20230704-000395 IM: 27 bits, not IM: not 27 bits(Both two interfaces’ functions need to be modified)
 2023.10.16  E20230925-000286 : 이병윤 : MULTI_SHIPTO logic modify_다중출하처 체크박스 삭제
                                         Same OUTBOX ID 3-5 bit -> match supplier code based on INBOX length
                                         Different OUTBOX ID 3-5 bit -> check whether INBOX length is the same ->if is, allow Pallet generation, and matched supplier code is “B100”; if not ,alarm
 2024.04.17  E20240411-000120 : 오수현 : Non_IM Customer consistency check (두 번째 Outbox부터 시작하여 모든 SHIPTO_ID가 NULL이거나 첫번째 Outbox의 SHIPTO_ID와 동일한 지 체크)
                                       : Outbox의 SHIPTO_ID가 DBNull 값일 경우 에러 부분 수정
 **************************************************************************************/
...
namespace LGC.GMES.MES.BOX001
{
    ...
    /// </summary>
    public partial class BOX001_240_CREATEPALLET : C1Window, IWorkArea
    {
        Util _util = new Util();
        ...
        #region Initialize
        public BOX001_240_CREATEPALLET()
        {
            InitializeComponent();
        }
        ...
        private void btnInPalletDelete_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                else
                {
                    // 출하처
                    DataTable dtSource = DataTableConverter.Convert(dgInPallet.ItemsSource);
                    // Same OUTBOX ID 3-5 bit
                    var query = (from t in dtSource.AsEnumerable()
                                   select t.Field<string>("OUTBOXID").Substring(2, 3)).Distinct();
                    if (query.Count() == 1)
                    {
                        setShipToPopControl(dtSource.Rows[0]["PRODID"].ToString(), dtSource.Rows[0]["SHIP_TO"].ToString());
                    }
                    else
                    {
                        setShipToPopControl(dtSource.Rows[0]["PRODID"].ToString(), "B100");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void txtInPalletID_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                try
                {
                    ...
                    if((txtInPalletID.Text).Trim().Length == 27)
                    {
                        sOutBox = "IM";
                    } else
                    {
                    ...
                    //OUTBOX 체크
                    DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_INPUT_OUTBOX_MIX_NJ", "INDATA,INOUTBOX", "OUTDATA", inDataSet);
                    ...
                    if (dsResult.Tables["OUTDATA"] != null || dsResult.Tables["OUTDATA"].Rows.Count > 0)
                    {
                        ...
                        {
                            //GetCompleteOutbox(dsResult.Tables["OUTDATA"].Rows[0]["BOXID"].ToString(), sAssyLotLinemixFlag, sOutBox, (string)dr[0]["SHIPTO_ID"]);
                            GetCompleteOutbox(dsResult.Tables["OUTDATA"].Rows[0]["BOXID"].ToString(), sAssyLotLinemixFlag, sOutBox, Util.NVC(dr[0]["SHIPTO_ID"])); // E20240411-000120
                        }
        ...
        private void RunStart(string pAssyLotLinemixFlag)
        {
            ...
            try
            {
                ...
                if(popShipto.SelectedValue.ToString().Trim().Equals("B100"))
                {
                    newRow["MULTI_SHIPTO_FLAG"] = "Y";
                }
                else
                ...
                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_2ND_PALLET_NEW_NJ", "INDATA,INOUTBOX", "OUTDATA", inDataSet);
        ...
        private void GetCompleteOutbox(string BoxID, string pAssyLotLinemixFlag, string OutBox, string ShipTo)
        {
            try
            {
                ...
                newRow["BOXID"] = BoxID;
                newRow["LANGID"] = LoginInfo.LANGID;
                newRow["MULTI_SHIPTO_FLAG"] = "N"; // 체크박스 삭제, 기본값:'N'
        ...
        private bool iMShipto(String boxId = null)//.GroupBy(x => x.GroupName)
        {
            DataTable dtSource = DataTableConverter.Convert(dgInPallet.ItemsSource);

            // Same OUTBOX ID 3-5 bit
            var query01 = (from t in dtSource.AsEnumerable()
                         select t.Field<string>("OUTBOXID").Substring(2, 3) ).Distinct();

            // INBOX length is the same
            var query02 = (from t in dtSource.AsEnumerable()
                           select t.Field<string>("INBOXID").Trim().Length).Distinct();
            ...
            if (query01.Count() == 1)
            {
                ...
                if(query02.Count() > 1)
                {
                    // match supplier code based on INBOX length
                    // different INBOX type, please check again
                    Util.MessageValidation("SFU8921");
                    dtSource.Rows.Remove(dtSource.Select("OUTBOXID = '" + boxId + "'")[0]);
                    Util.GridSetData(dgInPallet, dtSource, FrameOperation, false);
                    setShipToPopControl(dtSource.Rows[0]["PRODID"].ToString(), dtSource.Rows[0]["SHIP_TO"].ToString());
                    return false;
                }
