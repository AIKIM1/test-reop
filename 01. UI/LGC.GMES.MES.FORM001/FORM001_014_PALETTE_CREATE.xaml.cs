...
{
    public partial class FORM001_014_PALETTE_CREATE : C1Window, IWorkArea
    {
        #region Declaration
        ...
        public FORM001_014_PALETTE_CREATE()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private void SetControl()
        {
            ...
            // Pallet 구분
            SetCommCode(cboQultType, "WIP_QLTY_TYPE_CODE", "N");
            ...
            // 용량구분 : 전압검사는 E등급만 표시
            //////SetCommCode(cboCapaType, "CAPA_GRD_CODE", null, "G");
            //string[] sFilterCapa = { LoginInfo.CFG_AREA_ID, "CAPA_GRD_CODE", "N" };
            //_combo.SetCombo(cboCapaType, CommonCombo.ComboStatus.NONE, sCase: "FORM_GRADE_TYPE_CODE", sFilter: sFilterCapa);
            string[] sFilterCapa = { LoginInfo.CFG_AREA_ID, LoginInfo.CFG_EQSG_ID, "N" };
            _combo.SetCombo(cboCapaType, CommonCombo.ComboStatus.NONE, sCase: "FORM_GRADE_TYPE_CODE_LINE", sFilter: sFilterCapa);

            DataTable dtCapa = DataTableConverter.Convert(cboCapaType.ItemsSource);
            dtCapa.Select("CBO_CODE <> 'E'").ToList<DataRow>().ForEach(row => row.Delete());
            cboCapaType.ItemsSource = dtCapa.Copy().AsDataView();
            if (dtCapa.Rows.Count > 0)
                cboCapaType.SelectedIndex = 0;
        ...
        private void SetCreatePallet()
        {
            try
            {
                ...
                const string bizRuleName = "BR_PRD_REG_CREATE_PALLET_VT";
