...
--------------------------------------------------------------------------------------
 [Change History]
   2022.05.07   손우석   : 2022-05-17 운영소스
   2023.12.26   오수현   : E20231129-000989 1) 장치 종류별로 ETW에 해당하는 W/O를 표시
                           2) 마감일 기준 체크박스 기능 추가 - 종료일에 대한 매월 1일 08:00의 W/O는 전월에 속함(예: 10/31 8:00부터 11/1 8:00까지의 W/O는 표시되지 않음)
**************************************************************************************/
...
{
    public partial class FORM001_005_RUNSTART : C1Window, IWorkArea
    {
        #region Declaration
        Util _Util = new Util();
        ...
        private void SetWorkOrder()
        {
            try
            {
                ...
                DataTable inTable = _bizRule.GetDA_PRD_SEL_WORKORDER_LIST_BY_LINE();
                ...
                // 마감일 기준
                if ((bool)chkDueDate.IsChecked)
                    newRow["STDT"] = dtpWOYM.SelectedDateTime.ToString("yyyyMM") + "02";
                else
                    newRow["STDT"] = dtpWOYM.SelectedDateTime.ToString("yyyyMM") + "01";
                newRow["EDDT"] = day.ToString("yyyyMMdd");
                newRow["PROC_EQPT_FLAG"] = "EQPT";
        ...
        private void StartRunProcess()
        {
            try
            {
                ...
                const string bizRuleName = "BR_PRD_REG_START_PROD_LOT_TW";
        ...
        private bool ValidateStartRun()
        {
            ...
            {
                // 선택한 W/O의 판매구분과 조립 LOT의 판매 구분이 틀립니다.
