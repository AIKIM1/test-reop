...
{
    public partial class FORM001_003_PALETTE_CREATE : C1Window, IWorkArea
    {
        #region Declaration
        ...
        public FORM001_003_PALETTE_CREATE()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private void SetControl()
        {
            ...
            // Pallet 구분
            SetPalletGubun();
            ////SetCommCode(cboQultType, "WIP_QLTY_TYPE_CODE", "G");
            ...
            string[] sFilterCapa = { LoginInfo.CFG_AREA_ID, LoginInfo.CFG_EQSG_ID, "G" };
            _combo.SetCombo(cboCapaType, CommonCombo.ComboStatus.NONE, sCase: "FORM_GRADE_TYPE_CODE_LINE", sFilter: sFilterCapa);
            ...
            else
            {
                ...
                cboQultType.IsEnabled = false;
                if (cboQultType.SelectedValue.ToString().Equals("G"))
                {
                    dgPallet.Columns["CAPA_GRD_CODE"].Visibility = Visibility.Collapsed;
                    dgPallet.Columns["VLTG_GRD_CODE"].Visibility = Visibility.Visible;
                }
                else
                {
                    dgPallet.Columns["CAPA_GRD_CODE"].Visibility = Visibility.Visible;
            ...
            cboQultType.SelectedValueChanged += cboQultType_SelectedValueChanged;
        ...
        private void SetControlVisibility()
        {
            ...
            ////else
            ////{
            ////    tbInboxType.Visibility = Visibility.Collapsed;
            ////    cboInboxType.Visibility = Visibility.Collapsed;
            ////}
            tbShipping.Visibility = Visibility.Collapsed;
        ...
        #region [Pallet 구분 변경 양불]
        private void cboQultType_SelectedValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            SetPalletCreateGrid();
        }
        #endregion
        ...
        /// 양품 생성 Pallet 그리드 조회
        /// </summary>
        private void SetGridGoodCreatePallet()
        {
            try
            {
                DataTable inTable = new DataTable();
        ...
        /// 불량 생성 Pallet 그리드 조회
        /// </summary>
        private void SetGridDefectCreatePallet()
        {
            try
            {
                DataTable inTable = new DataTable();
                ...
                newRow["AREAID"] = LoginInfo.CFG_AREA_ID;
                newRow["EQSGID"] = LoginInfo.CFG_EQSG_ID;
                newRow["QLTY_TYPE_CODE"] = "N";

                inTable.Rows.Add(newRow);

                DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_PALLET_CAPA_CREATE_CELL_FO", "INDATA", "OUTDATA", inTable);
        ...
        private void SetGridModifyPallet()
        {
            try
            {
                ...
                DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_PALLET_CAPA_MODIFY_FO", "INDATA", "OUTDATA", inTable);
        ...
        private void SetCreatePallet()
        {
            try
            {
                ...
                const string bizRuleName = "BR_PRD_REG_CREATE_PALLET_GS";
                ...
                foreach (DataGridRow dRow in dgPallet.Rows)
                {
                    ...
                    if (Util.NVC_Int(DataTableConverter.GetValue(dRow.DataItem, "CELL_QTY")) != 0)
                    {
                        ...
                        if (cboQultType.SelectedValue.ToString().Equals("G"))
                        {
                            // 양품일 경우 CAPA_GRD_CODE = "A"
                            newRow["CAPA_GRD_CODE"] = cboCapaType.SelectedValue.ToString();
        ...
        private void SetModifyPallet()
        {
            try
            {
                ...
                inTable.Columns.Add("REASONID", typeof(string));
                inTable.Columns.Add("WIPNOTE", typeof(string));
                inTable.Columns.Add("USERID", typeof(string));
                inTable.Columns.Add("SHIFT", typeof(string));
                inTable.Columns.Add("WRK_USERID", typeof(string));
        ...
        #region [Func]
        private void SetPalletCreateGrid()
        {
            if (cboQultType.SelectedValue == null)
                return;

            if (cboQultType.SelectedValue.ToString().Equals("G"))
            {
                // 양품인 경우 전압등급 표시
                SetGridGoodCreatePallet();
            }
            else
            {
                // 불량인 경우 용량 등급 표시
                SetGridDefectCreatePallet();
            }
