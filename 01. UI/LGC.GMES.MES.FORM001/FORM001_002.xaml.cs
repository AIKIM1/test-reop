...
{
    public partial class FORM001_002 : IWorkArea
    {
        #region Declaration
        ...
        private void SetEventInUserControls()
        {
            if (UcFormCommand != null)
            {
                ...
                UcFormCommand.ButtonInspectionNew.Click += ButtonInspectionNew_Click;
        ...
        private void ButtonInspection_Click(object sender, RoutedEventArgs e)
        {
            ...
            int selectedRowIndex = _util.GetDataGridFirstRowIndexWithTopRow(DgProductLot, "CHK");
            DataTable dtSelfInspLatestLot = null;

            if (selectedRowIndex < 0)
            {
                //선택된 LOT 이 없을 경우 작업일 기준 오늘 가장 최근에 입력한 LOT 조회
                dtSelfInspLatestLot = GetSelfInspLatestLot();

                if (dtSelfInspLatestLot == null || dtSelfInspLatestLot.Rows.Count <= 0)
                {
                    // 생산 Lot 정보가 없습니다.
                ...
                {
                    if (String.IsNullOrEmpty(dtSelfInspLatestLot.Rows[0]["LOTID"].ToString()) || String.IsNullOrEmpty(dtSelfInspLatestLot.Rows[0]["WIPSEQ"].ToString()))
                    {
                        // 생산 Lot 정보가 없습니다.
                        Util.MessageValidation("SFU4014");
                        return;
            ...
            FORM001_QUALITY popupQualty = new FORM001_QUALITY { FrameOperation = FrameOperation };

            if (ValidationGridAdd(popupQualty.Name.ToString()) == false) {
                return;
            }

            int iRow = _util.GetDataGridFirstRowIndexWithTopRow(DgProductLot, "CHK");

            popupQualty.ShiftID = Util.NVC(UcFormShift.TextShift.Tag);
            popupQualty.ShiftName = Util.NVC(UcFormShift.TextShift.Text);
            popupQualty.WorkerID = Util.NVC(UcFormShift.TextWorker.Tag);
            popupQualty.WorkerName = Util.NVC(UcFormShift.TextWorker.Text);
            popupQualty.ShiftDateTime = Util.NVC(UcFormShift.TextShiftDateTime.Text);
            ...
            if(selectedRowIndex < 0){
                parameters[4] = dtSelfInspLatestLot.Rows[0]["LOTID"].ToString();
                parameters[5] = dtSelfInspLatestLot.Rows[0]["WIPSEQ"].ToString();
            }
            else
            {
            ...
            popupQualty.Closed += new EventHandler(popupQualty_Closed);
        ...
        {
            FORM001_QUALITY popup = sender as FORM001_QUALITY;
        ...
        private void ButtonInspectionNew_Click(object sender, RoutedEventArgs e)
        {
            ...
            popupQualty.Closed += new EventHandler(popupQualtyNew_Closed);
            grdMain.Children.Add(popupQualty);
            popupQualty.BringToFront();
        }
        ...
        {
            _processCode = Process.CircularCharacteristic;
            ...
            else
            {
                ...
                if (rowIndex >= 0)
                {
                    GetProductLot(false, DataTableConverter.GetValue(DgProductLot.Rows[rowIndex].DataItem, "LOTID").GetString());
                }
            }
        ...
        private void ButtonStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            FORM001_002_RUNSTART popupRunStart = new FORM001_002_RUNSTART { FrameOperation = FrameOperation };
        ...
        {
            FORM001_002_RUNSTART popup = sender as FORM001_002_RUNSTART;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                ClearControls();
        ...
        private void ButtonCompletion_Click(object sender, RoutedEventArgs e)
        {
            ...
            FORM001_002_CONFIRM popupConfirm = new FORM001_002_CONFIRM { FrameOperation = FrameOperation };
        ...
        {
            FORM001_002_CONFIRM popup = sender as FORM001_002_CONFIRM;
        ...
        private void SetPopupGoodPalletCreate()
        {
            ...
            FORM001_002_PALETTE_CREATE popupPalletCreate = new FORM001_002_PALETTE_CREATE { FrameOperation = this.FrameOperation };
        ...
        private void SetPopupGoodPalletEdit()
        {
            ...
            FORM001_002_PALETTE_CREATE popupPalletEdit = new FORM001_002_PALETTE_CREATE { FrameOperation = this.FrameOperation };
        ...
        {
            FORM001_002_PALETTE_CREATE popup = sender as FORM001_002_PALETTE_CREATE;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                GetProductLot(true);
                UcFormProductionPallet.SelectPalletList();
        ...
        private DataTable GetSelfInspResult()
        {
            ...
            try
            {
                ...
                string sBizName = "BR_PRD_CHK_SELF_INSP_CM";
                ...
                newRow["SHOPID"] = LoginInfo.CFG_SHOP_ID;
                newRow["EQSGID"] = LoginInfo.CFG_EQSG_ID;
                newRow["SHFT_ID"] = Util.NVC(UcFormShift.TextShift.Tag);
                inTable.Rows.Add(newRow);
        ...
        private bool ValidationCompletion()
        {
            ...
            if (string.IsNullOrEmpty(Util.NVC(UcFormShift.TextShift.Tag)))
            {
                // 작업조를 선택하세요.
            ...
            DataTable dtResult = GetSelfInspResult();

            if (dtResult != null && dtResult.Rows.Count > 0)
            {
                if ("N".Equals(dtResult.Rows[0]["INSP_RESULT"].ToString()))
                {
                    return false;
                }
            }
        ...
        private bool ValidationPalletHold()
        {
            ...
            DataRow[] drChk = DataTableConverter.Convert(DgProductionPallet.ItemsSource).Select("CHK = 1 AND (RCV_ISS_STAT_CODE = 'SHIPPING' OR RCV_ISS_STAT_CODE = 'FINISH_RECEIVE')");

            if (drChk.Length > 0)
            {
                // 출고완료, 입고완료 팔레트는 Hold 할 수 없습니다.
                Util.MessageValidation("SFU4063");
                return false;
            }
        ...
        private bool ValidationPalletEdit()
        {
            ...
            // 출고중, 출고완료는 팔레트 수정, 삭제 불가
            DataRow[] drChk = DataTableConverter.Convert(DgProductionPallet.ItemsSource).Select("CHK = 1 AND (RCV_ISS_STAT_CODE = 'SHIPPING' OR RCV_ISS_STAT_CODE = 'FINISH_RECEIVE')");

            if (drChk.Length > 0)
            {
                // 출고중, 출고완료 팔레트는 수정할 수 없습니다.
                Util.MessageValidation("SFU4034");
                return false;
            }
        ...
        private bool ValidationPalletDelete()
        {
            ...
            if (drChk.Length > 0)
            {
                // 출고중, 출고완료 팔레트는 삭제할 수 없습니다.
                Util.MessageValidation("SFU4035");
                return false;
            }
        ...
        private DataTable GetSelfInspLatestLot()
        {
            DataTable dtResult = null;
            ...
            try
            {
                ...
                string sBizName = "DA_QCA_SEL_SELF_INSP_CLCTVAL_LATEST";
                ...
                dtResult = new ClientProxy().ExecuteServiceSync(sBizName, "RQSTDT", "RSLTDT", inTable);
            }
            catch (Exception ex)
            {
                Util.MessageException(ex);
