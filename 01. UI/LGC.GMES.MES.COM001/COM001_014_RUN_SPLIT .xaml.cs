...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2024.01.30 김대현 설비 Loss 등록/수정 통합
  2024.03.19 김대현 공용 PC 사용자로 비조업 Split 등록시 Validation
  2024.10.24 이지은 MES 리빌딩 PJT : 앞 화면에서 받아온 TRANSACTION_SERIAL_NO 파라미터 추가
**************************************************************************************/
...
{
    public partial class COM001_014_RUN_SPLIT : C1Window, IWorkArea
    {
        ...
        string endTime = "";
        string _END_DTTM = "";
        bool _isTab_Main = false;
        string _TRAN_SEQ = ""; //MES 리빌딩 PJT
        ...
        string _WORK_DATE = "";
        COM001_014_Tab_Main win_Tab_Main = null;
        COM001_014_Tab_Unit win_Tab_Unit = null;
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (tmps != null && tmps.Length >= 1)
            {
                ...
                _TRAN_SEQ = Util.NVC(tmps[11]);

                if (tmps[9].Equals("Main"))
                {
                    _isTab_Main = true;
                    win_Tab_Main = tmps[6] as COM001_014_Tab_Main;
                }
                else
                {
                    _isTab_Main = false;
                    win_Tab_Unit = tmps[6] as COM001_014_Tab_Unit;
                }
                _EQSGID = Util.NVC(tmps[7]);
                _PROCID = Util.NVC(tmps[8]);
                occurEqptFlag = Util.NVC(tmps[10], "N"); // 2023.06.05 윤지해 CSR ID E20230330-001442 추가
            }
        ...
        private void initCombo()
        {
            try
            {
                ...
                //string[] sFilter3 = { "popup" }; // 2023.06.05 주석처리
                //combo.SetCombo(cboLoss, CommonCombo.ComboStatus.SELECT, cbChild: cboLossChild, sFilter:sFilter3); // 2023.06.05 주석처리
        ...
        private bool validationAddRow(string starttime, string endtime)
        {
            if (!cboLoss.SelectedValue.Equals("RUN"))
            {
                ...
                // 부동내용 run일 경우 -select- 가능
                if (!string.IsNullOrEmpty(cboLoss.Text) && !cboLoss.Text.Equals("RUN") && string.IsNullOrEmpty(cboLossDetl.Text))
                {
                    return false;
                }
                ...
                if (chkUpperLossCode)
                {
                    ...
                    new ClientProxy().ExecuteService_Multi("BR_EQPT_EQPTLOSS_RUN_SPLT", "INDATA, INLOSS", null, (bizResult, bizException) =>
                    {
                        try
                        {
                            ...
                            if (_isTab_Main)
                                win_Tab_Main.btnSearch_Click(null, null);
                            else
                                win_Tab_Unit.btnSearch_Click(null, null);
