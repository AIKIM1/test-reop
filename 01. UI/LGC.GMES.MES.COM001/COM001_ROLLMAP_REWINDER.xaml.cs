/*************************************************************************************
 Created Date : 2022.03.29
      Creator : 신광희
   Decription : 자동차 전극 Roll Map - 롤맵 차트(REWINDER)
--------------------------------------------------------------------------------------
 [Change History]
  2022.03.29  신광희 : Initial Created
  ...
  2024.01.19  신광희 : Input 영역 Tag Section Y좌표 위치 수정
...
namespace LGC.GMES.MES.COM001
{
    ...
    public partial class COM001_ROLLMAP_REWINDER : C1Window, IWorkArea
    {
        ...
        public COM001_ROLLMAP_REWINDER()
        {
            InitializeComponent();
        }
            ...
            if (query.Any())
            {
                foreach (var item in query)
                {
                    var queryLane = dt.AsEnumerable().Where(s => s.Field<string>("LANE_NO_CUR") == item.LaneNo && s.Field<string>("LOC2") != "XX").GroupBy(b => b.Field<string>("LANE_NO_CUR"))
                        ...
                        {
                            LaneNo = c.Key,
                            FirstLaneRate = c.Min(z => z.Field<decimal>("Y_PSTN_STRT_CUM_RATE")),
                            LastLaneRate = c.Max(z => z.Field<decimal>("Y_PSTN_END_CUM_RATE")),
                        }).FirstOrDefault();
                ...
                foreach (var item in queryTopBack)
                {
                    var queryTopBackRate = dt.AsEnumerable()
                        ...
                        .Select(s => new
                        {
                            LaneNo = s.Key.LaneNo,
                            TopBackFlag = s.Key.TopBackFlag,
                            FirstTopBackRate = s.Min(z => z.Field<decimal>("Y_PSTN_STRT_RATE")),
                            LastTopBackRate = s.Max(z => z.Field<decimal>("Y_PSTN_END_RATE"))
                        }).FirstOrDefault();
        ...
        private DataTable MakeTableForDisplay(DataTable dt, ChartDisplayType chartDisplayType, ChartConfigurationType chartConfigurationType)
        {
            ...
            else if (chartDisplayType == ChartDisplayType.TagSectionStart || chartDisplayType == ChartDisplayType.TagSectionEnd)
            {
                ...
                foreach (DataRow row in dtBinding.Rows)
                {
                    if(chartConfigurationType == ChartConfigurationType.Input)
                    {
                        row["SOURCE_Y_PSTN"] = chartDisplayType == ChartDisplayType.TagSectionStart ? -16 : -21;
                    }
                    else
                    {
