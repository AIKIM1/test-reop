/*************************************************************************************
 Created Date : 2024.03.19
      Creator : 이재우
   Decription : 롤프레스 롤맵 팝업(Roll Map 2.0) - 자동차 리와인더 롤맵과 통합 작업 필요.
--------------------------------------------------------------------------------------
 [Change History]
  2024.03.19  이재우 : Initial Created
  2024.08.27  조성근 : [E20240827-000372] - 롤프레스 롤맵 수불적용
  2024.10.25  신광희 : JSON 결과값 Deserialize 시 Data Type 비정상으로 Linq 사용 시 Data Type 부분 수정
  2025.05.29  이호승 : 롤프레스 롤맵 아웃피드 스크랩 추가
...
namespace LGC.GMES.MES.COM001
{
    ...
    /// 롤프레스 롤맵 팝업
    /// </summary>
    public partial class COM001_RM_CHART_RP : C1Window, IWorkArea
    {
        #region Variables
        ...
        private bool isShowDefectButton = false;
        /// <summary>
        /// INPUT LOT 기준 길이
        /// </summary>
        ...
        /// <summary>
        /// 투입랏(코터랏) 차트 MIN/MAX 툴팁표시 대상설비 여부
        /// </summary>
        ...
        private void InitSetting()
        {
            ...
            IsShowDefectButton(); //조성근 2024.08.28 롤프레스 롤맵 수불 적용
            ...
            if (wParams.processCode == Process.ROLL_PRESSING)
            {
                ...
                // 롤프레스 공정은 롤맵2.0 실적 적용 대상이 현재 아님(2024.11.04) 구간불량 등록은 현재 Visibility.Collapsed 로 지정
        ...
        private void InitializeControls()
        {
            ...
            if (isRollMapResultLink && isRollMapLot)
            {
                btnRollMapPositionEdit.Visibility = Visibility.Collapsed;
            }
            else
            ...
            btnDefect.Visibility = isShowDefectButton == true ? Visibility.Visible : Visibility.Collapsed;
        ...
        #region DrawInputTopBackDisplay
        /// <summary>
        /// Input Chart T(Top) / B(Back) 표시(TOP_BACK_REVS_FLAG(Y)이면 반대로 표시)
        /// </summary>
        /// <param name="dt"></param>
        private void DrawInputTopBackDisplay(DataTable dt)
        {
            //TOP_BACK_REVS_FLAG(Y)이면 반대로 표시
            var query = (from t in dt.AsEnumerable()
        ...
        private DataTable MakeTableForDisplay(DataTable dt, ChartDisplayType chartDisplayType, ChartConfigurationType chartConfigurationType)
        {
                            ...
                            foreach (var item in queryBarcodeInfoList)
                            {
                                ...
                                else
                                {
                                    row["TOOLTIP"] = row["EQPT_MEASR_PSTN_NAME"].GetString()
                                                   ...
                                                   + "\n"
                                                   + "[" + $"{ (row["SOURCE_ADJ_END_PSTN"].GetDecimal() - row["SOURCE_ADJ_STRT_PSTN"].GetDecimal()):###,###,###,##0.##}"
                                                   + "m"
                                                   + ", "
            ...
            else if (chartDisplayType == ChartDisplayType.TagSectionLaneStart || chartDisplayType == ChartDisplayType.TagSectionLaneEnd)
            {
                ...
                foreach (DataRow row in dtBinding.Rows)//전체 데이터 대상중에서, NFF 2D 바코드 정보가 있는 경우에 추가적인 정보 변경작업
                {
                    row["SOURCE_Y_PSTN"] = chartDisplayType == ChartDisplayType.TagSectionLaneStart ? -22 : -31;
                    row["TAG"] = $"{row["SOURCE_ADJ_STRT_PSTN"]:###,###,###,##0.##}"
                               + ";"
                               + $"{row["SOURCE_ADJ_END_PSTN"]:###,###,###,##0.##}"
                               + ";"
        ...
        private void DrawDefectMark(DataTable dt, ChartConfigurationType chartConfigurationType)
        {
            ...
            var queryLabel = dt.AsEnumerable().Where(x => x.GetValue("CHART_Y_STRT_CUM_RATE") != null && x.GetValue("CHART_Y_STRT_CUM_RATE").ToString() != null
                                                       && x.GetValue("CHART_Y_END_CUM_RATE") != null && x.GetValue("CHART_Y_END_CUM_RATE") != null
        ...
        private void DrawDefect(DataTable dt, ChartConfigurationType chartConfigurationType, bool isReDraw = false)
        {
            ...
            if (!isReDraw)
            {
                ...
                #region 4-1.TAG_SECTION_SINGLE => DrawDefectTagSectionSingle
        ...
        private void DrawThickness(DataTable dt, ChartConfigurationType chartConfigurationType)
        {
            ...
            if (CommonVerify.HasTableRow(dt))
            {
                ...
                DataTable dtLegend = dtLineLegend.Select().AsEnumerable().OrderByDescending(o => o.Field<int>("NO")).CopyToDataTable();
        ...
        private void SelectRollMap()
        {
            ...
            if (GetCboUserConf(ref mPoint_top, ref mPstn_top, ref mPoint_back, ref mPstn_back) == false)
            {
                ...
                if (CommonVerify.HasTableRow(dtResult))
                {
                    //W : 로딩량(Wet), D : 로딩량(Dry), T : 두께
                    //WET(로딩량) : WL => "WL".Substring(1, 1) > L 인 경우 mPoint_top 1, mPoint_back 1 L 아닌경우 mPoint_top 2, mPoint_back 2
                    //                    "WL".Substring(0, 1) > D 인 경우 mPstn_top 2, mPstn_back 2 D 아닌 경우 mPstn_top 1, mPstn_back 1
                    //DRY(로딩량) : DL => "DL".Substring(1, 1) > L 인 경우 mPoint_top 1, mPoint_back 1 L 아닌경우 mPoint_top 2, mPoint_back 2  
                    //                    "DL".Substring(0, 1) > D 인 경우 mPstn_top 2, mPstn_back 2 D 아닌 경우 mPstn_top 1, mPstn_back 1
                    // T 인경우 WET(두께) 로 간주 함. (기존 하드 코딩 제거를 위함.)
                    //WET(두께)   : WT => "WT".Substring(1, 1) > L 인 경우 mPoint_top 1, mPoint_back 1 L 아닌경우 mPoint_top 2, mPoint_back 2 
                    //                    "WT".Substring(0, 1) > D 인 경우 mPstn_top 2, mPstn_back 2 D 아닌 경우 mPstn_top 1, mPstn_back 1
                    ...
                    else if (string.Equals("D", dtResult.Rows[0]["ATTR1"].GetString()))
                    {
                        mPoint_top = "1";
                        mPoint_back = "1";
                        mPstn_top = "2";
                    ...
                    {
                        //if (string.Equals("T", dtResult.Rows[0]["ATTR1"].GetString()))
                        mPoint_top = "2";
                        mPstn_top = "1";
                        mPoint_back = "2";
                        mPstn_back = "1";
        ...
        private void SetUseScrapSection()
        {
            try
            {
                ...
                if (isRollMapResultLink == true && isRollMapLot == true) return;
        ...
        private void popScrapSection(string strClctSeqNo = "")
        {
            ...
            if (btnScrapSection.Visibility == Visibility.Collapsed) strMode = "1";
        ...
        private void IsShowDefectButton()
        {
            ...
            if (CommonVerify.HasTableRow(dtResult))
            {
                isShowDefectButton = true;
            }
            else
            {
                isShowDefectButton = false;
            }
        }
        ...
        {
            CMM_RM_RP_DATACOLLECT popup = sender as CMM_RM_RP_DATACOLLECT;
