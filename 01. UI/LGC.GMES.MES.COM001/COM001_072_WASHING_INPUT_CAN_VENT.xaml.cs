/*************************************************************************************
 Created Date : 2023.12.05
      Creator : 배현우
   Decription : CAN 등록 (ASSY006_WASHING_INPUT_CAN 참조)
--------------------------------------------------------------------------------------
 [Change History]
  2023.12.05  배현우 : 최초 생성
...
{
    public partial class COM001_072_WASHING_INPUT_CAN_VENT : C1Window, IWorkArea
    {
        ...
        public COM001_072_WASHING_INPUT_CAN_VENT()
        {
            InitializeComponent();
        }
        ...
        private void btnAdd_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (rowCount.Value + dgList.GetRowCount() > 144)
                {
                    object[] parameters = new object[1];
                    parameters[0] = 144;
                    Util.MessageValidation("SFU4573",parameters[0]);
                    return;
                }
            }
        ...
        private void txtCANID_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(txtCANID.Text)) return;
                ...
                if (e.Key == Key.Enter)
                {
                    ...
                    dr["SUBLOTID"] = txtCANID.Text.Trim();
                    dr["VENTID"] = txtVENTID.Text.Trim();
        ...
        private void txtVENTID_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(txtVENTID.Text)) return;
                ...
                if (e.Key == Key.Enter)
                {
                    ...
                    dgList.ScrollIntoView(dt.Rows.Count, dgList.Columns["VENTID"].Index);
        ...
        private void dgList_BeginningNewRow(object sender, DataGridBeginningNewRowEventArgs e)
        {
            ...
            e.Item.SetValue("VENTID", string.Empty);
        }
        ...
        private void btnDownLoad_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (od.ShowDialog() == true)
                {
                    ...
                    sheet[0, 0].Value = "CANID";
                    sheet[0, 1].Value = "VENTID";
                    sheet[0, 2].Value = "Tray 위치";
            ...
            try
            {
                ...
                for (int rowInx = 1; rowInx < sheet.Rows.Count; rowInx++)
                {
                    ...
                    if (sheet.GetCell(rowInx, 1) == null)
                        ...
                        dr["VENTID"] = Util.NVC(sheet.GetCell(rowInx, 1).Text);

                    if (sheet.GetCell(rowInx, 2) == null)
                        dr["CSTSLOT"] = "";
                    else
                        dr["CSTSLOT"] = Util.NVC(sheet.GetCell(rowInx, 2).Text);
        ...
        private bool IsValid()
        {
            ...
            foreach (var item in subLotInfo)
            {
                ...
                {
                    //중복된 CAN ID가 존재합니다.
                    Util.MessageValidation("SFU4574");
                    return false;
                }
            }
            ...
            var ventInfo =
                from t in dt.AsEnumerable()
                group t by t.Field<string>("VENTID") into g
                select new { VentId = g.Key, VentCount = g.Count() };

            foreach (var item in ventInfo)
            {
                if (item.VentCount > 1)
                {
                    //중복된 VENT ID가 존재합니다.
                    Util.MessageValidation("SFU4575");
                    return false;
                }
            }
            ...
            if (query.Any())
            {
                foreach (var item in query)
                {
                    ...
                    if (Convert.ToInt16(item["CSTSLOT"].GetString()) > 144)
                    {
            ...
            try
            {
                Util.MessageConfirm("SFU1241", (result) =>
                {
                    if (result == MessageBoxResult.OK)
                    {
                        ...
                        string bizRuleName = "BR_PRD_REG_PUT_SUBLOT_IN_CST_WS_CANID_V2";
                        ...
                        {
                            if ((!string.IsNullOrEmpty(DataTableConverter.GetValue(dgList.Rows[row].DataItem, "SUBLOTID").ToString()) || !string.IsNullOrEmpty(DataTableConverter.GetValue(dgList.Rows[row].DataItem, "VENTID").ToString())) || string.IsNullOrEmpty(DataTableConverter.GetValue(dgList.Rows[row].DataItem, "CSTSLOT").ToString()))
                            {
                                drow = inCST.NewRow();
                                ...
                                drow["VENTID"] = DataTableConverter.GetValue(dgList.Rows[row].DataItem, "VENTID").ToString().Trim();
        ...
        public DataSet GetBR_PRD_REG_START_OUT_LOT_WSS()
        {
            ...
            inInputLot.Columns.Add("VENTID", typeof(string));
