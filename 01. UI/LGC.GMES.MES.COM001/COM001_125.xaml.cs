...
--------------------------------------------------------------------------------------
 [Change History]
  2017.12.15  DEVELOPER : Initial Created.
                          COM001_011 에서 분할 처리/활성화 후공정에 필요한 항목이 추가 되어 별도 구성함.
  ...
  2025.05.27  김영택      NERP 대응               제외/제외취소/선택재고변경 시 차수마감여부 재조회 하도록 수정 (2025.06.23 git 머지 )
**************************************************************************************/
...
{
	public partial class COM001_125 : UserControl, IWorkArea
	{
		...
		private void btnExclude_SNAP_Click(object sender, RoutedEventArgs e)
		{
            ...
            // 2025.05.27 조회된 데이터의 MES 재고실사 마감여부, NERP 회계마감 여부 체크하도록 수정(전산재고 재공제외)
		...
		private void btnAddCompare_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				COM001_011_COMPARE_ADD wndCOMPARE_ADD = new COM001_011_COMPARE_ADD();
		...
		private void wndCOMPARE_ADD_Closed(object sender, EventArgs e)
		{
			try
			{
				COM001_011_COMPARE_ADD window = sender as COM001_011_COMPARE_ADD;
				if (window.DialogResult == MessageBoxResult.OK)
				{
					//Summary 조회
					//Detail 조회
