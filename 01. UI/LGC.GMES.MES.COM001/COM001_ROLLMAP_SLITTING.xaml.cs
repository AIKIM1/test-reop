/*************************************************************************************
 Created Date : 2022.03.09
      Creator : 신광희
   Decription : 자동차 전극 Roll Map - 롤맵 차트(SLITTING)
--------------------------------------------------------------------------------------
 [Change History]
  2022.03.09  신광희 : Initial Created
  2022.10.05  신광희 : Slitting 롤맵 차트에서 기준점(MARK) 라벨 표시는 마스터 Lane이 아닌 경우에도 표시가 되도록 요청함(심민규 책임)에 의해 소스 수정 함.
  ...
  2024.09.22  김지호 : [E20240905-001389] TAG_SECTION 툴팁내 불량 명칭 및 총 거리값 표시되도록 수정
**************************************************************************************/
...
namespace LGC.GMES.MES.COM001
{
    ...
    public partial class COM001_ROLLMAP_SLITTING : C1Window, IWorkArea
    {
        ...
        public COM001_ROLLMAP_SLITTING()
        {
            InitializeComponent();
        }
        ...
        private void DrawDefectRewinderScrap(DataTable dtRwScrap, string processCode)
        {
            if (CommonVerify.HasTableRow(dtRwScrap))
            {
                ...
                foreach (DataRow row in dtRwScrap.Rows)
                {
                    ...
                    string toolTip = row["EQPT_MEASR_PSTN_NAME"].GetString() + "[" + $"{row["SOURCE_ADJ_STRT_PSTN"]:###,###,###,##0.##}" + " ~ " + $"{row["SOURCE_ADJ_END_PSTN"]:###,###,###,##0.##}" + " ]";
        ...
        private void BeginUpdateChart()
        {
            foreach (C1Chart c1Chart in Util.FindVisualChildren<C1Chart>(grdMain))
            {
                ...
                c1Chart.View.Margin = c1Chart.Name == "chartInput" ? new Thickness(0) : new Thickness(0, 0, 0, 20);
                c1Chart.Padding = c1Chart.Name == "chartInput" ? new Thickness(10, 2, 5, 10) : new Thickness(10, 2, 5, 15);
            }
        }
