...
--------------------------------------------------------------------------------------
 [Change History]
 -  2019-11-18 최상민 : GMES Hold, QMS Hold건 보류재고등록 정보 상세 Lot, Cell정보 확인 팝업
...
namespace LGC.GMES.MES.COM001
{
    ...
    /// </summary>
    public partial class COM001_316_LOT_LIST : C1Window, IWorkArea
    {
        ...
        public COM001_316_LOT_LIST()
        {
            InitializeComponent();
            Loaded += COM001_316_LOT_LIST_Loaded;
        }

        private void COM001_316_LOT_LIST_Loaded(object sender, RoutedEventArgs e)
        {
            Loaded -= COM001_316_LOT_LIST_Loaded;           
        ...
        private void getLotIDInfo(string sHoldGRid, string sSearchGubun)
        {
            try
            {
                ...
                if (sSearchGubun== "LOT_CNT")
                {
                    dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_TB_SFC_ASSY_LOT_HOLD_GR_LIST", "RQSTDT", "OUTDATA", RQSTDT);
                    Util.GridSetData(dgLotList, dtResult, FrameOperation, true);
                }
                else
                {
                    dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_TB_SFC_ASSY_LOT_HOLD_GR_D_LIST", "RQSTDT", "OUTDATA", RQSTDT);
