/*************************************************************************************
 Created Date : 2023.04.18
      Creator : 신광희
   Decription : 자동차 전극 Roll Map - 통합 슬리터 차트 팝업
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2024.05.15  박학철 : 두께 차트영역에 예외 설비 측정 아이디 추가(COT_WIDTH_LEN_BACK)
...
namespace LGC.GMES.MES.COM001
{
    ...
    public partial class COM001_ROLLMAP_COMBINE_SLITTING : C1Window, IWorkArea
    {
        ...
        private bool _isSlitterSkipFlag = false;

        #endregion

        public COM001_ROLLMAP_COMBINE_SLITTING()
        {
            InitializeComponent();
        }
        ...
        private void rdoThickness_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!_isSlitterSkipFlag)
            {
                Util.MessageInfo("SFU8501", ObjectDic.Instance.GetObjectName("두꼐"));
        ...
        private void InitializeChart(C1Chart chart)
        {
            ...
            chart.View.AxisY.Min = ((bool)_isSlitterSkipFlag == false && chart.Name == "chartInput") ? -20 : -30;
        ...
        private void GetRollMapSlitterSkip()
        {
            // 설비가 통합 슬리터 여부 확인 2차 슬리터 SKIP 여부를 확인 함
            const string bizRuleName = "DA_PRD_SEL_SLITTER_SKIP_FLAG";
            ...
            {
                if (string.Equals("Y", result.Rows[0]["SLTR_SKIP_FLAG"].GetString()))
                {
                    _isSlitterSkipFlag = true;
                }
                else
                {
                    _isSlitterSkipFlag = false;
                }
                txtEquipmentName.Text = result.Rows[0]["EQPTNAME"] + " [" + result.Rows[0]["EQPTID"] + "]";
                _equipmentCode = result.Rows[0]["EQPTID"].GetString();
            }
            else
            {
                _isSlitterSkipFlag = false;
            ...
            _isSlitterSkipFlag = false;
            SetmeasurementControl();
        }

        private void SetmeasurementControl()
        {
            if (_isSlitterSkipFlag)
            {
                rdoBreadth.Visibility = Visibility.Visible;
                rdoWebgauge.Visibility = Visibility.Collapsed;
                rdoNone.Visibility = Visibility.Collapsed;
            ...
            {
                rdoBreadth.Visibility = Visibility.Collapsed;
                rdoWebgauge.Visibility = Visibility.Visible;
                rdoNone.Visibility = Visibility.Visible;
                rdoNone.IsChecked = true;
            }
        }


        private void MakeUserInterfaceFrameBySlitterSkipFlag()
        {
            // 2차 슬리터 SKIP 여부 true 인 경우 
            if (_isSlitterSkipFlag)
            {
                //전공정 영역 - 슬리팅
                //자공정 영역 - 2차 슬리팅
                //2차 슬리터 롤맵 팝업과 유사 함
                grdRollMap.RowDefinitions[0].Height = new GridLength(10, GridUnitType.Pixel);
                grdRollMap.RowDefinitions[1].Height = new GridLength(4.0, GridUnitType.Star);
                grdRollMap.RowDefinitions[2].Height = new GridLength(20, GridUnitType.Pixel);
                grdRollMap.RowDefinitions[3].Height = new GridLength(1.0, GridUnitType.Star);
                grdRollMap.RowDefinitions[4].Height = new GridLength(20, GridUnitType.Pixel);
                grdRollMap.RowDefinitions[5].Height = new GridLength(4.0, GridUnitType.Star);
                grdRollMap.RowDefinitions[6].Height = new GridLength(20, GridUnitType.Pixel);
                grdRollMap.RowDefinitions[7].Height = new GridLength(2.0, GridUnitType.Star);
                grdRollMap.RowDefinitions[8].Height = new GridLength(0, GridUnitType.Auto);

                grdInput.RowDefinitions[0].Height = new GridLength(1.0, GridUnitType.Star);
                grdInput.RowDefinitions[1].Height = new GridLength(70, GridUnitType.Pixel);

                grdOutput.RowDefinitions[0].Height = new GridLength(1.0, GridUnitType.Star);
                grdOutput.RowDefinitions[1].Height = new GridLength(5, GridUnitType.Pixel);
                grdOutput.RowDefinitions[2].Height = new GridLength(1.0, GridUnitType.Star);
                grdOutput.RowDefinitions[3].Height = new GridLength(70, GridUnitType.Pixel);
                grdOutput.ColumnDefinitions[0].Width = new GridLength(60, GridUnitType.Pixel);
                grdOutput.ColumnDefinitions[1].Width = new GridLength(30, GridUnitType.Pixel);
                grdOutput.ColumnDefinitions[2].Width = new GridLength(1.0, GridUnitType.Star);
                grdOutput.ColumnDefinitions[3].Width = new GridLength(150, GridUnitType.Pixel);
                grdOutput.ColumnDefinitions[4].Width = new GridLength(80, GridUnitType.Pixel);

                grdThickness.Visibility = Visibility.Visible;
            }
            else
            {
                //전공정 영역 - 롤프레스 ?
                ...
                //슬리터 롤맵 팝업과 유사 함
                grdRollMap.RowDefinitions[0].Height = new GridLength(10, GridUnitType.Pixel);
                grdRollMap.RowDefinitions[1].Height = new GridLength(7.0, GridUnitType.Star);
                grdRollMap.RowDefinitions[2].Height = new GridLength(20, GridUnitType.Pixel);
                grdRollMap.RowDefinitions[3].Height = new GridLength(1.0, GridUnitType.Star);
                grdRollMap.RowDefinitions[4].Height = new GridLength(20, GridUnitType.Pixel);
                grdRollMap.RowDefinitions[5].Height = new GridLength(3.0, GridUnitType.Star);
                grdRollMap.RowDefinitions[6].Height = new GridLength(0, GridUnitType.Auto);
                grdRollMap.RowDefinitions[7].Height = new GridLength(0, GridUnitType.Auto);
                grdRollMap.RowDefinitions[8].Height = new GridLength(0, GridUnitType.Auto);

                grdInput.RowDefinitions[0].Height = new GridLength(1.0, GridUnitType.Star);
                grdInput.RowDefinitions[1].Height = new GridLength(87, GridUnitType.Pixel);

                grdOutput.RowDefinitions[0].Height = new GridLength(0.5, GridUnitType.Star);
                grdOutput.RowDefinitions[1].Height = new GridLength(5, GridUnitType.Pixel);
                grdOutput.RowDefinitions[2].Height = new GridLength(0.5, GridUnitType.Star);
                grdOutput.RowDefinitions[3].Height = new GridLength(70, GridUnitType.Pixel);
                grdOutput.ColumnDefinitions[0].Width = new GridLength(60, GridUnitType.Pixel);
                grdOutput.ColumnDefinitions[1].Width = new GridLength(0, GridUnitType.Auto);
                grdOutput.ColumnDefinitions[2].Width = new GridLength(1.0, GridUnitType.Star);
                grdOutput.ColumnDefinitions[3].Width = new GridLength(150, GridUnitType.Pixel);
                grdOutput.ColumnDefinitions[4].Width = new GridLength(80, GridUnitType.Pixel);

                grdThickness.Visibility = Visibility.Collapsed;
            }
        }
        ...
        private void SelectRollMap()
        {
            ...
            new ClientProxy().ExecuteService_Multi(bizRuleName, "INDATA", "OUT_LANE,OUT_HEAD,OUT_DEFECT,OUT_GAUGE,OUT_IN_LANE,OUT_IN_HEAD,OUT_IN_DEFECT,OUT_IN_GAUGE", (bizResult, bizException) =>
            {
                try
                {
                    ...
                    MakeUserInterfaceFrameBySlitterSkipFlag();
        ...
        private void DrawStartEndYAxis(DataTable dt, ChartConfigurationType chartConfigurationType)
        {
            ...
            var query = (from t in dt.AsEnumerable()
                         ...
                         select new { StartPosition = t.Field<decimal?>("STRT_PSTN"), EndPosition = t.Field<decimal>("END_PSTN") }).FirstOrDefault();
            ...
            */
            ...
            var queryMergeLot = (from t in dt.AsEnumerable()
                                 ...
                                 select new { InputLotId = t.Field<string>("INPUT_LOTID"), StartPosition = t.Field<decimal>("CUM_STRT_PSTN"), EndPosition = t.Field<decimal>("CUM_END_PSTN") }).ToList();
        ...
        private void DrawGauge(DataTable dt, ChartConfigurationType chartConfigurationType)
        {
            ...
            string[] gaugeExceptions = new string[] { "PET", "SCRAP", "RW_SCRAP", "TAG_SECTION", "ROLLMAP_BACKGROUND", "COT_WIDTH_LEN_BACK" };
        ...
        private void DrawTopBackRow(DataTable dt, ChartConfigurationType chartConfigurationType)
        {
            ...
            for (int i = 0; i < laneCount; i++)
            {
                ...
                borderTop.Margin = new Thickness(0, 0, 0, 1);
                ...
                borderBack.Margin = new Thickness(0, 0, 0, 1);
        ...
        private DataTable MakeTableForDisplay(DataTable dt, ChartDisplayType chartDisplayType, ChartConfigurationType chartConfigurationType)
        {
            ...
            if (chartDisplayType == ChartDisplayType.MarkLabel)
            {
                ...
                foreach (DataRow row in dtBinding.Rows)
                {
                    ...
                    if (_isSlitterSkipFlag)
                        row["SOURCE_ADJ_Y_PSTN"] = -13; // 통합 슬리터 설비
                    else
            ...
            else if (chartDisplayType == ChartDisplayType.TagSpot)
            {
                ...
                foreach (DataRow row in dtBinding.Rows)
                {
                    ...
                    if (_isSlitterSkipFlag) // 통합 슬리터 설비
                        row["SOURCE_ADJ_Y_PSTN"] = -28;
                    else
