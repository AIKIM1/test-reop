    ...
    public partial class Report_Multi : C1Window, IWorkArea
    {
        ...
        private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    //txtPage.Text = (iPrint + 1).ToString();
        ...
        private void Open()
        {
            try
            {
                ...
                using (Stream stream = a.GetManifestResourceStream("LGC.GMES.MES.COM001.Report." + mFileName))
                {
                    cr1.Load(stream, mReportName);
                }
                using (Stream stream = a.GetManifestResourceStream("LGC.GMES.MES.COM001.Report." + dFileName))
                {
        ...
        private void SearchData()
        {
            try
            {
                ...
                dtRqst.Rows.Add(dr);
                DataSet dsRslt = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_SEL_STOCK_APPRV_PASS", "INDATA", "SUMMARYDATA,DETAILDATA", inData);
        ...
        private string RegisterPassNo()
        {
            ...
            try
            {
                ...
                DataTable ordTable = indataSet.Tables.Add("ORDDATA");
                ordTable.Columns.Add("MOVE_ORD_ID", typeof(String));
                ...
                foreach (string id in list)
                {
                    ...
                    ordRow["MOVE_ORD_ID"] = id;
                    ordTable.Rows.Add(ordRow);
                }
                DataSet dsRslt = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_STOCK_APPRV_PASS","INDATA,ORDDATA", "OUTDATA", indataSet);
                value = Util.NVC(dsRslt.Tables["OUTDATA"].Rows[0]["VALUE"]);
