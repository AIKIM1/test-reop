...
{
    public partial class COM001_014_SPLIT_APPR_ASSIGN : C1Window, IWorkArea
    {
        #region Declaration & Constructor 
        string areaId = string.Empty;
        DataTable dtLoss = new DataTable();

        public COM001_014_SPLIT_APPR_ASSIGN()
        {
            InitializeComponent();
        }
        ...
        private void Save()
        {
            ...
            if (dtLoss != null && dtLoss.Rows.Count > 0)
            {
                for (int i = 0; i < dtLoss.Rows.Count; i++)
                {
                    ...
                    dr["STRT_DTTM"] = DateTime.Parse(Util.NVC(dtLoss.Rows[i]["STRT_DTTM"])).ToString("yyyyMMddHHmmss");
                    dr["END_DTTM"] = DateTime.Parse(Util.NVC(dtLoss.Rows[i]["END_DTTM"])).ToString("yyyyMMddHHmmss");
                    dr["WRK_DATE"] = Util.NVC(dtLoss.Rows[i]["WRK_DATE"]);
                    dr["LOSS_SEQNO"] = Util.NVC(dtLoss.Rows[i]["TO_LOSS_SEQNO"]);
                    dr["APPR_STAT"] = "W";
                    dr["APPR_REQ_LOSS_CODE"] = Util.NVC(dtLoss.Rows[i]["LOSS_CODE"]);
                    dr["APPR_REQ_LOSS_DETL_CODE"] = Util.NVC(dtLoss.Rows[i]["LOSS_DETL_CODE"]);
                    dr["APPR_REQ_LOSS_CNTT"] = Util.NVC(dtLoss.Rows[i]["NOTE"]);
                    dr["USERID"] = LoginInfo.USERID;
                    dr["APPR_USERID"] = Util.GetCondition(cboApproval);
                    dr["SPLT_MRG_FLAG"] = "S";
                    dr["ORG_STRT_DTTM"] = DateTime.Parse(Util.NVC(dtLoss.Rows[0]["STRT_DTTM"])).ToString("yyyyMMddHHmmss");
                    dr["ORG_END_DTTM"] = DateTime.Parse(Util.NVC(dtLoss.Rows[dtLoss.Rows.Count - 1]["END_DTTM"])).ToString("yyyyMMddHHmmss");
                    RQSTDT.Rows.Add(dr);
                }
            }
            ...
            new ClientProxy().ExecuteService_Multi("BR_EQPT_EQPTLOSS_INS_REQ_SPLIT", "INDATA", null, (bizResult, bizException) =>
            {
                try
                {
