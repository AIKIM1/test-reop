...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2022.09.28  최재욱 : [C20221006-000307] CT검사, 동별 기준정보 구분 추가
  2024.01.30  SI    남기운K : (NERP 대응 프로젝트)실적처리 허용비율 초과 입력 기능추가
**************************************************************************************/
...
{
    /// <summary>
    /// ASSY004_COM_CONFIRM.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY004_COM_CONFIRM : C1Window, IWorkArea
    {
		...
		private string _UnidentifiedLossYN = string.Empty;
		private string _qmsDefectCodeYN = string.Empty;

        private bool _AUTOCONFIRM = true; // 2022.12.21
        ...
        private double GetLotINPUTQTY(string sLotID)
        {
            ...
            if (dgQty != null && dgQty.Rows.Count > 0)
            {
                ...
                dInputQty = Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "INPUTQTY")).Equals("") ? 0 : double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "INPUTQTY")));
        ...
        private void GetDefectInfo()
        {
            try
            {
				...
				//QMS에 등록된 불량코드 정보를 사용하는지 구분
				if (_qmsDefectCodeYN == "N")
					bizRuleID = "DA_QCA_SEL_WIPRESONCOLLECT";
				else
					bizRuleID = "DA_QCA_SEL_WIPRESONCOLLECT_L";
                ...
                DataTable searchResult = new ClientProxy().ExecuteServiceSync(bizRuleID, "INDATA", "OUTDATA", inTable);
		...
		{
			_UnidentifiedLossYN = string.Empty;
			_qmsDefectCodeYN = string.Empty;
			...
			try
			{
				...
				if (CommonVerify.HasTableRow(dtResult))
				{
					...
					if (_UnidentifiedLossYN == "N")
					{
						dgQty.Columns["UNIDENTIFIED_QTY"].Visibility = Visibility.Collapsed;
					}
				}
			}
			catch (Exception ex)
		...
		private bool CanSave(out double dUnidentifiedQty)
        {
			...
			//미확인 LOSS를 사용하는지 확인
			if (_UnidentifiedLossYN == string.Empty || _UnidentifiedLossYN == "Y")
			{
            ...
            // CT 검사 중 미확인 LOSS를 사용하지 않는 Site는 좌변(생산량) vs 우변(양품+불량...)의 값이 동일해야 한다.
            if(_ProcID.Equals(Process.CT_INSP) && _UnidentifiedLossYN == "N")
            {
                double dConsValue = double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "INPUTQTY")))  // 생산량
                                  - double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "GOODQTY")))   // 양품량
                                  - double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "DTL_DEFECT_LOT"))) // 불량량
                                  - double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "DTL_LOSS_LOT"))) // Loss량
                                  - double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "DTL_CHARGE_PROD_LOT"))); // 물품청구량

                if(dConsValue != 0)
                {
                    // 입력 오류
                    Util.MessageValidation("SFU6055");
                    return bRet;
                }
            }
            //if (dUnidentifiedQty < 0)
        ...
        private void wndShiftUser_Closed(object sender, EventArgs e)
        {
            ...
            {
                #region 작업조,작업자 등록 화면 변경 요청 건 [C20200511-000024]
                // 2022.12.21 CT검사 공정진척 이전 작업조, 작업자 데이터 반영 안됨
                if (_AUTOCONFIRM)
                {
                    GetEqptWrkInfo();
                }
        ...
        //2022.12.21 auto confir 대응
        // CT검사 공정진척(신규) 작업조, 작업자 문제로 추가
        private void SetAutoConfirm()
        {
            try
            {
                ...
                DataRow Indata = IndataTable.NewRow();
                Indata["LANGID"] = LoginInfo.LANGID;
                Indata["CMCDTYPE"] = "CT_INSP_AREA_BY_CHECK";
                Indata["CMCODE"] = LoginInfo.CFG_AREA_ID;

                IndataTable.Rows.Add(Indata);

                DataTable result = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMONCODE_TBL", "RQSTDT", "RSLTDT", IndataTable);

                if (result.Rows.Count > 0)
                {
                    if (Util.NVC(result.Rows[0]["ATTRIBUTE1"]).Equals("N") && _ProcID.Equals("A8800"))
                        _AUTOCONFIRM = false;
                    else
                        _AUTOCONFIRM = true;
                }
