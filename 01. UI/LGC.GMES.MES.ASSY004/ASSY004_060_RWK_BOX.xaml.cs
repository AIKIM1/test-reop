...
--------------------------------------------------------------------------------------
 [Change History]
  2019.10.28  정문교 : Initial Created.   폴란드3동 & 빈강용 조립 공정에서 사용
                                          ASSY004_050_RWK_BOX Copy ASSY004_060_RWK_BOX
  2021.02.05  정문교 : CSR : C20201229-000197
                       FOL/Stacking Rework 재작업BOX출고 탭
                       Carrier ID, 프로젝트 전체 체크,해제 선택 오류 수정
  2021.03.18  안인효 : CSR : C20210316-000472
                       재공목록 조회시에 LOT ID 조회 추가
                       1) Carrier ID 로 찾고 2) 없으면 LOT ID로 찾음
...
namespace LGC.GMES.MES.ASSY004
{
    ...
    /// </summary>
    public partial class ASSY004_060_RWK_BOX : UserControl, IWorkArea
    {
        ...
        public ASSY004_060_RWK_BOX(UserControl uc)
        {
            _UCParent = uc;
            _Util = new Util();
        ...
        private void txtCstID_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                // 이미 체크된 Lot이면 Skip
                if (Util.NVC(DataTableConverter.GetValue(dgWaitReleasedWip.Rows[idx].DataItem, "CHK")) != "1")
                {
                    DataTable tmpTable = DataTableConverter.Convert(dgWaitReleasedWip.ItemsSource);
                ...
                if (dtTo.Rows.Count > 1)
                {
                    for (int i = 0; i < dtTo.Rows.Count; i++)
                    {
                        if (dtTo.Rows[i]["PRJT_NAME"].ToString() == this.txtProject.Text.ToString())
                        {
                            ...
                            {
                                DataTableConverter.SetValue(dgWaitReleasedWip.Rows[i].DataItem, "CHK", 1);
