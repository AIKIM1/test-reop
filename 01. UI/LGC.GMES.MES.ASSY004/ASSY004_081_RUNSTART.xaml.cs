...
--------------------------------------------------------------------------------------
 [Change History]
  2022.09.26  최재욱 : [C20221006-000307] ASSY004_011_RUNSTART 기반으로 생성 및 기능 수정
**************************************************************************************/
...
{
    /// <summary>
    /// ASSY004_081_RUNSTART.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY004_081_RUNSTART : C1Window, IWorkArea
    {
        ...
        private void InitCombo()
        {
			...
			if (cboBoxMountPstsID.Items.Count == 2)
				cboBoxMountPstsID.SelectedIndex = 1;
        ...
        private void tbxInLot_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                try
                {
                    ...
                    new ClientProxy().ExecuteService("BR_PRD_SEL_WAIT_LOT_LIST_CI_L", "INDATA", "OUTDATA", dt, (resultDt, exception) =>
                    {
                        try
                        {
                            ShowLoadingIndicator();
        ...
        private string GetNewCTINSPLotid()
        {
            try
            {
				...
				//newRow["EQPT_MOUNT_PSTN_ID"] = _EqptMountPstn;
                ...
                DataSet dsRslt = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_NEW_PROD_LOTID_CI_L", "IN_EQP,IN_INPUT", "OUT_LOT", indataSet);
        ...
        private void RunStart(string sNewLot)
        {
            ...
            {                
                ...
                inMtrl.Columns.Add("INPUT_ID", typeof(string));
                ...
                DataSet dsRslt = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_REG_START_PROD_LOT_CI_L", "IN_EQP,IN_INPUT", null, indataSet);
        ...
        private bool CanRunStart()
        {
			...
			//if (string.IsNullOrEmpty(_EqptMountPstn))
			//{
			//    Util.MessageValidation("SFU2020"); //해당 설비의 장착위치부 정보가 없습니다.
			//}
			//else
