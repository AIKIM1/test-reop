...
{
    public partial class FCS002_306_SPLIT_SHIP_PLT : C1Window, IWorkArea
    {

        private int MIN_SPILT_BOX_CNT = 1;  //분할 가능한 최소 박스 수량
        
        #region Declaration & Constructor 
        public FCS002_306_SPLIT_SHIP_PLT()
        {
            InitializeComponent();
            Initialize();
        }
        ...
        private void DoSplitPallet()
        {
            try
            {
                ...
                */
                ...
                new ClientProxy().ExecuteService_Multi("BR_PRD_REG_SPLIT_PALLET_MB", "INDATA,INLOT", "OUTDATA", (Result, ex) =>
                {
                    if (ex != null)
                    {
                        Util.MessageException(ex);
