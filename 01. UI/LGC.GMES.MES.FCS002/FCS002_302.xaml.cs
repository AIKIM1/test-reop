/*************************************************************************************
 Created Date : 2018.08.09
      Creator : 오화백K
   Decription : 원/각 특성/Grading 공정진척(NEW)
--------------------------------------------------------------------------------------
 [Change History]
 2018.12.17  0.1                             Inbox 팝업 호출 추가
 2022.03.08  0.2   곽란영   SI               남경일 경우 콤보 변경
 2023.03.09  0.3   이홍주   SI               소형활성화 MES 복사
**************************************************************************************/
...
{
    public partial class FCS002_302 : IWorkArea
    {
        #region Declaration
        ...
        private void InitializeUserControls()
        {
            ...
            if (UcFormCommand != null)
            {
                ...
                //F5000은 불량등록 버튼이 보이지 않도록 설정됨.. TEST를 위해 일단제외함.
                //원각형 특성 Greaing(F5300) 가 아니고 원형특성 Grading F5000으로 갈지 다시 확인필요(TO : 정현수님) 2023.05.25 
                //UcFormCommand.ButtonDefect.Visibility = Visibility.Visible;
            ...
            if (UcFormProductionPallet != null)
            {
                ...
                UcFormProductionPallet.IsInboxCreate = true;
        ...
        private void SetEventInUserControls()
        {
            ...
            if (UcFormProductionPallet != null)
            {
                ...
                UcFormProductionPallet.ButtonInboxCreate.Click += ButtonInboxCreate_Click;
            }
        ...
        {
            //2023.05.11 IROCV, INSPECTOR.. 으로 설정되어서. 변경
            
            //_processCode = "FF5101, FFV101, B1000"; 
            //_processCode = Process.CELL_BOXING; //일단 CELL_BOXING B1000으로 설정

            _processCode = Process.CircularCharacteristicGrader; //F5300
            //_processCode = "FF5101";  //Grader로 지정
        ...
        private void ButtonDefect_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_DEFECT popupDefect = new FCS002_DEFECT { FrameOperation = FrameOperation }; //2023.05.24
            ...
            //parameters[4] = "CFHA700003051101"; //TEST 2023.05.25
        ...
        private void ButtonStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_302_RUNSTART popupRunStart = new FCS002_302_RUNSTART { FrameOperation = FrameOperation };
        ...
        {
            FCS002_302_RUNSTART popup = sender as FCS002_302_RUNSTART;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                ClearControls();
        ...
        private void ButtonCompletion_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_302_CONFIRM popupConfirm = new FCS002_302_CONFIRM { FrameOperation = FrameOperation };
        ...
        {
            FCS002_302_CONFIRM popup = sender as FCS002_302_CONFIRM;
        ...
        private void SetPopupGoodPalletCreate()
        {
            ...
            FCS002_302_PALETTE_CREATE popupPalletCreate = new FCS002_302_PALETTE_CREATE { FrameOperation = this.FrameOperation };
        ...
        private void SetPopupGoodPalletEdit()
        {
            ...
            FCS002_302_PALETTE_CREATE popupPalletEdit = new FCS002_302_PALETTE_CREATE { FrameOperation = this.FrameOperation };
        ...
        {
            FCS002_302_PALETTE_CREATE popup = sender as FCS002_302_PALETTE_CREATE;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                GetProductLot(true);
                UcFormProductionPallet.SelectPalletList();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ButtonInboxCreate_Click(object sender, RoutedEventArgs e)
        {
            ...
            DataRow dr = UcFormProductionPallet.GetSelectPalletLotRow();
            popupInboxCreate.ShiftID = Util.NVC(UcFormShift.TextShift.Tag);
            popupInboxCreate.ShiftName = Util.NVC(UcFormShift.TextShift.Text);
            popupInboxCreate.WorkerID = Util.NVC(UcFormShift.TextWorker.Tag);
            popupInboxCreate.WorkerName = Util.NVC(UcFormShift.TextWorker.Text);
        ...
        private bool ValidationDefect()
        {
            ...
            if (rowIndex < 0)
            {
                // 생산LOT등의 정보가 없어도 불량등록이 가능하도록 일단 막음. 2023.05.25
                // 생산 Lot 정보가 없습니다.
