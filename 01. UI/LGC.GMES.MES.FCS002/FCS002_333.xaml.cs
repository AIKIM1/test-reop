/*************************************************************************************
 Created Date : 2017.10.20
      Creator : 
   Decription : 원/각 특성/Grading 공정진척
--------------------------------------------------------------------------------------
 [Change History]

  날짜        버젼  수정자   CSR              내용
 -------------------------------------------------------------------------------------
 2018.12.26  0.1   이상훈   C20181121_50523  활성화 Grader In-line화 GMES 기능 추가
 2022.03.08  0.2   곽란영   SI               남경일 경우 콤보 변경
 2023.06.07  0.3   이홍주   SI               소형활성화 MES 복사
**************************************************************************************/
...
{
    public partial class FCS002_333 : IWorkArea
    {
        #region Declaration
        ...
        public UcFormInputTray UcFormInputTray { get; set; }
        ...
        private bool _isTray = true;

        private int _tagPrintCount;

        private string QLTY_TYPE_GOOD = "G";   //C20181121_50523 용량등급 조회 시 양품 구분
        private string QLTY_TYPE_DFCT = "N";   //C20181121_50523 용량등급 조회 시 불량 구분
        ...
        private void Initialize()
        {
            ...
            if (ComboEquipment?.SelectedValue != null)
            {
                ...
                UcFormInputTray.EquipmentCode = ComboEquipment.SelectedValue.GetString();
                UcFormInput.EquipmentCode = ComboEquipment.SelectedValue.GetString();
            }

            UcFormInputTray?.ChangeEquipment(equipmentCode);
        ...
        private void InitializeUserControls()
        {
            ...
            UcFormInputTray = grdInput.Children[0] as UcFormInputTray;
            UcFormInput = grdInput.Children[1] as UcFormInput;
            ...
            if (UcFormInputTray != null)
            {
                UcFormInputTray.UcParentControl = this;
                UcFormInputTray.FrameOperation = FrameOperation;
                UcFormInputTray.ProcessCode = _processCode;

                //C20181121_50523 트레이 잔량 처리 기능 추가
                if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
                {
                    UcFormInputTray.ButtonTrayRemainWait.Visibility = Visibility.Visible;  
                }
            ...
            if (UcFormProductionPallet != null)
            {
                ...
                //C20181121_50523 불량 pallet 버튼 활성화
                //if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
                //{
                //    UcFormProductionPallet.ButtonDefectPalletCreate.Visibility = Visibility.Visible;
                //}
            }
        ...
        private void InitializeUserControlsGrid()
        {
            ...
            DgProductionShipPallet = UcFormProductionPallet.DgProductionShipPallet;
        }
        ...
        private void SetEventInUserControls()
        {
            if (UcFormCommand != null)
            {
                ...
                //C20181121_50523 상위 command 버튼으로 변경 처리
                if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
                {
                    UcFormCommand.ButtonDefectPalletCreate.Click += ButtonDefectPalletCreate_Click;
                }
            ...
            if (UcFormInputTray != null)
            {
                ...
                //UcFormProductionPallet.ButtonProductWaitLot.Click += ButtonWaitPancake_Click;
                //UcFormProductionPallet.ButtonCellWindingLocation.Click += ButtonWindingTrayLocation_Click;
                //UcFormProductionPallet.TextProductLotId.PreviewKeyDown += TextProductLotId_PreviewKeyDown;
                UcFormInputTray.ButtonTrayRemainWait.Click += ButtonTrayRemainWait_Click; // [C20181121_50523] 추가 
            ...
            if (UcFormProductionPallet != null)
            {
                ...
                UcFormProductionPallet.ButtonChangeProdLot.Click += ButtonChangeProdLot_Click;
                //if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
                //{
        ...
        private void UserControl_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            UcFormInputTray.TextBoxTrayID.Focus();
        }
        ...
        private void ButtonDefect_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_DEFECT popupDefect = new FCS002_DEFECT { FrameOperation = FrameOperation };
            //FCS002_RECHECK_NG popupDefect = new FCS002_RECHECK_NG { FrameOperation = FrameOperation };
        ...
        {
            FCS002_DEFECT popup = sender as FCS002_DEFECT;
            //FCS002_RECHECK_NG popup = sender as FCS002_RECHECK_NG;
        ...
        private void ButtonStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_333_RUNSTART popupRunStart = new FCS002_333_RUNSTART { FrameOperation = FrameOperation };
        ...
        {
            FCS002_333_RUNSTART popup = sender as FCS002_333_RUNSTART;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                ClearControls();
        ...
        private void ButtonCompletion_Click(object sender, RoutedEventArgs e)
        {
            if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
            {
                UcFormProductionPallet.SelectShipPalletList();  //ShipPalletList 최신화 후 Validation 수행
            }

            if (!ValidationCompletion())
                return;

            FCS002_333_CONFIRM popupConfirm = new FCS002_333_CONFIRM { FrameOperation = FrameOperation };
        ...
        {
            FCS002_333_CONFIRM popup = sender as FCS002_333_CONFIRM;
        ...
        private void ButtonInboxType_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_EQPT_INBOX_TYPE popupInboxTyp = new FCS002_EQPT_INBOX_TYPE { FrameOperation = FrameOperation };
        ...
        {
            FCS002_EQPT_INBOX_TYPE popup = sender as FCS002_EQPT_INBOX_TYPE;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
            }
            grdMain.Children.Remove(popup);
        ...
        {
            if (!ValidationPalletRemain("PALLET")) return;

            FCS002_PALETTE_WAIT_REMAIN popupPalletRemain = new FCS002_PALETTE_WAIT_REMAIN { FrameOperation = this.FrameOperation };
        ...
        {
            FCS002_PALETTE_WAIT_REMAIN popup = sender as FCS002_PALETTE_WAIT_REMAIN;
            if (popup.ConfirmSave)
            {
                GetProductLot(true);
        ...
        /// <summary>
        /// [C20181121_50523]  Tray 잔량대기 
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ButtonTrayRemainWait_Click(object sender, RoutedEventArgs e)
        {
            //if (!ValidationPalletRemain("TRAY")) return;

            //FORM001_TRAY_WAIT_REMAIN popupTrayRemain = new FORM001_TRAY_WAIT_REMAIN { FrameOperation = this.FrameOperation };

            //if (ValidationGridAdd(popupTrayRemain.Name.ToString()) == false)
            //    return;

            //popupTrayRemain.ShiftID = Util.NVC(UcFormShift.TextShift.Tag);
            //popupTrayRemain.ShiftName = Util.NVC(UcFormShift.TextShift.Text);
            //popupTrayRemain.WorkerID = Util.NVC(UcFormShift.TextWorker.Tag);
            //popupTrayRemain.WorkerName = Util.NVC(UcFormShift.TextWorker.Text);
            //popupTrayRemain.ShiftDateTime = Util.NVC(UcFormShift.TextShiftDateTime.Text);
            ...
            ////parameters[4] = UcFormInput.GetSelectInputPalletRow();
            //parameters[4] = UcFormProdLot.GetSelectProductLotRow(); ;
            //C1WindowExtension.SetParameters(popupTrayRemain, parameters);

            //popupTrayRemain.Closed += new EventHandler(popupTrayRemainWait_Closed);
            //grdMain.Children.Add(popupTrayRemain);
            //popupTrayRemain.BringToFront();
        }

        
        private void popupTrayRemainWait_Closed(object sender, EventArgs e)
        {
            //FORM001_TRAY_WAIT_REMAIN popup = sender as FORM001_TRAY_WAIT_REMAIN;
            //if (popup.ConfirmSave)
            //{
            //    GetProductLot(true);
            //    UcFormInput.SelectInputPalletList();
        ...
        private void ButtonPalletHold_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_PALETTE_HOLD popupPalletHold = new FCS002_PALETTE_HOLD { FrameOperation = this.FrameOperation };
        ...
        private void ButtonChangeProdLot_Click(object sender, RoutedEventArgs e)
        {
            if (!ValidationChangeProdLot())
            {
                return;
            }
            ...
            FCS002_333_CHANGE_PROD_LOT popupChangeProdLot = new FCS002_333_CHANGE_PROD_LOT { FrameOperation = this.FrameOperation };

            if (ValidationGridAdd(popupChangeProdLot.Name.ToString()) == false)
            {
                return;
            }

            DataRow[] dr = DataTableConverter.Convert(DgProductionShipPallet.ItemsSource).Select("CHK = 1");

            object[] parameters = new object[6];
            parameters[0] = Util.NVC(ComboEquipment.SelectedValue); //EQPTID
            parameters[1] = "PROC"; //WIPSTAT
            parameters[2] = _processCode;   //PROCID
            parameters[3] = "PROD"; //WIPTYPECODE
            parameters[4] = Util.NVC(DataTableConverter.GetValue(DgProductLot.Rows[iRow].DataItem, "ASSY_LOTID"));   //ASSY_LOTID
            parameters[5] = dr;

            C1WindowExtension.SetParameters(popupChangeProdLot, parameters);

            popupChangeProdLot.Closed += new EventHandler(popupChangeProdLot_Closed);
            grdMain.Children.Add(popupChangeProdLot);
            popupChangeProdLot.BringToFront();
        }

        private void popupPalletHold_Closed(object sender, EventArgs e)
        {
            FCS002_PALETTE_HOLD popup = sender as FCS002_PALETTE_HOLD;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
        ...
        private void popupChangeProdLot_Closed(object sender, EventArgs e)
        {
            ////FCS002_333_CHANGE_PROD_LOT popup = sender as FCS002_333_CHANGE_PROD_LOT;
        ...
        {
            SetPopupGoodPalletCreate(QLTY_TYPE_GOOD);
        }
        ...
        {
            SetPopupGoodPalletCreate(QLTY_TYPE_DFCT);
        }
        ...
        private void SetPopupGoodPalletCreate(string sQltyType = "G")
        {
            if (!ValidationGoodPalletCreate()) return;

            FCS002_333_PALETTE_CREATE popupPalletCreate = new FCS002_333_PALETTE_CREATE { FrameOperation = this.FrameOperation };
            ...
            object[] parameters = new object[8]; //C20181121_50523 양/불 추가
        ...
        private void SetPopupGoodPalletEdit()
        {
            ...
            FCS002_333_PALETTE_CREATE popupPalletEdit = new FCS002_333_PALETTE_CREATE { FrameOperation = this.FrameOperation };
        ...
        {
            FCS002_333_PALETTE_CREATE popup = sender as FCS002_333_PALETTE_CREATE;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                GetProductLot(true);
                UcFormProductionPallet.SelectPalletList();
        ...
        private void ButtonPalletDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            FCS002_PALETTE_DELETE popupPalletDelete = new FCS002_PALETTE_DELETE { FrameOperation = this.FrameOperation };
        ...
        {
            FCS002_PALETTE_DELETE popup = sender as FCS002_PALETTE_DELETE;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                GetProductLot(true);
                UcFormProductionPallet.SelectPalletList();
        ...
        private void ButtonTagPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            //C20181121_50523 태그 출력이 오창과 남경이 상이 하여 분기 처리 함
            if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
            {
            ...
            {
                DataView dv = DataTableConverter.Convert(DgProductionPallet.ItemsSource).DefaultView;
                dv.Sort = "PALLETE_ID";

                DataRow[] drSelect = dv.ToTable().Select("CHK = 1");
        ...
        private void TagPrint(string pallet, int pageCnt)
        {
            ...
            parameters[1] = Util.NVC(ComboEquipment.SelectedValue);
            parameters[2] = pallet;
            parameters[3] = DataTableConverter.GetValue(DgProductLot.Rows[rowIndex].DataItem, "WIPSEQ").GetString(); 
            parameters[4] = null;
            //parameters[5] = DataTableConverter.GetValue(DgProductionPallet.Rows[iRow].DataItem, "DISPATCH_YN").GetString();
            parameters[5] = "Y";      // 디스패치 처리
        ...
        private void ComboEquipment_SelectedValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            try
            {
                ...
                UcFormInputTray?.ChangeEquipment(ComboEquipment.SelectedValue.ToString());
        ...
        public void GetProductLot(bool ProductOnly, string prodLot = null)
        {
            try
            {
                ...
                ////UcFormInputTray.InitializeControls();
        ...
        public void ProdListClickedProcess(int iRow)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(Util.NVC(DataTableConverter.GetValue(DgProductLot.Rows[iRow].DataItem, "FRST_CSTID"))))
                {
                    _isTray = false;

                    UcFormInput.Visibility = Visibility.Visible;
                    UcFormInputTray.Visibility = Visibility.Collapsed;
                }
                else
                {
                    _isTray = true;

                    UcFormInput.Visibility = Visibility.Collapsed;
                    UcFormInputTray.Visibility = Visibility.Visible;
                }

                UcFormInputTray.ProdLotId = Util.NVC(DataTableConverter.GetValue(DgProductLot.Rows[iRow].DataItem, "LOTID"));
                ...
                if (_isTray)
                {
                    UcFormInputTray.SelectInputTrayList();
                    UcFormInputTray.TextBoxTrayID.Focus();
                }
                else
                {
        ...
        private bool ValidationSearch()
        {
            ...
            if (ComboEquipment.SelectedValue == null || ComboEquipment.SelectedValue.ToString().Trim().Equals("SELECT"))
            {
                // "설비를 선택 하세요."
                Util.MessageValidation("SFU1673");
                return false;
        ...
        {         
            if (ComboEquipment.SelectedValue == null || ComboEquipment.SelectedValue.GetString().Equals("SELECT"))
            {
                // 설비를 선택 하세요.
                Util.MessageValidation("SFU1673");
                return false;
        ...
        private bool ValidationCancel()
        {
            ...
            if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
            {
                if (DgProductionShipPallet.Rows.Count - DgProductionShipPallet.FrozenBottomRowsCount > 0)
                {
                    Util.MessageValidation("SFU3819");  //출하Pallet가 존재하여 취소할 수 없습니다.
                    return false;
                }
            }
        ...
        private bool ValidationCompletion()
        {
            ...
            if (LoginInfo.CFG_SHOP_ID.Equals("A010"))
            {
                DataRow[] drchkShip = DataTableConverter.Convert(DgProductionShipPallet.ItemsSource).Select("BOXSTAT_LIST = 'CREATED'");

                if (drchkShip.Length > 0)
                {
                    Util.MessageValidation("SFU3820"); //포장중인 출하Pallet가 존재합니다.
                    return false;
                }
            }
        ...
        private bool ValidationPalletRemain(string sType)
        {
            if (ComboEquipment.SelectedIndex < 0 || ComboEquipment.SelectedValue.GetString().Equals("SELECT"))
            {
                //Util.Alert("설비를 선택 하세요.");
            ...
            if (sType.Equals("PALLET") && rowIndex < 0)
            {
                // 선택된 항목이 없습니다.
                Util.MessageValidation("SFU1651");
                return false;
        ...
        private bool ValidationChangeProdLot()
        {
            if (ComboEquipment.SelectedValue == null || ComboEquipment.SelectedValue.GetString().Equals("SELECT"))
            {
                //Util.Alert("설비를 선택 하세요.");
            ...
            DataRow[] drUnchk = dt.Select("CHK = 1");
