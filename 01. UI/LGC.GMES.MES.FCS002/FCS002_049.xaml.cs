...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2023.09.13  주훈        조회 조건에 구분(온도 측정 위치: 기구부, 전원부) 항목 추가
...
{
    public partial class FCS002_049 : UserControl, IWorkArea
    {
        ...
        DataTable _JigGrp = new DataTable();
        DataTable _PowerGrp = new DataTable();
        ...
        private void InitCombo()
        {
            ...
            // 2023.09.13  조회 조건에 구분(온도 측정 위치: 기구부, 전원부) 항목 추가
            string[] sFilter1 = { "TMPR_PSTN", "1, 2" };
            _combo.SetCombo(cboTempUnit, CommonCombo_Form_MB.ComboStatus.ALL, sCase: "CMN", sFilter: sFilter1);
        }
        ...
        private void chart_MouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            if (chrt.Tag.Equals("all"))
            {
                ...
                cl1.Height = grd.Height / 4 * 3;
                cl2.Height = grd.Height / 4 * 3;

                chrt.Tag = "one";
            }
            else
            {
                // 2023.09.13  추가 : Grid Visibility 설정
                if (cboTempUnit.SelectedIndex == 0)
                {
        ...
        private void GridInit()
        {
            ...
            chrJig.Data.Children.Clear();
            chrPower.Data.Children.Clear();

            // 2023.09.13  추가 : Grid Visibility 설정
            switch (cboTempUnit.SelectedIndex)
            {
                case 1: // 기구분
                    chrJig.Tag = "all";
                    chart_MouseDown(chrJig, null);
                    break;
                case 2:  // 전원부
                    chrPower.Tag = "all";
                    chart_MouseDown(chrPower, null);
                    break;
                default:
                    break;
            }
        ...
        private void GetList()
        {
            try
            {
                ...
                // 2023.09.13  추가 : 구분 조건에 따라 보여지는 항목 결정
                int iTempUnitIdx = 0;
                iTempUnitIdx = cboTempUnit.SelectedIndex;
                ...
                dtRslt = new ClientProxy().ExecuteServiceSync("DA_SEL_FORMATION_TEMP_HIST_MB", "RQSTDT", "RSLTDT", dtRqst);
                ...
                if ((iTempUnitIdx == 0) || (iTempUnitIdx == 1))  // 기구부
                {
                    if (dtRslt.Rows.Count > 0)
                    {
                        //동적 컬럼 생성
                        ...
                        {
                            if (c.ColumnName.Contains("JIG_TEMP"))
                            {
                ...
                if ((iTempUnitIdx == 0) || (iTempUnitIdx == 2))  // 전원부
                {
                    //Power
                    foreach (DataColumn c in dtRslt.Columns)
                    {
                        if (c.ColumnName.Contains("POW_TEMP"))
                        {
                            dgEqpTemp.Columns.Add(new C1.WPF.DataGrid.DataGridNumericColumn()
                            {
                                Name = Util.NVC(c.ColumnName),
               ...
               // dgEqpTemp.ItemsSource = DataTableConverter.Convert(dtRslt);
                ...
                SetChartDataTable(dtRslt,jigCnt,powCnt);
        ...
        private void SetChartDataTable(DataTable dt,int iJigCnt, int iPowCnt)
        {
            string[] sJig = new string[iJigCnt+1];
            string[] sPwCnt = new string[iPowCnt+1];
            
            //Jig
            sJig[0] = "MEAS_TIME";
            for (int i = 1; i < iJigCnt + 1; i++)
            {
                sJig[i] = "JIG_TEMP" + Convert.ToString(i).PadLeft(2, '0');
            }

            _JigGrp = dt.DefaultView.ToTable(false, sJig);

            //Power
            sPwCnt[0] = "MEAS_TIME";
            for (int i = 1; i < iPowCnt + 1; i++)
            {
                sPwCnt[i] = "POW_TEMP" + Convert.ToString(i).PadLeft(2, '0');
            }

            _PowerGrp = dt.DefaultView.ToTable(false, sPwCnt);
        }
        ...
        private void SetSeries()
        {
            ...
            {
                //Jig Chart
                for (int i = 1; i < _JigGrp.Columns.Count; i++)
                {
                    C1.WPF.C1Chart.XYDataSeries x = new XYDataSeries();
                    x.Label = "JIG_TEMP" + Convert.ToString(i).PadLeft(2, '0');
                    x.RenderMode = RenderMode.Default;
                    x.ValueBinding = new Binding("JIG_TEMP" + Convert.ToString(i).PadLeft(2, '0'));

                    chrJig.Data.Children.Add(x);
                }

                chrJig.Data.ItemsSource = DataTableConverter.Convert(_JigGrp);
            }
            ...
            {
                //Power Chart
                for (int i = 1; i < _PowerGrp.Columns.Count; i++)
                {
                    C1.WPF.C1Chart.XYDataSeries x = new XYDataSeries();
                    x.Label = "POW_TEMP" + Convert.ToString(i).PadLeft(2, '0');
                    x.RenderMode = RenderMode.Default;
                    x.ValueBinding = new Binding("POW_TEMP" + Convert.ToString(i).PadLeft(2, '0'));

                    chrPower.Data.Children.Add(x);
                }

                chrPower.Data.ItemsSource = DataTableConverter.Convert(_PowerGrp);
            }
        }

        private void ModifyGraph()
        {
            chrJig.View.AxisX.IsTime = true;
            chrJig.View.AxisX.AnnoFormat = "yyyy-MM-dd HH:mm:ss";
            chrJig.View.AxisX.AnnoAngle = -90;
            chrJig.View.AxisY.AnnoFormat = "#0.##";

            chrPower.View.AxisX.IsTime = true;
            chrPower.View.AxisX.AnnoFormat = "yyyy-MM-dd HH:mm:ss";
            chrPower.View.AxisX.AnnoAngle = -90;
            chrPower.View.AxisY.AnnoFormat = "#0.##";
        }
        ...
        private void changeYScale()
        {
            try
            {
                ...
                //{
                //    chrJig.View.AxisY.Max = Convert.ToDouble(txtUpperTemp.Text);
                //    chrJig.View.AxisY.Min = Convert.ToDouble(txtLowerTemp.Text);

                //    chrPower.View.AxisY.Max = Convert.ToDouble(txtUpperTemp.Text);
                //    chrPower.View.AxisY.Min = Convert.ToDouble(txtLowerTemp.Text);
                //}
                ...
                {
                    chrJig.View.AxisY.Min = double.NaN;
                    chrPower.View.AxisY.Min = double.NaN;
                }
                ...
                {
                    chrJig.View.AxisY.Max = double.NaN;
                    chrPower.View.AxisY.Max = double.NaN;
                }
