...
--------------------------------------------------------------------------------------
 [Change History]
  2018.12.18                                  원/각 특성/Grading 공정진척(NEW)
  2023.03.09  0.2   LEEHJ    SI               소형활성화 MES 복사
**************************************************************************************/
...
{
    public partial class FCS002_302_CONFIRM : C1Window, IWorkArea
    {
        #region Declaration
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            if (_load)
            {
                ...
                //C20210522-000015 : "Pallet ID 전부 NEW” 체크 기능 취소 요청
                string sONLY_All_NEW_RPOD = DataTableConverter.GetValue(dgLot.Rows[0].DataItem, "ONLY_All_NEW_RPOD").GetString();
                if (sONLY_All_NEW_RPOD == "Y")
                {
                    chkAllNew.IsChecked = true;
                    chkAllNew.IsEnabled = false;
                    chkAllNew.Visibility = Visibility.Hidden;
                }
                else
                {
                    chkAllNew.IsChecked = false;
                    chkAllNew.IsEnabled = true;
                    chkAllNew.Visibility = Visibility.Visible;
                }
            }
        ...
        public void SetGridProduct()
        {
            try
            {
                ...
                //2023.06.02 F5300 --> FFI101, FFV101, B1000 수정 
                //06.04 원복
                DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_CONFIRM_PRODUCT_SUM_FO", "INDATA", "OUTDATA", inTable);
                //DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_CONFIRM_PRODUCT_SUM_MB", "INDATA", "OUTDATA", inTable);
        ...
        private void ConfirmProcess()
        {
            try
            {
                ...
                const string bizRuleName = "BR_PRD_REG_END_PROD_LOT_NEW_CM"; //기존 ROUTE
                //const string bizRuleName = "BR_PRD_REG_END_PROD_LOT_NEW_CM_MB"; //신규 ROUTE 
        ...
        private void SetProductCalculator()
        {
            ...
            if (inputQty == 0)
            {
                DataTableConverter.SetValue(dgProduct.Rows[dgProduct.TopRows.Count].DataItem, "DIFF_QTY", 0);
            }
            else
            {
