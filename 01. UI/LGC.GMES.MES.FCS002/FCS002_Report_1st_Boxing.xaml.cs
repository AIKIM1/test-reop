...
--------------------------------------------------------------------------------------
 [Change History]
  ...
  2023.03.16  LEEHJ       소형활성화MES(기존 BOX001_201_CELL_DETL)
...
{
    public partial class FCS002_Report_1st_Boxing : C1Window, IWorkArea
    {
        ...
        public FCS002_Report_1st_Boxing()
        {
            InitializeComponent();
        ...
        private void txtPage_ValueChanged(int iPage)
        {
            try
            {
                ...
                using (Stream stream = a.GetManifestResourceStream("LGC.GMES.MES.FCS002.Report.Pallet_1st_Boxing.xml"))
                {
        ...
        private void PrintView(string sPalletID)
        {
            try
            {
                ...
                // 데이터 조회
                if (bRemainder == true)
                    bizRule = "BR_PRD_GET_TAG_INPALLET_MB";
                else
                    bizRule = "BR_PRD_GET_TAG_PALLET_MB";
