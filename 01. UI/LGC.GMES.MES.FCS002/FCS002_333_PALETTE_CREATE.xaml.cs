...
--------------------------------------------------------------------------------------
 ...
 수정일자    수정자  CSR No           수정내용
 --------------------------------------------------------------------------------------
 2018.12.21  이상훈  C20181121_50523  [CSR ID:3850523] 활성화 Grader In-line화 GMES 기능 추가
 2019.11.05  이상훈  SI                CSR_20191105_01  활성화 후 공정 inbox 자동 포장 기능 추가 대응 개선
**************************************************************************************/
...
{
    public partial class FCS002_333_PALETTE_CREATE : C1Window, IWorkArea
    {
        #region Declaration
        ...
        public FCS002_333_PALETTE_CREATE()
        {
            InitializeComponent();
        }
        #endregion
        ...
        private void SetControl()
        {
            ...
            prodLotBind.Columns.Add("FCS_PLLT_ID", typeof(string)); // CSR_20191105_01 FCS 조회 되어 있는 Pallet 설정
            ...
            //C20181121_50523 오창은 양품/불량 조회 되도록 변경
            string sQltyType = tmps[7] as string;
            SetGradeComboCode(dgPallet, "CAPA_GRD_CODE", sQltyType);
            ...
            else
            {
                ...
                SetGridModifyPallet();
                GetFCSPalletID(drPallet["PALLETE_ID"].ToString()); //CSR_20191105_01 FCS pallet id 조회
            }
        ...
        private void SetControlVisibility()
        {
            ...
            if(!LoginInfo.CFG_SHOP_ID.ToString().Equals("A010"))
            {
                dgLot.Columns[11].Visibility = Visibility.Hidden;
                dgLot.Columns[12].Visibility = Visibility.Hidden;

                tbLaserCleaning.Visibility = Visibility.Collapsed;
                chkLaserCleaning.Visibility = Visibility.Collapsed;

                tbMaskingJig.Visibility = Visibility.Collapsed;
                chkMaskingJig.Visibility = Visibility.Collapsed;
            }
            else
            {
                chkMaskingJig.IsEnabled = false;
            }
        ...
        {
            FCS002_SHIPTO popupShipTo = new FCS002_SHIPTO { FrameOperation = this.FrameOperation };
        ...
        {
            FCS002_SHIPTO popup = sender as FCS002_SHIPTO;
        ...
        private void SetGradeComboCode(C1DataGrid dg, string comTypeCode, string QltyTypeCode = null)
        {
            try
            {
                string sBizName = comTypeCode.Equals("CAPA_GRD_CODE") && LoginInfo.CFG_SHOP_ID.Equals("A010") ? "DA_PRD_SEL_FORM_CAPA_GRD_CODE_LINE_CBO_OC" : "DA_PRD_SEL_FORM_GRADE_TYPE_CODE_LINE_CBO";
                ...
                DataTable dtResult = new ClientProxy().ExecuteServiceSync(sBizName, "INDATA", "OUTDATA", inTable);
        ...
        /// <summary>
        /// CSR_20191105_01 FCS Pallet 매핑 정보 조회
        /// </summary>
        private void GetFCSPalletID(string sLotid)
        {
            try
            {
                if (!LoginInfo.CFG_SHOP_ID.ToString().Equals("A010"))
                ...
                {
                    sFCSPalletId = Util.NVC(result.Rows[0]["FCS_PLLT_ID"].ToString());
                }
                DataTableConverter.SetValue(dgLot.Rows[0].DataItem, "FCS_PLLT_ID", sFCSPalletId);
                //if (!sFCSPalletId.ToString().Equals(""))
                //{
                //    GetFCSPalleInfo(sFCSPalletId);
                //}
                return;
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// CSR_20191105_01 FCS Pallet 상세 정보 조회
        /// </summary>
        /// <param name="sLotid"></param>
        private void GetFCSPalleInfo(string sLotid)
        {
            try
            {
                if (!LoginInfo.CFG_SHOP_ID.ToString().Equals("A010"))
                ...
                DataRow Indata = IndataTable.NewRow();
                Indata["AREAID"] = LoginInfo.CFG_AREA_ID.ToString();
                Indata["PALLET_ID"] = sLotid;
                IndataTable.Rows.Add(Indata);

                DataTable result = new ClientProxy().ExecuteServiceSync("BR_INF_SEL_TC_AUTOBOXING_BOX_PALLET_INFO", "INDATA", "OUTDATE", IndataTable);

                if (result != null && result.Rows.Count > 0)
                {
                    Util.GridSetData(dgPallet, result, null, true);
                }
        ...
        private void SetCreatePallet()
        {
            try
            {
                ...
                //[C20181121_50523] 활성화 Grader In-line화 GMES 기능 추가
                ...
                inTable.Columns.Add("FCS_PALLET_ID", typeof(string)); //CSR_20191105_01 추가

                inTable.Columns.Add("LASER_CLEANING_FLAG", typeof(string));
                inTable.Columns.Add("MASKING_JIG_FLAG", typeof(string));
                ...
                newRow["FCS_PALLET_ID"] = DataTableConverter.GetValue(dgLot.Rows[0].DataItem, "FCS_PLLT_ID"); //CSR_20191105_01 추가

                newRow["LASER_CLEANING_FLAG"] = (bool)chkLaserCleaning.IsChecked ? "Y" : "N";
                newRow["MASKING_JIG_FLAG"] = (bool)chkMaskingJig.IsChecked ? "Y" : "N";
        ...
        private void SetModifyPallet()
        {
            try
            {
                ...
                const string bizRuleName = "BR_PRD_REG_MODIFY_PALLET";
        ...
        private bool ValidateCreate()
        {
            ...
            foreach (DataRow row in drChk)
            {
                ...
                if (string.IsNullOrWhiteSpace(row["CAPA_GRD_CODE"].ToString()))
                {
                    ...
                    //return false;
                    // [C20181121_50523] 오창만 필수로 입력 되도록 변경
        ...
        private void SetBoxQty(bool FormLoad)
        {
            ...
            string sFCSPalletId = Util.NVC(DataTableConverter.GetValue(dgLot.Rows[0].DataItem, "FCS_PLLT_ID"));
        ...
        private void btnFCSPallet_Click(object sender, RoutedEventArgs e)
        {
            if (LoginInfo.CFG_SHOP_ID.ToString().Equals("A010"))
            {
                SetPopupGoodPalletSelect();
            }
        }

        /// <summary>
        /// /// CSR_20191105_01 FCS pallet 조회 popup 실행 이벤트
        /// </summary>
        private void SetPopupGoodPalletSelect()
        {
            FCS002_PALLET_FCS_LIST popupPalletFCSList = new FCS002_PALLET_FCS_LIST { FrameOperation = this.FrameOperation };


            object[] parameters = new object[1];  
            parameters[0] = LoginInfo.CFG_AREA_ID;                     
            C1WindowExtension.SetParameters(popupPalletFCSList, parameters);

            popupPalletFCSList.Closed += new EventHandler(popupPalletCreate_Closed);

            foreach (Grid tmp in Util.FindVisualChildren<Grid>(Application.Current.MainWindow))
            {
                if (tmp.Name == "grdMain")
                {
                    tmp.Children.Add(popupPalletFCSList);
                    popupPalletFCSList.BringToFront();
                    break;
                }
            }
        }
        ...
        /// <summary>
        /// CSR_20191105_01 FCS pallet 조회 popup 종료 처리 이벤트
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void popupPalletCreate_Closed(object sender, EventArgs e)
        {
            FCS002_PALLET_FCS_LIST popup = sender as FCS002_PALLET_FCS_LIST;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                string sFCSPalletID = Util.NVC(popup.FCSPALLETID);
                DataTableConverter.SetValue(dgLot.Rows[0].DataItem, "FCS_PLLT_ID", sFCSPalletID);
                if (!sFCSPalletID.ToString().Equals(""))
                {
                    GetFCSPalleInfo(sFCSPalletID);
        ...
        private void chkLaserCleaning_Checked(object sender, RoutedEventArgs e)
        {
            chkMaskingJig.IsEnabled = true;
        }

        private void chkLaserCleaning_UnChecked(object sender, RoutedEventArgs e)
        {
            chkMaskingJig.IsEnabled = false;
            chkMaskingJig.IsChecked = false;
        }
    }
}
