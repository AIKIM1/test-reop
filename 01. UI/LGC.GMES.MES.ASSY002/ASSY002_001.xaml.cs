...
--------------------------------------------------------------------------------------
 [Change History]
   2017.05.29   신광희C   : 와인더 공정진척 UserControl(UcAssyCommand,UcAssySearch,UcAssyProdLot,UcAssyResultDetail,UcAssyDataCollect,UcAssyShift) 내에서 발생하는 이벤트 및 메소드 분리 작업 
                          : 1. 서로 다른 UserControl 의 영향이 가지 않는 경우 각각의 UserControl 내부에서 이벤트 및 메소드 처리를 원칙으로 함
                            2. UserControl 간의 참조가 필요한 경우 ASSY002_001.xaml.cs 파일에서 처리 함
   2019.03.05   강민준C   : CNJ 원형9,10호 Inline 와인더 이외(LOT_ID) 처리 SetComboBox()
   ...
   2024.01.25   오수현    : E20230901-001504 Windgind 공정의 실적확정 처리 후 'Winding 이력카드' Popup 호출을 skip 하는 경우(MMD 대차ID 항목 표기 여부에 등록된 라인) 추가. UcAssyResultDetail에 ChangeEquipment()함수 호출 부분 추가
                          : 대차ID 항목 표기여부의 이력카드 SKIP 여부를 속성2로 수정
   2024.02.02   오수현    : E20230901-001504 실적상세 영역에 대차ID 항목 보이도록 함
   2024.01.30   차의진    : 실적확정 시 허용범위를 넘은 물품 요청자, 사유 등 정보 DB 저장 
   2024.09.06   윤현호    : 대차 저장 Biz 변경  
**************************************************************************************/
...
{
    public partial class ASSY002_001 : IWorkArea
    {
        #region Declaration
        ...
        private void Initialize()
        {
            ...
            UcAssyResultDetail?.ChangeEquipment(equipmentCode, equipmentSegmentCode); // E20230901-001504 추가
            UcAssyResultDetail.SetControlProperties(); // E20230901-001504 추가
        ...
        private void SetEventInUserControls()
        {
            if (UcAssyCommand != null)
            {
                ...
                UcAssyCommand.ButtonModlChgCheck.Click += ButtonModlChgCheck_Click;
            ...
            if (UcAssyResultDetail != null)
            {
                ...
                UcAssyResultDetail.ButtonCSTIDSave.Click += BtnCSTIDSave_Click;
            }
        }
            ...
            Util.MessageConfirm("SFU1706", (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    ...
                    {   // 양품량이 0입니다. 그래도 실적확정 하시겠습니까?
                        Util.MessageConfirmByWarning("SFU4497", result1 =>
                        {
        ...
        private void ButtonEditEqptQty_Click(object sender, RoutedEventArgs e)
        {
            ...
            string password = "";
            if (isPasswordCheckArea(out password))
            {
                checkPasswordPopup(password);
            }
            else
            {
                modifyEqptInputQtyPopup();
            }
        }
        ...
        private void modifyEqptInputQtyPopup()
        {
        ...
        private bool isPasswordCheckArea(out string pw)
        {
            ...
            dr["CMCDTYPE"] = "ASSY_EQPT_INPUT_QTY_PW";
            ...
            {
                pw = Util.NVC(searchResult.Rows[0]["ATTRIBUTE1"].GetString());
                return true;
            }
        ...
        private void checkPasswordPopup(string password)
        {
            CMM_ASSY_PASSWORD popupPW = new CMM_ASSY_PASSWORD();
            object[] parameters = new object[1];
            parameters[0] = password;
            C1WindowExtension.SetParameters(popupPW, parameters);
            popupPW.Closed += new EventHandler(popupPW_Closed);
            this.Dispatcher.BeginInvoke(new Action(() => popupPW.ShowModal()));
        }

        private void popupPW_Closed(object sender, EventArgs e)
        {
            CMM_ASSY_PASSWORD popup = sender as CMM_ASSY_PASSWORD;
            if (popup != null && popup.DialogResult == MessageBoxResult.Cancel)
            {
                Util.MessageValidation("SFU8213");
                return;
            }
            modifyEqptInputQtyPopup();
        }
        ...
        protected virtual void OnCloseVersion(object sender, EventArgs e)
        {
            ...
            {
                //    if (window._ReturnRecipeNo.ToString() == "선택취소")
                //    {
            ...
            try
            {
                ...
                new ClientProxy().ExecuteService_Multi(bizRuleName, "INDATA", null, (bizResult, bizException) =>
                {
                    try
                    {
                        ...
                        {
                            if (!GetWindingHistoryCardSkipYn()) // E20230901-001504
                            {
                                // Winding 이력카드 출력
        ...
        #region E20230901-001504 실적확정 처리 후 이력카드 팝업 SKIP 여부
        private bool GetWindingHistoryCardSkipYn()
        {
            if (String.IsNullOrEmpty(ComboEquipmentSegment.SelectedValue.GetString()))
            {
                return false;
            }

            bool isPopupSkipYn = false;
            ...
            dr["CBO_CODE"] = ComboEquipmentSegment.SelectedValue.GetString();
            dt.Rows.Add(dr);

            DataTable dtResult = new ClientProxy().ExecuteServiceSync("DA_BAS_SEL_COMMONCODE_ATTRIBUTE", "RQSTDT", "RSLTDT", dt);

            if (dtResult.Rows.Count != 0 && dtResult.Rows[0]["ATTRIBUTE2"].ToString() == "Y") //이력카드 Skip 여부
            {
                isPopupSkipYn = true;
            }
            else
        ...
        private void SaveCSTID()
        {
            ...
            DataRow newRow = inTable.NewRow();
            newRow["LOTID"] = Util.NVC(UcAssyResultDetail.TxtLotID.Text); //_util.GetDataGridFirstRowBycheck(DgProductLot, "CHK").Field<string>("LOTID").GetString();
            newRow["CSTID"] = Util.NVC(UcAssyResultDetail.TxtCSTID.Text);
            newRow["USERID"] = LoginInfo.USERID;
            newRow["SRCTYPE"] = SRCTYPE.SRCTYPE_UI;
            ...
            Util.MessageConfirm("SFU1241", (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    new ClientProxy().ExecuteService("BR_PRD_REG_CSTID_USING_WND_CT", "INDATA", null, inTable, (searchResult, searchException) =>
                    {
                        try
                        {
        ...
        private void SumDefectQty()
        {
            try
            {
                ...
                if (CommonVerify.HasTableRow(dtTmp))
                {
                    ...
                    DataTableConverter.SetValue(DgDefectDetail.Rows[DgDefectDetail.TopRows.Count].DataItem, "DTL_LOSS", dtTmp.Compute("SUM(RESNQTY)", "ACTID = 'LOSS_LOT' AND RESNCODE <> 'PL17L07' AND RSLT_EXCL_FLAG <> 'Y' ").GetString().Equals("") ? 0 : double.Parse(dtTmp.Compute("SUM(RESNQTY)", "ACTID = 'LOSS_LOT' AND RESNCODE <> 'PL17L07' AND RSLT_EXCL_FLAG <> 'Y'").GetString()));
        ...
        public void CalculateDefectQty()
        {
            ...
            if (_processCode.Equals(Process.WINDING))
            {
                if (_isSmallType)
                {
            ...
            if (CommonVerify.HasDataGridRow(DgDefectDetail))
            {
                if (DgDefectDetail.Rows.Count - DgDefectDetail.TopRows.Count > 0)
                {
                    ...
                    ////패턴초과 로직 추가 2020-09-01
                    UcAssyResultDetail.TxtPatternResultQty.Text = (UcAssyDataCollect.GetPatternQty() * -1).ToString("##,###");
                    patternQty = (UcAssyDataCollect.GetPatternQty()).GetDouble();

                    if (Math.Abs((eqptQty + inputQtyByType) - (defectQty + goodQty)) > 0)
                    {
                        UcAssyResultDetail.TextAssyResultQty.Text = ((eqptQty + inputQtyByType) - (defectQty + goodQty) + patternQty).ToString("##,###");
        ...
        private bool ValidationConfirm()
        {
            ...
            if (DataTableConverter.GetValue(DgDefectDetail.Rows[DgDefectDetail.TopRows.Count].DataItem, "GOODQTY").GetDecimal() == 0)
            {
                //전극 기인 수량을 확인하세요. C20210826-000188 로 제거
                /*
                ...
                */
                ...
                //C20210826-000188 로 특이사항 필수 입력으로 변경
            ...
            //C20210119-000409
            for (int inx = 0; inx < DgDefect.Rows.Count - DgDefect.BottomRows.Count; inx++)
            {
                double resnQty = Util.NVC(DataTableConverter.GetValue(DgDefect.Rows[inx].DataItem, "RESNQTY")).Equals("") ? 0 : double.Parse(Util.NVC(DataTableConverter.GetValue(DgDefect.Rows[inx].DataItem, "RESNQTY")));
                string winderRemarkMandatoryWipresoncollect = Util.NVC(DataTableConverter.GetValue(DgDefect.Rows[inx].DataItem, "WINDER_REMARK_MANDATORY_WIPRESONCOLLECT"));

                if (winderRemarkMandatoryWipresoncollect.Equals("Y") && resnQty != 0)
                {
                    //동별 공통코드 WINDER_REMARK_MANDATORY_WIPRESONCOLLECT 에 지정된 RESN일 경우 수량이 입력되면 특이사항 입력 필수
                    if (string.IsNullOrEmpty(new TextRange(UcAssyResultDetail.TextRemark.Document.ContentStart, UcAssyResultDetail.TextRemark.Document.ContentEnd).Text))
                    {
                        Util.MessageValidation("SFU3774");  //기타 불량 및 Loss 조정 내역이 특이사항에 입력 및 저장되지 않아 실적 확정 불가능합니다.
                        return false;
                    }
                }
            }
            ...
            //C20210126-000047
            //설비 Loss 등록 여부 체크, 서버시간 기준으로 오늘, 오늘-1 에 MMD 설비에서 설정된 기준시간 이상인 미입력 Loss 가 있으면 확정 불가
            DataTable dtEqpLossInfo = Util.Get_EqpLossInfoFromToyyyyMMdd(ComboEquipment.SelectedValue.ToString(), Process.WINDING, (DateTime.Now.AddDays(-1)).ToString("yyyyMMdd"), DateTime.Now.ToString("yyyyMMdd"));
            ...
            if (dtEqpLossInfo != null && dtEqpLossInfo.Rows.Count > 0 && dtEqpLossInfo.Columns.Contains("NOINPUT_CNT") && Util.NVC_Int(dtEqpLossInfo.Rows[0]["NOINPUT_CNT"]) > 0)
            {
                ...
                //미입력한 설비 Loss가 존재합니다. 입력하시겠습니까? %1
                Util.MessageConfirm("SFU3775", (result) =>
                {
                    if (result == MessageBoxResult.OK)
                    {
                        CMM_COM_EQPT_LOSS popupEqptLoss = new CMM_COM_EQPT_LOSS { FrameOperation = FrameOperation };
                        object[] parameters = new object[3];
                        parameters[0] = ComboEquipment.SelectedValue;
                        parameters[1] = _processCode;
                        parameters[2] = ComboEquipmentSegment.SelectedValue;

                        C1WindowExtension.SetParameters(popupEqptLoss, parameters);

                        popupEqptLoss.Closed += new EventHandler(popupEqptLoss_Closed);

                        // 팝업 화면 숨겨지는 문제 수정.
                        grdMain.Children.Add(popupEqptLoss);
                        popupEqptLoss.BringToFront();
                    }
                }, param);
        ...
        private void popupEqptLoss_Closed(object sender, EventArgs e)
        {
            CMM_COM_EQPT_LOSS popup = sender as CMM_COM_EQPT_LOSS;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
        ...
        private bool ValidationInspectionLot()
        {
            ...
            if (CommonVerify.HasTableRow(dtResult))
            {
                ...
                // Lot 단위 필수 검사 항목 입력 필요 : [%1]
                Util.MessageInfo("SFU5078", stnMsg);
                return false;
            }
            else
            {
        ...
        private bool ValidationInspectionTime()
        {
            ...
            if (CommonVerify.HasTableRow(dtResult))
            {
                ...
                // Time 단위 필수 검사 항목 입력 필요 :[%1]
                Util.MessageInfo("SFU5079", stnMsg);
                return false;
            }
            else
            {
        ...
        {
            if (UcAssyResultDetail.TxtCSTID.IsNullOrEmpty())
                return false;
        ...
        private void SetComboBox()
        {
            ...
            // CNJ 원형9,10호 화인더 이외(LOT_ID)  //원각 : CR , 초소형 : CS
            const string gubun = "CR";

            //String[] sFilter = { LoginInfo.CFG_AREA_ID, gubun, _processCode};
            String[] sFilter = { LoginInfo.CFG_AREA_ID, gubun, _processCode, "LOT_ID" };
        ...
        private void ButtonModlChgCheck_Click(object sender, RoutedEventArgs e)
        {
            if (!ValidationModlChg())
                return;

            CMM_ASSY_MODL_CHG_CHECK_LIST popupModlChgCheckList = new CMM_ASSY_MODL_CHG_CHECK_LIST
            {
                FrameOperation = FrameOperation
            };
            ...
            C1WindowExtension.SetParameters(popupModlChgCheckList, parameters);
            popupModlChgCheckList.Closed += new EventHandler(popupModlChgCheckList_Closed);

            // 팝업 화면 숨겨지는 문제 수정.
            this.Dispatcher.BeginInvoke(new Action(() => popupModlChgCheckList.ShowModal()));
        }

        private bool ValidationModlChg()
        {
            if (_util.GetDataGridCheckFirstRowIndex(DgProductLot, "CHK") < 0)
            {
                //Util.Alert("선택된 항목이 없습니다.");
        ...
        private void popupModlChgCheckList_Closed(object sender, EventArgs e)
        {
            CMM_ASSY_MODL_CHG_CHECK_LIST popup = sender as CMM_ASSY_MODL_CHG_CHECK_LIST;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
            }
            this.grdMain.Children.Remove(popup);
