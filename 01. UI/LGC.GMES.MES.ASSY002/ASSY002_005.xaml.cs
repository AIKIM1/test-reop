/*************************************************************************************
 Created Date : 2017.03.02
      Creator : 신광희C
   Decription : Assembly 공정진척(초소형) 작업 화면
--------------------------------------------------------------------------------------
 [Change History]
   2017.05.29   신광희C   : Assembly 공정진척(초소형) UserControl(UcAssyCommand,UcAssySearch,UcAssyProdLot,UcAssyResultDetail,UcAssyDataCollect,UcAssyShift) 내에서 발생하는 이벤트 및 메소드 분리 작업 
                          : 1. 서로 다른 UserControl 의 영향이 가지 않는 경우 각각의 UserControl 내부에서 이벤트 및 메소드 처리를 원칙으로 함
                            2. UserControl 간의 참조가 필요한 경우 ASSY002_005.xaml.cs 파일에서 처리 함
...
{
    public partial class ASSY002_005 : IWorkArea
    {
        #region Declaration
        ...
        /// <summary>
        /// Description     :   ASSY002_005.xaml Loaded 이벤트(Process Code 및 초소형 여부 설정, 권한별 버튼 설정, UserControl 정의 및 이벤트 설정)
        /// Author          :   신 광희
        ...
        private void ButtonStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            ASSY002_005_RUNSTART popupRunStart = new ASSY002_005_RUNSTART { FrameOperation = FrameOperation };
        ...
        {
            ASSY002_005_RUNSTART popup = sender as ASSY002_005_RUNSTART;
            if (popup != null && popup.DialogResult == MessageBoxResult.OK)
            {
                ClearControls();
                GetProductLot();
        ...
        private string GetConfirmDate()
        {
            ...
            return string.IsNullOrEmpty(Util.NVC(searchResult.Rows[0]["CALDATE_LOT"]).Trim()) ? Util.NVC(searchResult.Rows[0]["NOW_CALDATE"]).GetString() : Util.NVC(searchResult.Rows[0]["CALDATE_LOT"]).GetString();
        }
        ...
        private Decimal GetGoodQty()
        {
            ...
            DataTable searchResult = new ClientProxy().ExecuteServiceSync(bizRuleName, "RQSTDT", "RSLTDT", inDataTable);
            return CommonVerify.HasTableRow(searchResult) ? searchResult.AsEnumerable().Sum(s => s.Field<Decimal>("WIPQTY_INPUT")) : 0;
        }
        ...
        private Decimal GetInputQty()
        {
            ...
            return CommonVerify.HasTableRow(dt) ? dt.AsEnumerable().Sum(s => s.Field<Decimal>("INPUT_QTY")) : 0;
        }
        ...
        private decimal GetReInputQty()
        {
            ...
            if (UcAssyDataCollect.TabDefectReInput.Visibility == Visibility.Visible)
            {
                ...
                ds.Tables.Add(inTable);
                //string xml = ds.GetXml();
                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi(bizRuleName, "INDATA", "OUTTYPE,OUTDATA",
                    ds, FrameOperation.MENUID);
                if (CommonVerify.HasTableInDataSet(dsResult))
                {
                    return CommonVerify.HasTableRow(dsResult.Tables["OUTDATA"])
                        ? dsResult.Tables["OUTDATA"].AsEnumerable().Sum(s => s.Field<decimal>("RESNQTY"))
                        : 0;
                }
                return 0;
            }
