/*************************************************************************************
 Created Date : 2016.11.16
   ...
   Decription : Winding 공정 착공
--------------------------------------------------------------------------------------
 [Change History]
 2017.07.10   INS 이대영D : 파라미터 수정
 2019.03.05   INS 강민준 : 남경 원형9,10호 in-line 원형타입으로 바꿈 ASSY002_001_RUNSTART 참조
 2023.12.05   황재원      : E20231127-000340 대기 Pancake 팝업 호출 시 극성 정보 선택 오류 수정
 2023.12.14	  황재원	  : E20231207-001471 대기 Pancake 조회창에서 선택된 항목의 극성 정보와 작업시작 창에서 극성 정보 매칭 오류 수정
**************************************************************************************/
...
{
    public partial class ASSY002_010_RUNSTART : C1Window, IWorkArea
    {
        #region Declaration
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            workOrderBind.Columns.Add("WOID", typeof(string));
            workOrderBind.Columns.Add("WO_DETL_ID", typeof(string));
            workOrderBind.Columns.Add("LOTYNAME", typeof(string));
            workOrderBind.Columns.Add("PRJT_NAME", typeof(string));
            workOrderBind.Columns.Add("PROD_VER_CODE", typeof(string));
            workOrderBind.Columns.Add("PRODID", typeof(string));
            workOrderBind.Columns.Add("PRODNAME", typeof(string));
            workOrderBind.Columns.Add("INPUT_QTY", typeof(string));
        ...
        private static DataTable AddVisibilityColumn(DataTable dt)
        {
            ...
            {
                if (row["MOUNT_MTRL_TYPE_CODE"].GetString() == "PROD_MTRL" || row["MOUNT_MTRL_TYPE_CODE"].GetString() == "MTRL")
                {
            ...
            if (e.Key == System.Windows.Input.Key.Enter)   // BARCODE 입력 시
            {
                ...
                if (!string.IsNullOrEmpty(txtBox.Text.Trim()))
                {
                    ...
                    if (txtBox.Text.Trim().Length == 10)
                    {
                        ...
                        {
                            // SET VALUE
                            DataTableConverter.SetValue(dgInputProduct.Rows[rowIndex].DataItem, "LOTID", Convert.ToString(dt.Rows[0]["LOTID"]));         // LOTID
                            DataTableConverter.SetValue(dgInputProduct.Rows[rowIndex].DataItem, "PRODID", Convert.ToString(dt.Rows[0]["PRODID"]));       // 제품ID
                            DataTableConverter.SetValue(dgInputProduct.Rows[rowIndex].DataItem, "PRODNAME", Convert.ToString(dt.Rows[0]["PRODNAME"]));   // 제품명
                            DataTableConverter.SetValue(dgInputProduct.Rows[rowIndex].DataItem, "WIPQTY", Convert.ToString(dt.Rows[0]["WIPQTY"]));       // 수량
                            DataTableConverter.SetValue(dgInputProduct.Rows[rowIndex].DataItem, "PRJT_NAME", Convert.ToString(dt.Rows[0]["PRJT_NAME"])); // PJT
                            return;
                        }
                    }
                }
                ...
                if (_InputProduct != null)
                {
                    ...
                    parameters[4] = "1";
                    parameters[5] = Util.NVC(txtBox.Text.Trim());                                                                   // LOTID
                    parameters[6] = "D";                                                                                            // Page 구분자
                    parameters[7] = Util.NVC(eqptID);                                                                               // 설비
        ...
        private void btnSearch_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_InputProduct != null)
            {
				...
				//2023.12.05 E20231127-000340 대기 Pancake 팝업 호출 시 극성 정보 선택 오류 수정
				string sElectrodeCode = dataRow.Row["PRDT_CLSS_CODE"].ToString();
                int rowIndex = -1;
        ...
        private void InputHalfProduct_Closed(object sender, EventArgs e)
        {
            ...
            if (_InputProduct.DialogResult == MessageBoxResult.OK)
            {
                ...
                for (int i = 0; i < dgInputProduct.Rows.Count; i++)
                {
					//2023.12.14 E20231207-001471 대기 Pancake 조회창에서 선택된 항목의 극성 정보와 작업시작 창에서 극성 정보 매칭 오류 수정
					if (string.Equals(selectRow[10], dgInputProduct[i, 2].Text))
                    {
