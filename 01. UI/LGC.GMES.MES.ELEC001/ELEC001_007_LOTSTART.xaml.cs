...
{
    public partial class ELEC001_007_LOTSTART : C1Window, IWorkArea
    {
        #region Initialize
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            //{
            //    coatSide = tmps[4].ToString();
            //    if (coatSide.Equals("B"))
            //    {
            //        dgFoil.Columns["Button"].Visibility = Visibility.Visible;
            //        sLargeLot = string.Empty;
            //        sLotID = tmps[5].ToString();  // 단면코터 작업시작 Lot ID
            //        chkFirstFlag.Visibility = Visibility.Collapsed;
            //    }
            //}
        ...
        public ELEC001_007_LOTSTART(Dictionary<string, string> dic)
        {
            InitializeComponent();
            try
            {
                ...
                /*
                if (dicParam["SINGL"].Equals("true"))
                { 
                    if (coatSide.Equals("B"))
                    {
                        dgFoil.Columns["Button"].Visibility = Visibility.Visible;
                        sLargeLot = string.Empty;
                        if (dicParam.ContainsKey("LOTID_PR")) sLotID = dicParam["LOTID_PR"];
                        chkFirstFlag.Visibility = Visibility.Collapsed;
                    }
                }
                */
        ...
        private void GetCurrentMount()
        {
            try
            {
                ...
                //if (!string.IsNullOrEmpty(coatSide))
                //    Indata["COATING_SIDE_TYPE_CODE"] = !coatSide.Equals("B") ? null : coatSide;
                if (!string.IsNullOrEmpty(coatSide))
                ...
                if (dtMain.Rows.Count > 0)
                {
                    ...
                    //dgFoil.LoadedCellPresenter += OnLoadedCellPresenter;

                    //단면코터 Back 시작 시 Top Coater Lot 자동 셋팅 ************************************
                    /*
                    if (IsSingleCoater == true)
                    {
                        if (Foildt.Rows.Count > 0 && coatSide.Equals("B"))
                        {
                            DataTable dt = DataTableConverter.Convert(dgFoil.ItemsSource);

                            DataTableConverter.SetValue(dgFoil.Rows[0].DataItem, "INPUT_LOTID", sLotID);
                        }
                    }
                    */
                    //***********************************************************************************
        ...
        private bool ValidData()
        {
            ...
            if (txtLotID.Text.Trim().Equals("") && !Convert.ToBoolean(chkFirstFlag.IsChecked))
            {
                ...
                //if (IsSingleCoater == false || (IsSingleCoater == true && !coatSide.Equals("B")))
                //{
                ...
                if (IsSingleCoater == true)
                {
                    if (coatSide.Equals("B"))
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            if (i == 0)
                            {
                                if (string.IsNullOrEmpty(Util.NVC(dt.Rows[i]["INPUT_LOTID"])))
                                    continue;
        ...
        private void btnWaitList_Click(object sender, RoutedEventArgs e)
        {
            ELEC001_007_SINGLECT _LotList = new ELEC001_007_SINGLECT();
            _LotList.FrameOperation = FrameOperation;
            RowIndex = ((C1.WPF.DataGrid.DataGridCellPresenter)((sender as Button).Parent)).Row.Index;
            ...
            if (_LotList != null)
            {
                ...
                Parameters[0] = Process.COATING;
                Parameters[1] = Util.NVC(txtWorkOrder.Text);
                C1WindowExtension.SetParameters(_LotList, Parameters);

                _LotList.Closed += new EventHandler(LotList_Closed);
                _LotList.ShowModal();
                _LotList.CenterOnScreen();
            }
        }
        private void LotList_Closed(object sender, EventArgs e)
        {
            ELEC001_007_SINGLECT _Lot = sender as ELEC001_007_SINGLECT;

            if (_Lot.DialogResult == MessageBoxResult.OK)
            {
                /*
                DataTable dt = DataTableConverter.Convert(dgFoil.ItemsSource);
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        DataTableConverter.SetValue(dgFoil.Rows[i].DataItem, "INPUT_LOTID", "");
                    }
                }
                */
                DataTableConverter.SetValue(dgFoil.Rows[0].DataItem, "INPUT_LOTID", _Lot._ReturnLotID);
            }
        }

        protected virtual void OnLoadedCellPresenter(object sender, C1.WPF.DataGrid.DataGridCellEventArgs e)
        {
