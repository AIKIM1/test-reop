...
--------------------------------------------------------------------------------------
 [Change History]
  2024.03.20  백광영 : ELEC001_028 Copy (VD 재작업 공정진척)
...
{
    public partial class ELEC001_041 : UserControl, IWorkArea
    {
                ...
                if (e.Column is C1.WPF.DataGrid.DataGridCheckBoxColumn)
                {
                    ...
                    else
                    {
                        for (int i = 0; i < dgLotInfo.Rows.Count; i++)
                        {
                            if (idx != i)
                            {
                                if ((Util.NVC(DataTableConverter.GetValue(dgLotInfo.Rows[idx].DataItem, "CHK")).Equals("0") || Util.NVC(DataTableConverter.GetValue(dgLotInfo.Rows[idx].DataItem, "CHK")).Equals("False")))
                                {
            ...
            Util.MessageConfirm("SFU1435", (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    ...
                    else
                    {
                        ...
                        //list = list.GroupBy(c => c["REP_LOTID"]).Select(group => group.First()).ToList();
                        for (int i = 0; i < list.Count; i++)
                        {
                            row = inMtrl.NewRow();
                            row["LOTID"] = list[i]["LOTID"];
                            indataSet.Tables["IN_LOT"].Rows.Add(row);
                        }
                    }
                    ...
                    try
                    {
                        ...
                        new ClientProxy().ExecuteService_Multi("BR_PRD_REG_START_LOT_VD_REWORK", "IN_EQP,IN_LOT", null, (bizResult, bizException) =>
                        {
                            try
                            {
        ...
        private void btnRunCancel_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                new ClientProxy().ExecuteServiceSync("BR_PRD_REG_CANCEL_START_LOT_VD_REWORK", "RQSTDT", null, dt);
                Util.MessageInfo("SFU1839");  //작업 시작 취소
                GetProductLot();
                GetDefectList();
            }
            catch (Exception ex)
        ...
        private void FillLotInfo()
        {
            try
            {
                ...
                if (!cLot.STARTTIME_CHAR.Equals("") && !cLot.ENDTIME_CHAR.Equals(""))
                {
                    ...
                    if (oper < 0)
                    {
                        Util.MessageValidation("SFU3037");  // 종료시간이 시작시간보다 전 시간 일 수 없습니다.
                        return;
                    }
