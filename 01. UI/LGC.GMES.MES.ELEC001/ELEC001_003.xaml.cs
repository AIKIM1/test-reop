...
{
    public partial class ELEC001_003 : UserControl, IWorkArea
    {
        ...
        private void cboMtrlLOTID_SelectedValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            try
            {
                if (cboMtrlLOTID.Text == "-SELECT-" || cboMtrlLOTID.Text == "") return;
                if (string.IsNullOrEmpty(txtRequestNo.Text))
                {
                    //투입요청서를 입력하세요.
                    Util.MessageInfo("SFU1976", (result) =>
        ...
        #region Mehod
        private bool SupplierProdVerCheck() //생산계획 제품버전과 업체별 제품 버전 비교 비즈룰
        {
            try
            {
        ...
        private void GetRequest()
        {
            try
            {
                ...
                DataTable dtMain = new ClientProxy().ExecuteServiceSync("DA_PRD_SEL_MTRL_INPUT_REQUEST", "INDATA", "RSLTDT", IndataTable);

                //dgRequest.ItemsSource = DataTableConverter.Convert(dtMain);
        ...
        private void GetRequstList(Object SelectedItem)
        {
            ...
            try
            {
                ...
                //dgMaterial.ItemsSource = DataTableConverter.Convert(dtMain);
            }
            catch (Exception ex)
            {
                Util.MessageException(ex);
