/*************************************************************************************
 Created Date : 2020.11.16
      Creator : 신광희
   Decription : CNB2동 증설 - Notching 공정 - 실적확정(ASSY004_001_CONFIRM Copy 후 작성)
--------------------------------------------------------------------------------------
 [Change History]
  2020.11.16  신광희 : Initial Created.
  2024.02.15  남기운 : (NERP 대응 프로젝트)실적처리 허용비율 초과 입력 기능추가
**************************************************************************************/
...
{
    /// <summary>
    /// ASSY005_001_CONFIRM.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY005_001_CONFIRM : C1Window, IWorkArea
    {
        ...
        private bool _isSideRollDirctnUse = false;          //무지부/권취 방향 사용여부, 2024-11-26, 김선영
        ...
        private void InitializeControls()
        {
            ...
            // 무지부/권취 두 방향 모두 사용하는 AREA에선 코터 완공 시 무지부/권취 방향 저장할 수 있도록 함, 2024-11-26, 김선영, START
            if (string.Equals(_ProcID, Process.NOTCHING) && _Util.IsCommonCodeUse("NON_COATED_WINDING_DIRCTN_USE_AREA", LoginInfo.CFG_AREA_ID))
            {
                ...
                txtSSWD.Visibility = Visibility.Visible;
                btnSSWD.Visibility = Visibility.Visible;
            }
            else
            {
                _isSideRollDirctnUse = false;
                txtSSWD.Visibility = Visibility.Collapsed;
                btnSSWD.Visibility = Visibility.Collapsed;
            }
            // 무지부/권취 두 방향 모두 사용하는 AREA에선 코터 완공 시 무지부/권취 방향 저장할 수 있도록 함, 2024-11-26, 김선영, END
        ...
        private double GetLotINPUTQTY(string sLotID)
        {
            ...
            if (dgQty != null)
            {
                ...
                {                    
                    dInputQty = Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "EQPT_INPUT_QTY")).Equals("") ? 0 : double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "EQPT_INPUT_QTY")));
                }
            }
        ...
        private void GetLotInfo()
        {
            try
            {
                ...
                if (dtRslt != null && dtRslt.Rows.Count > 0)
                {
                    ...
                    if (dtRslt.Columns.Contains("OUT_RMN_UNMNT_TYPE_CODE") &&
                       !Util.NVC(dtRslt.Rows[0]["OUT_RMN_UNMNT_TYPE_CODE"]).Equals(""))
                    {
                        cboEndType.SelectedValue = Util.NVC(dtRslt.Rows[0]["OUT_RMN_UNMNT_TYPE_CODE"]);
                    }
        ...
        private void Save()
        {
            try
            {
                ...
                inDataTable.Columns.Add("LOT_SPLIT", typeof(string));       //2024-11-27, 김선영
                ...
                if (_isSideRollDirctnUse)    //무지부 방향/권취방항 추가, 2024-11-26, 김선영 
                {
                ...
                newRow["EQPTID"] = _EqptID;                
                newRow["USERID"] = LoginInfo.USERID;
                newRow["LOT_SPLIT"] = "N";          //2024-11-27, 김선영
                ...
                new ClientProxy().ExecuteService_Multi("BR_PRD_REG_END_LOT_NT_L", "INDATA,IN_INPUT,IN_OUTPUT,IN_PROC_WRKR", "OUTDATA", (bizResult, bizException) =>
                {
                    try
                    {
                        if (bizException != null)
                        ...
                        if ((bool)bizResult?.Tables?.Contains("OUTDATA") && bizResult?.Tables["OUTDATA"]?.Rows?.Count > 0)
                        {
                            if (Util.NVC(bizResult?.Tables["OUTDATA"]?.Rows?[0]["RESULT_VALUE"]).ToUpper().Equals("OK"))
                            {
                                Util.MessageInfo("SFU1275");
                            }
                            else
                            {
                                Util.MessageInfo(Util.NVC(bizResult?.Tables["OUTDATA"]?.Rows?[0]["RESULT_CODE"]), Util.NVC(bizResult?.Tables["OUTDATA"]?.Rows?[0]["INPUT_LOTID"]));
                            }
                        }
                        else
                        {
        ...
        private void GetWorkUser()
        {
            if (string.IsNullOrWhiteSpace(_WorkerID))
            {
                try
                {
                    DataTable inTable = new DataTable();
                    ...
                    {
                        txtUserName.Text = dtResult.Rows[0]["REAL_WRKR_NAME"].ToString();
                        txtUserName.Tag = dtResult.Rows[0]["USERID"].ToString();
                    }
            ...
            {
                txtUserName.Text = _WorkerName.Split(',')[0];
                txtUserName.Tag = _WorkerID.Split(',')[0];
            }
        ...
        private void cboHoldEnableChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
        ...
        private void cboEndTypeEnableChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
        ...
        private void cboHoldValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            if (cboHold.IsEnabled == true && cboHold.Text == "Y" && cboInHoldCode.Items.Count != 0)
            {
                cboInHoldCode.IsEnabled = true;
        ...
        private void cboEndTypeValueChanged(object sender, PropertyChangedEventArgs<object> e)
        {
            if (cboEndType.IsEnabled == true && cboEndType.Text == "Y" && cboOutHoldCode.Items.Count != 0)
            {
                cboOutHoldCode.IsEnabled = true;
