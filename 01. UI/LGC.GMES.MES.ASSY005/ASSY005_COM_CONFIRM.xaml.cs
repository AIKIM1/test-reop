/*************************************************************************************
 Created Date : 2020.11.18
      Creator : 신광희
   Decription : CNB2동 증설 - 조립 공정 공통 - 실적확정(ASSY004_COM_CONFIRM Copy 후 작성)
--------------------------------------------------------------------------------------
 [Change History]
  2020.11.18  신광희 : Initial Created.
  2022.11.21  김용군 : PACKAGE 실적 확정시 불량수량 음수Validation 기능 추가.
  2023.02.24  김용군 : ESHM 증설 - AZS-STAKING 대응
  2023.03.15  김용군 : ESHM 증설 - AZS_ECUTTER, AZS-STAKING 대응
  2024.02.15  남기운 : (NERP 대응 프로젝트)실적처리 허용비율 초과 입력 기능추가
  2024.05.07  안유수 E20240325-000138 PKG 공급 Carrier 수량 컬럼 추가
  2025.05.23  천진수 : ESHG증설 조립공정진척DNC추가
**************************************************************************************/
...
{
    /// <summary>
    /// ASSY005_COM_CONFIRM.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY005_COM_CONFIRM : C1Window, IWorkArea
    {
        ...
        private void InitializeGrid()
        {
            ...
            dtTmp.Columns.Add("SUPPLY_BOX_QTY", typeof(int));
        ...
        private void InitializeControls()
        {
            ...
            dgQty.Columns["SUPPLY_BOX_QTY"].Visibility = Visibility.Collapsed;

            if (_ProcID.Equals(Process.LAMINATION) || _ProcID.Equals(Process.DNC))      // ESHG증설 조립공정진척DNC추가 250523 천진수
            {
            ...
            // 김용군 AZS_ECUTTER AZS_STACKING
            else if (_ProcID.Equals(Process.AZS_ECUTTER))
            {
                dgQty.Columns["RE_INPUT_QTY"].Visibility = Visibility.Collapsed;
                dgQty.Columns["PRE_SECTION_QTY"].Visibility = Visibility.Collapsed;
                dgQty.Columns["AFTER_SECTION_QTY"].Visibility = Visibility.Collapsed;
        ...
        private double GetLotGoodQty(string sLotID)
        {
            ...
            if (dgQty != null)
            {
                ...
                {
                    goodQty = Util.NVC(DataTableConverter.GetValue(dgQty.Rows[0].DataItem, "INPUTQTY")).Equals("") ? 0 : double.Parse(Util.NVC(DataTableConverter.GetValue(dgQty.Rows[0].DataItem, "INPUTQTY")));
                }
            }
        ...
        private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            // PKG공정 실적확정시 불량수량 음수Validation 기능 추가
            if (_ProcID.Equals(Process.PACKAGING))
            {
                if (!DefectQtyValidation())
                    return;
            }
            ...
            try
            {
                ...
                if (dtRslt != null && dtRslt.Rows.Count > 0)
                {
                    if (dgQty.GetRowCount() > 0)
                    {
                        ...
                        if (dgQty.Columns.Contains("SUPPLY_BOX_QTY") && dtRslt.Columns.Contains("SUPPLY_BOX_QTY"))
                            DataTableConverter.SetValue(dgQty.Rows[dgQty.TopRows.Count].DataItem, "SUPPLY_BOX_QTY", dtRslt.Rows[0]["SUPPLY_BOX_QTY"]);
        ...
        private void Save()
        {
            try
            {
                ...
                {
                    bizName = "BR_PRD_REG_END_LOT_AZC";
                    sInDataName = "INDATA,IN_INPUT,IN_OUTPUT,IN_PROC_WRKR";
                }
                // 김용군 AZS_STACKING
                else if (_ProcID.Equals(Process.AZS_STACKING))
                {
                    bizName = "BR_PRD_REG_END_LOT_AZS_L";
                    sInDataName = "INDATA,IN_INPUT,IN_OUTPUT,IN_PROC_WRKR";
                }
                else if (_ProcID.Equals(Process.DNC))       // ESHG증설 조립공정진척 DNC공정추가 250523 천진수 
                {
                    bizName = "BR_PRD_REG_END_LOT_DNC";
                    sInDataName = "INDATA,IN_INPUT,IN_OUTPUT,IN_PROC_WRKR";
                }
        ...
        private void Save_Rate()
        {
            try
            {
                ...
                else if (_ProcID.Equals(Process.DNC))        // ESHG증설 조립공정진척DNC추가 250523 천진수
                {
                    bizName = "BR_PRD_REG_END_LOT_DNC";
                    sInDataName = "INDATA,IN_INPUT,IN_OUTPUT,IN_PROC_WRKR";
                }
            ...
            try
            {
                ...
                if (dtRslt?.Rows?.Count > 0)
                {
                    if (Util.NVC(dtRslt.Rows[0]["UNLDR_SHR_FLAG"]).Equals("Y"))
                    {
                        ...
                        {
                            dgQty.Columns["SUPPLY_BOX_QTY"].Visibility = Visibility.Visible;
                        }
                    }
                }
            }
        ...
        private void GetAllData()
        {
            ...
            //if (_ProcID.Equals(Process.LAMINATION))
            if (_ProcID.Equals(Process.LAMINATION) || _ProcID.Equals(Process.AZS_ECUTTER) || _ProcID.Equals(Process.DNC))   // ESHG증설 조립공정진척DNC추가 250523 천진수
            {
                GetMagzineList();
                GetDefectInfo();
            }
            //else if (_ProcID.Equals(Process.STACKING_FOLDING))
            else if (_ProcID.Equals(Process.STACKING_FOLDING) || _ProcID.Equals(Process.AZS_STACKING))
            {
                GetBoxList();
                GetDefectInfo();
                if (_UNLDR_SHR_FLAG)
                {
                    ...
                    foreach (DataRowView drv in dtOtherProdLotInfo.DefaultView)
                    {
                        ...
                        dtRow["SUPPLY_BOX_QTY"] = Util.NVC_Decimal(lotInfo.Rows[0]["SUPPLY_BOX_QTY"]);
        ...
        private void GetWorkUser()
        {
            if (string.IsNullOrWhiteSpace(_WorkerID))
            {
                try
                {
                    ...
                    {
                        txtUserName.Text = dtResult.Rows[0]["USERID"].ToString();
                        btnReqUser_Click(null, null);
                    }
                ...
                txtUserName.Text = _WorkerID.Split(',')[0];
                btnReqUser_Click(null, null);
            }
        ...
        private bool DefectQtyValidation()
        {
            bool bRet = false;
            ...
            foreach (C1.WPF.DataGrid.DataGridRow row in dgDefect.Rows)
            {
                ...
                if (dRsn < 0)
                {
                    // PKG 실적 확정 시 불량 수량이 음수값 입니다.
                    Util.MessageValidation("SFU9984");
                    return bRet;
                }
            }
