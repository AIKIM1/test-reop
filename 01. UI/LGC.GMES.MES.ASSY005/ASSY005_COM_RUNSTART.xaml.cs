/*************************************************************************************
 Created Date : 2020.11.13
      Creator : 신광희
   Decription : CNB2동 증설 - 조립 공정 공통 - 작업시작(ASSY004_COM_RUNSTART Copy 후 작성)
--------------------------------------------------------------------------------------
 [Change History]
  2020.11.13  신광희 : Initial Created.
  2023.02.24  김용군 : ESHM 증설 - AZS-STAKING 대응
  2023.03.22  김용군 : ESHM 증설 - AZS-ECUTTER 대응
  2024.01.09  안유수 : E20231205-000745 설비 투입자재 정보 누락시 Interlock 기능 구현
  2024.08.21  김동일 : E20240719-001505 AZS-Stacking 공정 Lot 채번 시 내일 날짜 발번 기능 추가
...
{
    /// <summary>
    /// ASSY005_COM_RUNSTART.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class ASSY005_COM_RUNSTART : C1Window, IWorkArea
    {
        ...
        private void C1Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (string.Equals(_procid, Process.PACKAGING) || string.Equals(_procid, Process.AZS_STACKING))
        ...
        private void btnOK_Click(object sender, RoutedEventArgs e)
        {
            ...
            Util.MessageConfirm("SFU1240", (result) =>
            {
                if (result == MessageBoxResult.OK)
                {
                    ...
                    if (string.Equals(_procid, Process.LAMINATION) || string.Equals(_procid, Process.AZS_ECUTTER))  
                    ...
                    // 김용군
                    else if (string.Equals(_procid, Process.AZS_STACKING))
                        sNewLot = GetNewStkLotId();
                    else if (string.Equals(_procid, Process.DNC))   // 20250428 ESHG DNC공정신설
                        sNewLot = GetNewDNCLotId();
        ...
        //  김용군 AZS Stack PROD_LOTID 발번
        private string GetNewStkLotId()
        {
            try
            {
                ShowLoadingIndicator();
                ...
                newRow["AN_LOT_TYPE"] = Util.GetCondition(cboAnLotType);
                newRow["USERID"] = LoginInfo.USERID;
                newRow["NEXT_DAY_WORK"] = Util.NVC(cboDay.SelectedValue).Equals("TOMORROW") ? "Y" : "N";

                inTable.Rows.Add(newRow);
                newRow = null;

                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_NEW_PROD_LOTID_AZS_L", "IN_EQP,IN_INPUT", "OUTDATA", indataSet);
        ...
        private string GetNewDNCLotId()     // 20250428 ESHG DNC공정신설
        {
            try
            {
                ShowLoadingIndicator();
                ...
                DataSet dsResult = new ClientProxy().ExecuteServiceSync_Multi("BR_PRD_GET_NEW_PROD_LOTID_DNC", "IN_EQP,IN_INPUT", "OUT_LOT", indataSet);
        ...
        private void RunStart(string sNewLot)
        {
            try
            {
                ...
                {
                    sBizName = "BR_PRD_REG_START_PROD_LOT_AZS_L";
                    sRetDataSet = "OUT_LOT";
                }
                // 김용군
                else if (string.Equals(_procid, Process.AZS_ECUTTER))
                {
                    sBizName = "BR_PRD_REG_START_PROD_LOT_AZC";
                    sRetDataSet = "OUT_LOT";
                }
                else if (string.Equals(_procid, Process.DNC))   // 20250428 ESHG DNC공정신설
                {
                    sBizName = "BR_PRD_REG_START_PROD_LOT_DNC";
                    sRetDataSet = "OUT_LOT";
                }
